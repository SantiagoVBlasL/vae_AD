...
# -*- coding: utf-8 -*-
"""
The location of this file is: src/betavae_xai/feature_extraction_manual_COVID.py
CLI-ready version: paths/params can be overridden via argparse to support new datasets (e.g., COVID).
"""
import itertools
...
# SUGERENCIA (Mantenibilidad): Considerar hacer el nivel de logging configurable (ej. vía argparse o var de entorno)
...
# SUGERENCIA (Mantenibilidad): Mover estas constantes a un archivo de configuración (e.g., config.yaml o config.py)
# IMPORTANT:
# - Most of these globals are DEFAULTS for the AAL3/ADNI workflow.
# - They can be overridden at runtime via argparse (see _parse_args / _apply_args_to_globals).
...
# CSV con el ORDEN YEO17 MANUAL para las 131 ROIs finales (tras limpiar AAL3).
# Si no existe, el script va a generar un esqueleto para que lo edites.
AAL3_MANUAL_NETWORK_MAPPING_CSV = BASE_PATH_AAL3 / "aal3_131_manual_network_order.csv"
...
# NOTE: these are recomputed inside _apply_args_to_globals() when args override runtime config.
...
OUTPUT_CONNECTIVITY_DIR_NAME_BASE = f"AAL3_dynamicROIs_fmri_tensor_NeuroEnhanced_v6.5.17"
...
def _parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    """
    Runtime configuration to support multiple datasets (ADNI/AAL3 default, COVID, etc.).

    Typical COVID usage:
      python -m betavae_xai.feature_extraction_manual \\
        --dataset_name COVID \\
        --roi_signals_dir data/ROISignals_FunImgARWSDCFN \\
        --subject_metadata_csv data/covid_metadata_for_pipeline.csv \\
        --qc_report_csv data/covid_qc_dummy.csv \\
        --output_dir_name_base COVID_dynamicROIs_fmri_tensor_NeuroEnhanced_v6.5.17

    If you prefer a fully custom output folder name:
      ... --output_dir_name "AAL3_dynamicROIs_fmri_tensor_COVID_v1_ParallelTuned"
    """
    p = argparse.ArgumentParser(
        description="Connectivity feature extraction pipeline (AAL3 defaults; configurable for other datasets)."
    )

    # --- Dataset / IO paths ---
    p.add_argument("--dataset_name", type=str, default="AAL3",
                   help="Dataset tag used for logging and (optionally) output naming.")
    p.add_argument("--data_root", type=str, default=str(PROJECT_ROOT / "data"),
                   help="Root directory for data assets (atlas files, default I/O).")
    p.add_argument("--roi_signals_dir", type=str, default=str((PROJECT_ROOT / "data") / "ROISignals_AAL3_NiftiPreprocessedAllBatchesNorm"),
                   help="Directory containing ROISignals_*.mat files.")
    p.add_argument("--roi_filename_template", type=str, default="ROISignals_{subject_id}.mat",
                   help="Template for ROI .mat filenames.")
    p.add_argument("--subject_metadata_csv", type=str, default=str((PROJECT_ROOT / "data") / "SubjectsData_AAL3_procesado2.csv"),
                   help="CSV with SubjectID and (optionally) ResearchGroup.")
    p.add_argument("--qc_report_csv", type=str, default=str(PROJECT_ROOT / "notebooks" / "qc_outputs_doctoral" / "report_qc_final_with_discard_flags_v3.2.csv"),
                   help="QC CSV with SubjectID/Subject, ToDiscard_Overall, TimePoints.")
    p.add_argument("--output_root", type=str, default="",
                   help="Root directory for outputs. If empty, uses --data_root.")
    p.add_argument("--output_dir_name_base", type=str, default="AAL3_dynamicROIs_fmri_tensor_NeuroEnhanced_v6.5.17",
                   help="Base string used to build OUTPUT_CONNECTIVITY_DIR_NAME (unless --output_dir_name is provided).")
    p.add_argument("--output_dir_name", type=str, default="",
                   help="If set, overrides the automatically-built OUTPUT_CONNECTIVITY_DIR_NAME entirely.")

    # --- Atlas / ROI processing assets (keep AAL3 defaults; override if needed) ---
    p.add_argument("--aal3_meta_path", type=str, default=str((PROJECT_ROOT / "data") / "ROI_MNI_V7_vol.txt"),
                   help="AAL3 ROI metadata text file used for dropping missing/small ROIs.")
    p.add_argument("--aal3_nifti_path", type=str, default=str((PROJECT_ROOT / "data") / "AAL3v1.nii.gz"),
                   help="AAL3 NIfTI path (only needed if you later use overlap-based mapping).")
    p.add_argument("--aal3_manual_mapping_csv", type=str, default=str((PROJECT_ROOT / "data") / "aal3_131_manual_network_order.csv"),
                   help="Manual mapping CSV used to reorder ROIs by Yeo-17 networks.")

    # --- Time series / filtering ---
    p.add_argument("--tr_seconds", type=float, default=3.0, help="TR in seconds.")
    p.add_argument("--low_cut_hz", type=float, default=0.01, help="Bandpass low cut (Hz).")
    p.add_argument("--high_cut_hz", type=float, default=0.08, help="Bandpass high cut (Hz).")
    p.add_argument("--filter_order", type=int, default=2, help="Butterworth filter order.")
    p.add_argument("--taper_alpha", type=float, default=0.1, help="Tukey taper alpha.")
    p.add_argument("--target_len_ts", type=int, default=140, help="Target #timepoints after homogenization.")

    # --- Raw ROI signals format ---
    p.add_argument("--raw_expected_columns", type=int, default=170,
                   help="Expected number of ROI columns in raw signals (pre AAL3 cleaning).")

    # --- Connectivity hyperparams ---
    p.add_argument("--n_neighbors_mi", type=int, default=5, help="KNN neighbors for MI estimation.")
    p.add_argument("--dfc_win_points", type=int, default=30, help="Window length (in TRs) for dFC.")
    p.add_argument("--dfc_step", type=int, default=5, help="Step (in TRs) for dFC.")
    p.add_argument("--use_granger_channel", action="store_true", default=True, help="Enable Granger channel.")
    p.add_argument("--no_use_granger_channel", action="store_false", dest="use_granger_channel",
                   help="Disable Granger channel.")
    p.add_argument("--granger_max_lag", type=int, default=1, help="Max lag for Granger (in TRs).")

    # --- Parallelism ---
    p.add_argument("--max_workers", type=int, default=0,
                   help="Override MAX_WORKERS. If 0, keep default heuristic.")

    # --- Optional ROI key override ---
    p.add_argument("--possible_roi_keys_csv", type=str, default="",
                   help="Optional comma-separated list of keys to try inside .mat files (overrides POSSIBLE_ROI_KEYS).")
...
def _apply_args_to_globals(args: argparse.Namespace) -> None:
    """
    Apply runtime args to module-level configuration globals.
    Keeps backward compatibility: defaults reproduce the original AAL3 behavior.
    """
    global BASE_PATH_AAL3, QC_OUTPUT_DIR, SUBJECT_METADATA_CSV_PATH, QC_REPORT_CSV_PATH
    global ROI_SIGNALS_DIR_PATH_AAL3, ROI_FILENAME_TEMPLATE
    global AAL3_META_PATH, AAL3_NIFTI_PATH, AAL3_MANUAL_NETWORK_MAPPING_CSV
    global TR_SECONDS, LOW_CUT_HZ, HIGH_CUT_HZ, FILTER_ORDER, TAPER_ALPHA, TARGET_LEN_TS
    global RAW_DATA_EXPECTED_COLUMNS, N_NEIGHBORS_MI, DFC_WIN_POINTS, DFC_STEP
    global USE_GRANGER_CHANNEL, GRANGER_MAX_LAG
    global OUTPUT_CONNECTIVITY_DIR_NAME_BASE, OUTPUT_CONNECTIVITY_DIR_NAME
    global POSSIBLE_ROI_KEYS, deconv_str, granger_suffix_global
    global MAX_WORKERS

    data_root = Path(args.data_root).expanduser().resolve()
    BASE_PATH_AAL3 = data_root
    QC_OUTPUT_DIR = BASE_PATH_AAL3 / 'qc_outputs_doctoral_v3.2_aal3_shrinkage_flexible_thresh_fix'

    SUBJECT_METADATA_CSV_PATH = Path(args.subject_metadata_csv).expanduser().resolve()
    QC_REPORT_CSV_PATH = Path(args.qc_report_csv).expanduser().resolve()
    ROI_SIGNALS_DIR_PATH_AAL3 = Path(args.roi_signals_dir).expanduser().resolve()
    ROI_FILENAME_TEMPLATE = args.roi_filename_template

    AAL3_META_PATH = Path(args.aal3_meta_path).expanduser().resolve()
    AAL3_NIFTI_PATH = Path(args.aal3_nifti_path).expanduser().resolve()
    AAL3_MANUAL_NETWORK_MAPPING_CSV = Path(args.aal3_manual_mapping_csv).expanduser().resolve()
    ...
    TARGET_LEN_TS = int(args.target_len_ts)

    RAW_DATA_EXPECTED_COLUMNS = int(args.raw_expected_columns)
    N_NEIGHBORS_MI = int(args.n_neighbors_mi)
    DFC_WIN_POINTS = int(args.dfc_win_points)
    DFC_STEP = int(args.dfc_step)

    USE_GRANGER_CHANNEL = bool(args.use_granger_channel)
    GRANGER_MAX_LAG = int(args.granger_max_lag)

    OUTPUT_CONNECTIVITY_DIR_NAME_BASE = str(args.output_dir_name_base).strip() or OUTPUT_CONNECTIVITY_DIR_NAME_BASE

    # Recompute derived suffixes (important when args override GRANGER/DECONV)
    deconv_str = "_deconv" if APPLY_HRF_DECONVOLUTION else ""
    granger_suffix_global = f"GrangerLag{GRANGER_MAX_LAG}" if USE_GRANGER_CHANNEL else "NoEffConn"

    # If user fully overrides output dir name, honor it; otherwise init() will build it.
    OUTPUT_CONNECTIVITY_DIR_NAME = str(args.output_dir_name).strip() or None

    # Optional override of ROI keys in .mat
    if args.possible_roi_keys_csv.strip():
        POSSIBLE_ROI_KEYS = [k.strip() for k in args.possible_roi_keys_csv.split(",") if k.strip()]

    # Optional override of MAX_WORKERS
    if int(args.max_workers) > 0:
        MAX_WORKERS = int(args.max_workers)
        logger.info(f"MAX_WORKERS overridden via CLI: {MAX_WORKERS}")

    logger.info(f"Runtime config applied. dataset_name={args.dataset_name}")
    logger.info(f"  BASE_PATH_AAL3 (data_root): {BASE_PATH_AAL3}")
    logger.info(f"  ROI_SIGNALS_DIR_PATH_AAL3 : {ROI_SIGNALS_DIR_PATH_AAL3}")
    logger.info(f"  SUBJECT_METADATA_CSV_PATH : {SUBJECT_METADATA_CSV_PATH}")
    logger.info(f"  QC_REPORT_CSV_PATH        : {QC_REPORT_CSV_PATH}")
    if OUTPUT_CONNECTIVITY_DIR_NAME:
        logger.info(f"  OUTPUT_CONNECTIVITY_DIR_NAME overridden: {OUTPUT_CONNECTIVITY_DIR_NAME}")
    else:
        logger.info(f"  OUTPUT_CONNECTIVITY_DIR_NAME will be auto-built from base: {OUTPUT_CONNECTIVITY_DIR_NAME_BASE}")
...
def _get_aal3_network_mapping_and_order() -> Optional[Dict[str, Any]]:
    """
    Versión MANUAL del mapeo AAL3 -> redes Yeo-17.

    - NO usa NIfTI ni fetch_atlas_yeo_2011.
    - Asume que:
        * VALID_AAL3_ROI_INFO_DF_166 ya contiene las 166 ROIs válidas (sin las 4 missing).
        * INDICES_OF_SMALL_ROIS_TO_DROP_FROM_166 tiene los índices (en ese espacio de 166)
          de los ROIs pequeños que se descartan.
        * FINAL_N_ROIS_EXPECTED == 131 (tras descartar pequeños).

    Flujo:
    1) Construye el dataframe de las 131 ROIs finales.
    2) Si AAL3_MANUAL_NETWORK_MAPPING_CSV NO existe:
         - genera un CSV esqueleto con columnas:
             Index_131, nom_c, nom_l, color, vol_vox, vol_mm3,
             Yeo17_Label_manual, Yeo17_Network_manual, Hemi
         - pide que lo completes a mano y devuelve None (sin reorden).
    3) Si el CSV existe:
         - lo carga, chequea coherencia (mismas 131 ROIs),
         - usa las columnas manuales para:
             * determinar a qué red pertenece cada ROI,
             * definir un orden (por etiqueta de red, hemisferio, nombre),
             * devolver new_order_indices (perm. de 0..130),
               roi_names_new_order, network_labels_new_order.
    """
    logger.info("Attempting to build MANUAL AAL3 -> Yeo-17 mapping and ROI order (no NIfTI overlap).")

    if VALID_AAL3_ROI_INFO_DF_166 is None or \
       INDICES_OF_SMALL_ROIS_TO_DROP_FROM_166 is None or \
       FINAL_N_ROIS_EXPECTED is None:
        logger.error("Global AAL3 processing variables not initialized. Cannot perform manual ROI mapping.")
        return None

    # 1) Dataframe de las 131 ROIs finales, en el orden actual (post-limpieza) que
    #     coincide con las columnas de las series temporales reducidas.
    final_aal3_rois_info_df = VALID_AAL3_ROI_INFO_DF_166.drop(
        INDICES_OF_SMALL_ROIS_TO_DROP_FROM_166
    ).reset_index(drop=True)

    if len(final_aal3_rois_info_df) != FINAL_N_ROIS_EXPECTED:
        logger.error(
            f"Mismatch in expected final ROI count. "
            f"Expected {FINAL_N_ROIS_EXPECTED}, derived {len(final_aal3_rois_info_df)} "
            f"from VALID_AAL3_ROI_INFO_DF_166. Cannot proceed with manual mapping."
        )
        return None

    # 2) Si el CSV manual NO existe: generar esqueleto y pedir intervención del usuario
    if not AAL3_MANUAL_NETWORK_MAPPING_CSV.exists():
        logger.error(
            f"Manual AAL3->Yeo17 mapping CSV NOT found at: {AAL3_MANUAL_NETWORK_MAPPING_CSV}. "
            "Creating a skeleton file. Please edit it to assign Yeo17 networks, then rerun."
        )
        try:
            skeleton_df = final_aal3_rois_info_df.copy()
            # Índice 0..130 que coincide con las columnas de tus series 131×TP
            skeleton_df.insert(0, "Index_131", skeleton_df.index)

            # Hemi tentativa a partir de nom_l (sólo ayuda visual)
            hemi_col = []
            for name in skeleton_df["nom_l"].astype(str):
                if name.endswith("_L"):
                    hemi_col.append("L")
                elif name.endswith("_R"):
                    hemi_col.append("R")
                else:
                    hemi_col.append("U")  # Unknown / Midline / Vermis, etc.
            skeleton_df["Hemi"] = hemi_col

            # Columnas para que las completes a mano
            skeleton_df["Yeo17_Label_manual"] = 0  # 0 = Background/NonCortical por defecto
            skeleton_df["Yeo17_Network_manual"] = YEO17_LABELS_TO_NAMES[0]

            AAL3_MANUAL_NETWORK_MAPPING_CSV.parent.mkdir(parents=True, exist_ok=True)
            skeleton_df.to_csv(AAL3_MANUAL_NETWORK_MAPPING_CSV, index=False)
            logger.info(
                f"Skeleton manual mapping CSV created at: {AAL3_MANUAL_NETWORK_MAPPING_CSV}\n"
                "Please:\n"
                "  - Edit 'Yeo17_Label_manual' with integers 1..17 (0 para subcortical/fondo),\n"
                "  - Optionally edit 'Yeo17_Network_manual' (texto) coherente con YEO17_LABELS_TO_NAMES,\n"
                "  - No modifiques 'Index_131' (es la posición original 0..130)."
            )
        except Exception as e_skel:
            logger.error(f"Could not create skeleton manual mapping CSV: {e_skel}", exc_info=True)

        # Sin mapping manual, devolvemos None (no reorden)
        return None

    # 3) Cargar el CSV manual ya rellenado por vos
    try:
        mapping_df = pd.read_csv(AAL3_MANUAL_NETWORK_MAPPING_CSV)
    except Exception as e_read:
        logger.error(f"Error reading manual mapping CSV {AAL3_MANUAL_NETWORK_MAPPING_CSV}: {e_read}")
        return None

    # Asegurarnos de que tenemos Index_131 o reconstruirlo si no existe
    if "Index_131" not in mapping_df.columns:
        logger.warning(
            "Column 'Index_131' not found in manual mapping CSV. "
            "Attempting to align rows by 'nom_c'."
        )
        # Merge con final_aal3_rois_info_df para recuperar índice 0..130
        merged = pd.merge(
            final_aal3_rois_info_df[["nom_c", "nom_l", "color", "vol_vox", "vol_mm3"]],
            mapping_df,
            on="nom_c",
            how="inner",
            suffixes=("_meta", "_map"),
        )
        if len(merged) != FINAL_N_ROIS_EXPECTED:
            logger.error(
                f"Manual mapping CSV does not match the 131 final ROIs "
                f"(merged rows = {len(merged)}, expected {FINAL_N_ROIS_EXPECTED}). "
                "Check that 'nom_c' in the CSV correspond exactly to the filtered AAL3 ROIs."
            )
            return None
        # El orden 0..130 es el del dataframe final
        merged.insert(0, "Index_131", merged.index)
        mapping_df = merged
    else:
        # Chequeo de coherencia básica del índice
        idx_vals = mapping_df["Index_131"].astype(int).values
        if len(idx_vals) != FINAL_N_ROIS_EXPECTED or \
           sorted(idx_vals) != list(range(FINAL_N_ROIS_EXPECTED)):
            logger.error(
                "Column 'Index_131' in manual mapping CSV does not contain a "
                f"permutation of [0..{FINAL_N_ROIS_EXPECTED-1}]. "
                "Please ensure it was not modified or regenerate the skeleton."
            )
            return None

        # Ordenamos por Index_131 para alinear con las 131 columnas de las series
        mapping_df = mapping_df.sort_values("Index_131").reset_index(drop=True)

        # Chequeo suave de que los nombres coinciden
        if "nom_c" in mapping_df.columns:
            if not np.array_equal(
                mapping_df["nom_c"].values.astype(str),
                final_aal3_rois_info_df["nom_c"].values.astype(str),
            ):
                logger.warning(
                    "Warning: 'nom_c' order in manual CSV does not match 'final_aal3_rois_info_df'. "
                    "Using 'Index_131' as ground truth for column positions, but you may want "
                    "to regenerate the skeleton CSV to avoid confusion."
                )

    # Elegimos columnas para etiqueta de red y nombre de red (flexible por si cambiás nombres)
    label_col_candidates = ["Yeo17_Label_manual", "Yeo17_Label", "Consensus_Label", "NetworkID"]
    network_name_candidates = ["Yeo17_Network_manual", "Yeo17_Network", "NetworkName", "Network"]

    label_col = next((c for c in label_col_candidates if c in mapping_df.columns), None)
    network_name_col = next((c for c in network_name_candidates if c in mapping_df.columns), None)

    if label_col is None:
        logger.warning(
            f"No label column found among {label_col_candidates}. "
            "Assuming all ROIs are Background/NonCortical (label=0)."
        )
        mapping_df["Yeo17_Label_manual"] = 0
        label_col = "Yeo17_Label_manual"

    if network_name_col is None:
        # Convertimos label->nombre usando YEO17_LABELS_TO_NAMES si se puede
        mapping_df["Yeo17_Network_manual"] = mapping_df[label_col].apply(
            lambda lab: YEO17_LABELS_TO_NAMES.get(lab, f"UnknownYeo{lab}")
        )
        network_name_col = "Yeo17_Network_manual"

    # Construimos claves de ordenamiento:
    #  - primero redes corticales (label > 0),
    #  - luego fondo/subcortical (label <= 0),
    #  - dentro de cada red, por hemisferio y nombre.
    labs = mapping_df[label_col].astype(int)
    mapping_df["__sort_is_bg"] = (labs <= 0).astype(int)

    # Hemisferio: si hay columna Hemi la usamos, si no la inferimos de nom_l
    if "Hemi" in mapping_df.columns:
        hemi_raw = mapping_df["Hemi"].astype(str).str.upper()
    else:
        hemi_raw = []
        for name in mapping_df.get("nom_l", mapping_df.get("nom_c", "")).astype(str):
            if name.endswith("_L"):
                hemi_raw.append("L")
            elif name.endswith("_R"):
                hemi_raw.append("R")
                ...
                hemi_raw.append("U")
        hemi_raw = pd.Series(hemi_raw)

    hemi_rank = hemi_raw.map({"L": 0, "R": 1}).fillna(2).astype(int)
    mapping_df["__sort_hemi"] = hemi_rank

    # Nombre para ordenar dentro de la red
    name_for_sort = mapping_df.get("nom_l", mapping_df.get("nom_c"))
    mapping_df["__sort_name"] = name_for_sort.astype(str)

    mapping_df_sorted = mapping_df.sort_values(
        ["__sort_is_bg", label_col, "__sort_hemi", "__sort_name"],
        kind="mergesort"  # estable
    ).reset_index(drop=True)

    # Estos índices 0..130 son los que hay que aplicar a las columnas de las series/matrices 131×131
    new_order_indices = mapping_df_sorted["Index_131"].astype(int).tolist()

    if len(new_order_indices) != FINAL_N_ROIS_EXPECTED or len(set(new_order_indices)) != FINAL_N_ROIS_EXPECTED:
        logger.error(
            "Error building new_order_indices for manual ROI reordering. "
            "Length or uniqueness is incorrect."
        )
        return None

    # Nombres y etiquetas de red en el nuevo orden
    roi_names_original_order = final_aal3_rois_info_df["nom_l"].astype(str).tolist()
    roi_names_new_order = mapping_df_sorted.get("nom_l", mapping_df_sorted["nom_c"]).astype(str).tolist()
    network_labels_new_order = mapping_df_sorted[network_name_col].astype(str).tolist()

    logger.info(
        "Successfully built MANUAL AAL3->Yeo17 mapping and new ROI order "
        f"from CSV: {AAL3_MANUAL_NETWORK_MAPPING_CSV}"
    )

    return {
        "order_name": "aal3_manual_yeo17_order",
        "roi_indices_original_order": list(range(FINAL_N_ROIS_EXPECTED)),
        "roi_names_original_order": roi_names_original_order,
...
def _initialize_aal3_roi_processing_info(): # Definida ANTES de su llamada a nivel de módulo
    ...
    # If user provided explicit output dir name, keep it (do not overwrite).
    # Still initialize ROI info/channels and return True.
    explicit_output_dir_name = OUTPUT_CONNECTIVITY_DIR_NAME
    ...
    # Simplificar la lógica del nombre del directorio (solo si no fue override por CLI)
    if explicit_output_dir_name:
        OUTPUT_CONNECTIVITY_DIR_NAME = explicit_output_dir_name
    else:
...
def main(argv: Optional[List[str]] = None):
    # --- 0) Parse args and apply runtime configuration ---
    args = _parse_args(argv)
    _apply_args_to_globals(args)

    # --- 1) Initialize ROI processing + channel list AFTER applying args ---
