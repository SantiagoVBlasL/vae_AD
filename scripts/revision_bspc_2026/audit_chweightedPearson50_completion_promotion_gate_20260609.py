#!/usr/bin/env python3
"""Read-only completion and promotion-gate audit for chweightedPearson50 candidate.

Candidate:
  recover035_latent384_beta3p75_chweightedPearson50_T80_h10000_p560_full5x5

Scientific question:
  Does Pearson-heavy weighted off-diagonal reconstruction loss
  (weights=[0.5, 0.25, 0.25] for channels [Pearson_Full, Pearson_OMST, MI_KNN])
  preserve the Pearson-Full discriminative axis inside the multichannel [1,0,2] VAE
  and improve ADNI OOF performance vs equal-weight chmeanloss and the promoted T80?

Comparison set:
  - promoted [1,0,2] beta3.75 T80 (primary reference)
  - ch1-only beta3.75 T80
  - plus-ch1 PR-recovery meta-logreg candidates (external batch)
  - beta9.5 T80
  - T160
  - beta6.5 T80
  - chmeanloss T80 (direct loss comparison)
...
OUT_DEFAULT = RESULTS / "chweightedPearson50_completion_promotion_gate_audit_20260609"
...
class RunSpec:
    ...
    role: str
    lr_T0: int = 80
    recon_loss_mode: str = "mse_offdiag"
    channel_weights: str = "equal"


RUNS = [
    RunSpec(
        "candidate_latent384_beta3p75_chweighted_T80",
        ...
        "candidate",
        lr_T0=80,
        recon_loss_mode="mse_offdiag_channel_weighted_sum",
    ...
    RunSpec(
        ...
        "loss_reference",
        lr_T0=80,
        recon_loss_mode="mse_offdiag_channel_mean",
        channel_weights="equal",
    ),
]
...
def stagea_summary(stagea: pd.DataFrame) -> pd.DataFrame:
    ...
    metric_cols = [
        c
        for c in [
            "auc_raw", "pr_auc_raw", "auc_final", "pr_auc_final",
            "auc", "pr_auc", "balanced_accuracy", "sensitivity", "specificity", "f1_score",
        ]
...
def stageb_all_pooled(run: RunSpec) -> pd.DataFrame:
    ...
    keep = [
        "run_id", "source", "model_name", "feature_set", "calib_method", "threshold_strategy",
        "n", "n_cn", "n_ad", "tn", "fp", "fn", "tp",
        "sensitivity", "specificity", "balanced_accuracy", "f1",
        "predicted_ad_rate", "auc", "pr_auc", "status",
    ]
    for col in keep:
...
def plus_ch1_external_rows() -> pd.DataFrame:
    """Load plus-ch1 PR-recovery candidates as external reference rows."""
    ...
    for _, row in df.iterrows():
        ...
        if cid.startswith("reference_"):
            continue
        rows.append({
            "run_id": f"plus_ch1__{cid}",
            "source": "plus_ch1_external_batch",
            "model_name": "meta_logreg",
            "feature_set": "promoted_plus_ch1_scores",
            "calib_method": "oof_ecdf",
            "threshold_strategy": PRIMARY_THRESHOLD,
            "n": safe_float(row.get("n")),
            "n_cn": safe_float(row.get("n_cn")),
            "n_ad": safe_float(row.get("n_ad")),
            "tn": safe_float(row.get("tn")),
            "fp": safe_float(row.get("fp")),
            "fn": safe_float(row.get("fn")),
...
def focused_stageb_rows(pooled: pd.DataFrame) -> pd.DataFrame:
    ...
    status = standard.get("status", pd.Series(index=standard.index, dtype=object)).astype(str)
    standard = standard[~status.eq("missing_classifier_only_or_oof_artifacts")].copy()
    missing = pooled[
        pooled.get("status", pd.Series(index=pooled.index, dtype=object)).astype(str)
        .eq("missing_classifier_only_or_oof_artifacts")
    ]
    external = pooled[pooled["source"].astype(str).eq("plus_ch1_external_batch")].copy()
    rows = pd.concat([standard, external, missing], ignore_index=True)
...
def primary_oof_rows(pooled: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = pd.concat([standard, missing], ignore_index=True)
...
def plus_ch1_manufacturer_errors() -> pd.DataFrame:
    """Load per-manufacturer errors for plus-ch1 candidates from external batch."""
    path = PLUS_CH1_BATCH / "manufacturer_error_by_candidate.csv"
    df = read_csv(path)
    if df.empty:
        return pd.DataFrame()
    rows = df[~df["candidate_id"].astype(str).str.startswith("reference_")].copy()
    rows["run_id"] = "plus_ch1__" + rows["candidate_id"].astype(str)
    rename_map = {}
    for col in ["philips_cn_fpr", "Philips_cn_fpr"]:
        if col in rows.columns:
            rename_map[col] = "cn_fpr_philips"
    if rename_map:
        ...
        rows.append(
            {
                ...
                "recon_loss_mode": run.recon_loss_mode,
                "channel_weights": run.channel_weights,
...
def promotion_gate(
    ...
    cand = "candidate_latent384_beta3p75_chweighted_T80"
...
def channel_weighting_hypothesis_text(rd_summary: pd.DataFrame, primary: pd.DataFrame, latent_summary: pd.DataFrame) -> str:
    """Narrative: does Pearson-heavy weighting improve over equal chmeanloss?"""
    ...
    def mi_for(run_id: str, split: str, col: str) -> float:
        row = latent_summary[(latent_summary["run_id"] == run_id) & (latent_summary["split"] == split)]
        if row.empty:
            return float("nan")
        mean_col = f"{col}_mean"
        if mean_col in row.columns:
    ...
    cand_id = "candidate_latent384_beta3p75_chweighted_T80"
    chmean_id = "chmeanloss_latent384_beta3p75_T80"
    prom_id = "promoted_latent384_beta3p75_T80"
    ch1_id = "ch1only_latent384_beta3p75_T80"

    cand_auc = get_primary_metric(primary, cand_id, PRIMARY_CALIB, "auc")
    cand_pr = get_primary_metric(primary, cand_id, PRIMARY_CALIB, "pr_auc")
    cand_fpr = get_primary_metric(primary, cand_id, PRIMARY_CALIB, "balanced_accuracy")
    chmean_auc = get_primary_metric(primary, chmean_id, PRIMARY_CALIB, "auc")
    chmean_pr = get_primary_metric(primary, chmean_id, PRIMARY_CALIB, "pr_auc")
    prom_auc = get_primary_metric(primary, prom_id, PRIMARY_CALIB, "auc")
    prom_pr = get_primary_metric(primary, prom_id, PRIMARY_CALIB, "pr_auc")
    ch1_auc = get_primary_metric(primary, ch1_id, PRIMARY_CALIB, "auc")
    ch1_pr = get_primary_metric(primary, ch1_id, PRIMARY_CALIB, "pr_auc")

    cand_reg = mean_for(cand_id, "beta_KLD_over_D_best")
    chmean_reg = mean_for(chmean_id, "beta_KLD_over_D_best")
    prom_reg = mean_for(prom_id, "beta_KLD_over_D_best")
    ch1_reg = mean_for(ch1_id, "beta_KLD_over_D_best")

    cand_mi_y = mi_for(cand_id, "test", "MI_Z_Y_nats")
    cand_mi_mfr = mi_for(cand_id, "test", "MI_Z_Manufacturer_nats")
    cand_ratio = mi_for(cand_id, "test", "MI_Manufacturer_over_MI_Y")
    chmean_mi_y = mi_for(chmean_id, "test", "MI_Z_Y_nats")
    chmean_ratio = mi_for(chmean_id, "test", "MI_Manufacturer_over_MI_Y")

    auc_vs_chmean = cand_auc - chmean_auc if not (math.isnan(cand_auc) or math.isnan(chmean_auc)) else float("nan")
    pr_vs_chmean = cand_pr - chmean_pr if not (math.isnan(cand_pr) or math.isnan(chmean_pr)) else float("nan")
    auc_vs_prom = cand_auc - prom_auc if not (math.isnan(cand_auc) or math.isnan(prom_auc)) else float("nan")
    pr_vs_prom = cand_pr - prom_pr if not (math.isnan(cand_pr) or math.isnan(prom_pr)) else float("nan")

    improved_over_chmean = (
        (not math.isnan(auc_vs_chmean) and auc_vs_chmean > 0)
        or (not math.isnan(pr_vs_chmean) and pr_vs_chmean > 0)
    )

    lines = [
        "# Channel-Weighting Hypothesis Test\n\n",
        "Question: does Pearson-heavy weighting [0.5,0.25,0.25] preserve the Pearson-Full\n",
        "discriminative axis and improve OOF performance over equal chmeanloss?\n\n",
        "## AUC comparison (OOF-ECDF)\n",
    ]
    fmt = lambda v: f"{v:.6f}" if not math.isnan(v) else "missing"
    lines += [
        f"- chweighted AUC={fmt(cand_auc)}, PR-AUC={fmt(cand_pr)}\n",
        f"- chmeanloss AUC={fmt(chmean_auc)}, PR-AUC={fmt(chmean_pr)}\n",
        f"- promoted (equal offdiag) AUC={fmt(prom_auc)}, PR-AUC={fmt(prom_pr)}\n",
        f"- ch1-only AUC={fmt(ch1_auc)}, PR-AUC={fmt(ch1_pr)}\n\n",
        f"Delta chweighted vs chmeanloss: ΔAUC={fmt(auc_vs_chmean)}, ΔPR-AUC={fmt(pr_vs_chmean)}\n",
        f"Delta chweighted vs promoted:   ΔAUC={fmt(auc_vs_prom)}, ΔPR-AUC={fmt(pr_vs_prom)}\n\n",
        f"chweighted improved over chmeanloss: **{improved_over_chmean}**\n\n",
        "## Effective regularization (beta*KLD/D)\n",
        f"- chweighted: {fmt(cand_reg)}\n",
        f"- chmeanloss: {fmt(chmean_reg)}\n",
        f"- promoted (equal offdiag): {fmt(prom_reg)}\n",
        f"- ch1-only: {fmt(ch1_reg)}\n\n",
        "## Latent signal/nuisance MI (test split)\n",
        f"- chweighted MI(Z;Y)={fmt(cand_mi_y)}, MI(Z;Mfr)={fmt(cand_mi_mfr)}, ratio={fmt(cand_ratio)}\n",
        f"- chmeanloss MI(Z;Y)={fmt(chmean_mi_y)}, ratio={fmt(chmean_ratio)}\n\n",
        "## Interpretation\n",
        "Pearson-heavy weighting up-weights the Pearson_Full channel (weight=0.5 vs 0.333 equal)\n",
        "during reconstruction. If the Pearson channel contains the discriminative AD signal,\n",
        "heavier weighting should increase MI(Z;Y) or AUC relative to equal-weight chmeanloss.\n",
        "The channel_weighted_sum loss does not affect the ELBO constraint (beta*KLD) directly,\n",
        "but shifts what the encoder is asked to reconstruct faithfully.\n",
    ]
    return "".join(lines)
...
def final_decision_text(gate: pd.DataFrame, decision: str, primary: pd.DataFrame) -> str:
    ...
    if cand_rows.empty:
        row_text = "- No chweighted primary Stage B rows were available."
    else:
        lines = []
...
def plus_ch1_summary_table(plus_ch1_df: pd.DataFrame) -> pd.DataFrame:
    """Summary table of plus-ch1 PR-recovery candidates for easy comparison."""
    if plus_ch1_df.empty:
        return pd.DataFrame()
    cols = ["run_id", "auc", "pr_auc", "balanced_accuracy", "sensitivity", "specificity", "f1", "status"]
    for col in cols:
        if col not in plus_ch1_df.columns:
            plus_ch1_df[col] = np.nan
    return plus_ch1_df[cols].copy()
...
def main() -> None:
    ...
    planned = [
        ...
        "plus_ch1_reference_summary.csv/.md",
        ...
        "channel_weighting_hypothesis_test.md",
        "final_decision.md",
        "command_log.json",
    ]
    if args.dry_run:
    ...
    pooled_vae = pd.concat([stageb_all_pooled(run) for run in available_runs], ignore_index=True)
    plus_ch1 = plus_ch1_external_rows()
    pooled = pd.concat([pooled_vae, plus_ch1], ignore_index=True) if not plus_ch1.empty else pooled_vae

    focused = focused_stageb_rows(pooled)
    write_table(focused, "stageb_oof_calibration_metrics", out)
    primary = primary_oof_rows(pooled_vae)  # gate uses only VAE runs
    ...
    write_table(plus_ch1_summary_table(plus_ch1), "plus_ch1_reference_summary", out)

    mfr_errors_vae = pd.concat([manufacturer_errors_from_predictions(run) for run in available_runs], ignore_index=True)
    mfr_errors = mfr_errors_vae  # plus-ch1 has different format, kept separate
    ...
    rd_metrics = [
        ...
        "D_val_best", "R_val_bits_best", "bits_per_dim_best",
        "KLD_over_D_best", "beta_KLD_over_D_best",
        "D_val_final", "R_val_bits_final", "bits_per_dim_final",
        "KLD_over_D_final", "beta_KLD_over_D_final",
    ]
    ...
    latent_metrics = [
        "MI_Z_Y_nats", "MI_Z_Manufacturer_nats", "MI_Manufacturer_over_MI_Y",
        "active_units", "frac_active", "total_correlation_nats",
    ]
    ...
    (out / "channel_weighting_hypothesis_test.md").write_text(
        channel_weighting_hypothesis_text(rd_summary, primary, latent_summary),
        encoding="utf-8",
    )
    (out / "final_decision.md").write_text(final_decision_text(gate, decision, primary), encoding="utf-8")

    cand_global = status[
        (status["run_id"].eq("candidate_latent384_beta3p75_chweighted_T80"))
    ...
    readme = f"""# chweightedPearson50 Completion and Promotion-Gate Audit

Candidate: `recover035_latent384_beta3p75_chweightedPearson50_T80_h10000_p560_full5x5`.

Scientific question: does Pearson-heavy weighted off-diagonal reconstruction loss
(weights=[0.5, 0.25, 0.25] for channels [Pearson_Full, Pearson_OMST, MI_KNN])
preserve the Pearson-Full discriminative axis inside the multichannel [1,0,2] VAE
and improve ADNI OOF performance?

This audit compares chweightedPearson50 against:
- promoted [1,0,2] beta3.75 T80 (primary reference)
- ch1-only beta3.75 T80
- plus-ch1 PR-recovery meta-logreg candidates (external batch)
- beta9.5 T80
- T160 (scheduler reference)
- beta6.5 T80
- chmeanloss T80 (direct loss comparison)
...
"""
    ...
    command_log = {
        ...
        "candidate": "recover035_latent384_beta3p75_chweightedPearson50_T80_h10000_p560_full5x5",
        "scientific_question": (
            "Does Pearson-heavy weighted reconstruction loss [0.5,0.25,0.25] "
            "improve ADNI OOF performance over equal chmeanloss and promoted T80?"
        ),
        ...
        "plus_ch1_external_batch": rel(PLUS_CH1_BATCH),
        "primary_gate": {
            "model_name": PRIMARY_MODEL_OOF,
            "feature_set": PRIMARY_FEATURES,
            "calib_method": PRIMARY_CALIB,
