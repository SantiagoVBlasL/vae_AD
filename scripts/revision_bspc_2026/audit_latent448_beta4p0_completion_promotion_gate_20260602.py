#!/usr/bin/env python3
"""Read-only promotion-gate audit for latent448 beta4.0 vs promoted latent384 beta3.75.
...
CANDIDATE_RUN = ROOT / "results/revision_bspc_2026/recover035_latent448_beta4p0_T80_h10000_p560_full5x5"
CANDIDATE_OOF = ROOT / "results/revision_bspc_2026/recover035_latent448_beta4p0_stageB_oof_score_calibration"
...
OUT = ROOT / "results/revision_bspc_2026/latent448_beta4p0_completion_promotion_gate_audit_20260602"
...
def oasis_available_comparison() -> pd.DataFrame:
    ...
    out["candidate_latent448_available"] = out["adni_model"].astype(str).str.contains("latent448|beta4p0", case=False, regex=True)
    if not out["candidate_latent448_available"].any():
        note = {
            "status": "not_available_for_candidate",
            "build_candidate": "all",
            "adni_model": "recover035_latent448_beta4p0",
            ...
            "candidate_latent448_available": False,
        }
...
def promotion_gate(
    ...
    cand = primary_pooled_row(pooled, "candidate_latent448_beta4p0")
    ...
    cand_phil = philips[
        (philips["run_label"] == "candidate_latent448_beta4p0")
    ...
    if not leak.empty:
        for run_label, holder in [
            ("candidate_latent448_beta4p0", "cand"),
    ...
    if bool(gate_df["passes"].all()):
        ...
        decision = "capacity_sensitivity_only"
...
def main() -> None:
    ...
    cand_status = completion_status(CANDIDATE_RUN, CANDIDATE_OOF, "candidate_latent448_beta4p0")
    ...
    cand_stagea, cand_stagea_summary = load_stagea(CANDIDATE_RUN, "candidate_latent448_beta4p0")
    ...
    pooled = pd.concat(
        [
            pooled_metrics(CANDIDATE_RUN, CANDIDATE_OOF, "candidate_latent448_beta4p0"),
    ...
    cand_fold = selected_foldwise(foldwise_oof(CANDIDATE_OOF, "candidate_latent448_beta4p0"), PRIMARY_CALIB)
    ...
    vae_qc = pd.concat(
        [
            load_vae_qc(CANDIDATE_RUN, "candidate_latent448_beta4p0"),
    ...
    scanner = pd.concat(
        [
            load_scanner_leakage(CANDIDATE_RUN, "candidate_latent448_beta4p0"),
    ...
    philips = pd.concat(
        [
            load_philips_pooled(CANDIDATE_OOF, "candidate_latent448_beta4p0"),
    ...
    cand_pred = primary_predictions(CANDIDATE_OOF, "candidate_latent448_beta4p0")
    ref_pred = primary_predictions(REFERENCE_OOF, "promoted_reference_latent384_beta3p75")
    by_age_site = pd.concat(
        [
            philips_by_age_phase_site(cand_pred, "candidate_latent448_beta4p0"),
    ...
    cand_primary = primary_pooled_row(pooled, "candidate_latent448_beta4p0")
    ...
    if cand_primary is None or ref_primary is None:
        ...
        primary_text = (
            ...
            f"- Candidate latent448/beta4.0: AUC={cand_primary['auc']:.6f}, "
...
Interpretation:
The latent448/beta4.0 capacity-beta candidate is complete and valid as a sensitivity run, but it does not meet the pre-specified promotion gate against the promoted latent384/beta3.75 model. The primary ranking metrics are lower, and the candidate should not replace the promoted reference. It can be retained as a capacity/beta-axis negative or neutral audit.

OASIS:
Candidate-specific mega-OASIS 90/90 scoring artifacts were not found. The OASIS table in this package reports available reference rows and explicitly marks latent448 as not scored, so no external-performance inference is made for this candidate.
"""
    write_text("final_decision.md", decision_text, out)

    readme = f"""# Latent448 Beta4.0 Completion and Promotion-Gate Audit

This package compares `recover035_latent448_beta4p0_T80_h10000_p560_full5x5` against the promoted `recover035_latent384_beta3p75_T80_h10000_p560_full5x5` reference.
