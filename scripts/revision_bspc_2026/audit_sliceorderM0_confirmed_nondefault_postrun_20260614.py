#!/usr/bin/env python3
"""Read-only postrun audit for slice-order M0 confirmed-nondefault sensitivity."""
...
RUN = RESULTS / "recover035_latent384_beta3p75_T80_h10000_p560_full5x5_sliceorderM0_confirmed_nondefault_only_sensitivity_20260614"
PREFLIGHT = RESULTS / "slice_order_M0_confirmed_nondefault_only_sensitivity_20260614"
M1_AUDIT = RESULTS / "sliceorderM1_validonly_postrun_audit_20260614"
...
OUT = RESULTS / "sliceorderM0_confirmed_nondefault_postrun_audit_20260614"
...
def completion_status() -> pd.DataFrame:
    ...
    rows.append({
        ...
        "stageB_oof_score_calibration_exists": any(RESULTS.glob("*sliceorderM0*score_calibration*")),
    })
...
def promoted_m0_scoreonly() -> dict[str, Any]:
    p = PREFLIGHT / "locked_model_score_only_M0.csv"
    df = pd.read_csv(p)
    r = df.iloc[0]
    return {
        "comparison_row": "promoted_retained_set_score_only_M0",
        "source": str(p.relative_to(PROJECT_ROOT)),
        "AUC": r["AUC"],
        "PR_AUC": r["PR_AUC"],
        "BA": r["BA"],
...
def m1_retrained_rows() -> list[dict[str, Any]]:
    p = M1_AUDIT / "promoted_vs_m1_comparison.csv"
    if not p.exists():
        return []
    df = pd.read_csv(p)
    rows = []
    for _, r in df[df["comparison_row"].astype(str).str.startswith("M1_retrained_StageA_")].iterrows():
        rows.append({
            "comparison_row": r["comparison_row"],
            "source": str(p.relative_to(PROJECT_ROOT)),
            "AUC": r["AUC"],
            "PR_AUC": r["PR_AUC"],
            "BA": r["BA"],
...
def comparison_table(pooled: pd.DataFrame, manufacturer_df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows.append(prom)
    rows.append(promoted_m0_scoreonly())
    rows.extend(m1_retrained_rows())
    for clf in ["logreg", "svm"]:
        for score_type in ["final_score"]:
            ...
            r = sub.iloc[0]
            rows.append({
                "comparison_row": f"M0_retrained_StageA_{clf}_{score_type}",
                "source": "M0 all_folds_clf_predictions",
                "AUC": r["AUC"],
                "PR_AUC": r["PR_AUC"],
                "BA": r["BA"],
                "Sensitivity": r["Sensitivity"],
...
def threshold_diagnostic(pooled: pd.DataFrame, stagea: pd.DataFrame, comparison: pd.DataFrame) -> str:
    lines = ["# Threshold / Operating-Point Diagnostic", ""]
    lines.append("No new threshold fitting or calibration was performed in this audit.")
    lines.append("")
    lines.append("## Available outputs")
    lines.append("")
    lines.append("- M0 Stage A fold predictions exist for logreg and SVM.")
    lines.append(f"- `classifier_only_readout` exists: `{(RUN / 'classifier_only_readout').exists()}`")
    lines.append(f"- M0 OOF score-calibration directory exists: `{any(RESULTS.glob('*sliceorderM0*score_calibration*'))}`")
    lines.append("")
    ...
    lines.append("The M0 run currently has Stage A predictions but does not have the separate classifier-only latent-cache readout or OOF-ECDF calibration artifacts.")
    lines.append("")
    stageb_cmd = (
    ...
    calib_cmd = (
        ...
        f"--run-dir {RUN} "
        f"--output-dir {RESULTS / 'recover035_latent384_beta3p75_sliceorderM0_stageB_oof_score_calibration_20260614'} "
    ...
    lines.append("")
    lines.append("Rank separation and operating-point behavior are both assessed from the available Stage A predictions only. If AUC/PR-AUC are below the promoted reference, the issue is not threshold-only. If specificity is high but sensitivity is low, the threshold/calibration operating point is also conservative. Stage B OOF-ECDF is required before making a final promoted-convention comparison.")
    return "\n".join(lines) + "\n"
...
def scanner_leakage_md(scanner: pd.DataFrame, latent_mi: pd.DataFrame) -> str:
    ...
    if not latent_mi.empty:
        lines.append("")
        lines.append("Latent MI summaries are available in `latent_mi_signal_nuisance_summary_m0.csv`.")
...
def final_text(comparison: pd.DataFrame, manufacturer: pd.DataFrame) -> tuple[str, str]:
    ...
    m0_log = comparison[comparison["comparison_row"].eq("M0_retrained_StageA_logreg_final_score")]
    m0_svm = comparison[comparison["comparison_row"].eq("M0_retrained_StageA_svm_final_score")]
    log = m0_log.iloc[0] if not m0_log.empty else None
    svm = m0_svm.iloc[0] if not m0_svm.empty else None
    stageb_missing = not (RUN / "classifier_only_readout").exists() or not any(RESULTS.glob("*sliceorderM0*score_calibration*"))
    decision = "incomplete_needs_stageB_oof_ecdf" if stageb_missing else "neutral_sensitivity"
    if log is not None:
        if (
            ...
            decision = "incomplete_needs_stageB_oof_ecdf" if stageb_missing else "negative_sensitivity"
        else:
            decision = "incomplete_needs_stageB_oof_ecdf" if stageb_missing else "neutral_sensitivity"
    interp = ["# Final Interpretation", ""]
    interp.append("The exploratory M0 confirmed-nondefault slice-order run completed Stage A for all five folds, but promoted-convention Stage B classifier-only OOF-ECDF artifacts are not present yet.")
    interp.append("")
    if log is not None:
        interp.append(
    ...
    interp.append("")
    interp.append(
        "The score-only retained-set comparison is mechanical because it removes the five confirmed Site31 reverse/non-default Philips CN subjects from the promoted predictions; it is not a new model."
    )
    interp.append(
        "The strict positive-sensitivity rule is not met by the available Stage A outputs unless AUC/PR-AUC and BA/Sensitivity/F1 improve together. Because Stage B OOF-ECDF is missing, the promoted-convention comparison remains incomplete."
    )
    interp.append("")
    ...
    dec.append("- M0 excluded only the five confirmed Site31 reverse/non-default Philips CN subjects, so it is narrower than M1.")
    dec.append("- Available Stage A metrics are not sufficient to promote M0 without the predefined Stage B OOF-ECDF readout.")
    dec.append("- The promoted model remains primary unless later Stage B OOF-ECDF meets the strict gate.")
    return "\n".join(interp) + "\n", "\n".join(dec) + "\n"
...
def main() -> int:
    ...
    write_df("pooled_stagea_metrics", pooled, max_rows=20)
    write_df("promoted_vs_m0_vs_m1_comparison", comparison, max_rows=30)
    write_df("manufacturer_error_m0", manufacturer, max_rows=160)
    write_df("philips_cn_fp_remaining_m0", high_fp, max_rows=120)
    write_df("training_curve_summary", train_curve, max_rows=20)
    write_df("scanner_leakage_summary_m0", scanner, max_rows=30)
    write_df("rate_distortion_summary_m0", rd, max_rows=20)
    write_df("latent_mi_signal_nuisance_summary_m0", latent_mi, max_rows=40)

    (OUT / "threshold_operating_point_diagnostic.md").write_text(threshold_diagnostic(pooled, stagea, comparison), encoding="utf-8")
    (OUT / "stageB_missing_steps.md").write_text(threshold_diagnostic(pooled, stagea, comparison), encoding="utf-8")
    (OUT / "scanner_leakage_summary_m0.md").write_text(scanner_leakage_md(scanner, latent_mi), encoding="utf-8")
    (OUT / "rate_distortion_summary_m0.md").write_text(rate_distortion_md(rd), encoding="utf-8")
    ...
    write_json(OUT / "command_log.json", {
        ...
        "guardrails": {
            ...
            "m0_run_modified": False,
            "promoted_run_modified": False,
        },
        "stageB_oof_calibration_present": any(RESULTS.glob("*sliceorderM0*score_calibration*")),
