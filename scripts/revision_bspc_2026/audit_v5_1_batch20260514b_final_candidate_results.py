#!/usr/bin/env python3
"""Audit ADNI v5.1 batch20260514b final-candidate baseline ML results.
...
OUTPUT_DIR = (
    ...
    / "results"
    / "revision_bspc_2026"
    / "adni_v5_1_batch20260514b_final_candidate_audit"
)
...
COMPARISON_RUNS = [
    {
        "version": "v5_1_batch20260514b_ch1_0_2",
        "metrics_dir": BIG_RUN_DIR,
        "metadata_path": METADATA_PATH,
        "channels": "[1,0,2]",
    },
    {
        "version": "v5_1_batch20260514_interim_ch1_0_2",
        "metrics_dir": Path(
            "/media/diego/Datos/vae_AD_results/revision_bspc_2026/"
...
def make_foldwise_metrics(metrics: pd.DataFrame, pred: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in metrics.iterrows():
        ...
        out = {
            "fold": fold,
            "classifier": clf,
            "best_clf_params": row.get("best_clf_params", ""),
...
def fold3_error_audit(pred: pd.DataFrame, foldwise: pd.DataFrame) -> pd.DataFrame:
    fold3 = pred[pred["fold"].eq(3)].copy()
    rows: List[Dict[str, Any]] = []
    summary_cols = ["manufacturer_label", "Sex", "diagnosis_label", "source_batch", "source_label", "tensor_source"]
    for _, r in foldwise.sort_values(["classifier", "fold"]).iterrows():
        rows.append(
            {
                "row_type": "fold3_vs_other_folds",
                "classifier": r.get("classifier"),
                "fold": int(r.get("fold")),
                "is_fold3": bool(int(r.get("fold")) == 3),
                "auc_final": r.get("auc_final"),
                "pr_auc_final": r.get("pr_auc_final"),
    ...
    for clf, clf_df in fold3.groupby("classifier", dropna=False):
        rows.append(
            {
                "row_type": "fold3_score_summary",
                "classifier": clf,
                "n": int(len(clf_df)),
                "score_mean": float(clf_df["y_score_final"].mean()),
                "score_median": float(clf_df["y_score_final"].median()),
                "score_q25": float(clf_df["y_score_final"].quantile(0.25)),
                "score_q75": float(clf_df["y_score_final"].quantile(0.75)),
                "ad_score_mean": float(clf_df.loc[clf_df["y_true"].eq(1), "y_score_final"].mean()),
                "cn_score_mean": float(clf_df.loc[clf_df["y_true"].eq(0), "y_score_final"].mean()),
            }
        )
...
def scanner_vs_auc(foldwise: pd.DataFrame, leakage: pd.DataFrame) -> pd.DataFrame:
    ...
    merged["is_fold3"] = merged["fold"].eq(3)
...
def load_vae_best_epoch_audit(run_dir: Path, run_config: Optional[Dict[str, Any]]) -> pd.DataFrame:
    args = (run_config or {}).get("args", {}) if isinstance(run_config, dict) else {}
    max_epochs = int(args.get("epochs_vae", 2560) or 2560)
    cycles = int(args.get("cyclical_beta_n_cycles", 32) or 32)
    patience = int(args.get("early_stopping_patience_vae", 240) or 240)
    rows: List[Dict[str, Any]] = []
    for fold_dir in sorted(run_dir.glob("fold_*")):
        if not fold_dir.is_dir():
            continue
        ...
        vals = np.asarray(hist.get(metric_name, []), dtype=float)
        if vals.size == 0:
            row.update({"best_epoch": np.nan, "epochs_completed": 0, "metric_used": metric_name})
            rows.append(row)
            continue
        best_idx = int(np.nanargmin(vals))
        epochs_completed = int(vals.size)
        final_epoch = epochs_completed
        reached_max_epochs = bool(epochs_completed >= max_epochs)
        early_stopped = bool(epochs_completed < max_epochs)
        best_epoch = best_idx + 1
        recent_window_start = max(0, len(vals) - patience)
        best_in_last_patience_window = bool(best_idx >= recent_window_start)
        near_max = bool(best_epoch >= int(0.9 * max_epochs))
        row.update(
            {
                ...
                "final_minus_best": float(vals[-1] - vals[best_idx]),
                "distance_best_to_2560": int(max_epochs - best_epoch),
                "reached_max_epochs": reached_max_epochs,
                "early_stopped": early_stopped,
                "best_in_last_patience_window": best_in_last_patience_window,
                "best_epoch_near_max_90pct": near_max,
                "rerun_3840_48cycles_recommendation": (
                    "consider_only_if_reconstruction_or_latent_qc_needs_it"
                    if best_in_last_patience_window or near_max
                    else "not_recommended_from_val_loss_plateau"
                ),
            }
        )
        rows.append(row)
    ...
    if not out.empty:
        any_pressure = bool(
            out.get("best_in_last_patience_window", pd.Series(dtype=bool)).fillna(False).any()
            or out.get("best_epoch_near_max_90pct", pd.Series(dtype=bool)).fillna(False).any()
        )
        summary = {
            "fold": "summary",
            "history_available": bool(out["history_available"].all()),
            "configured_max_epochs": max_epochs,
            "configured_cycles": cycles,
            "early_stopping_patience": patience,
            "best_epoch": float(pd.to_numeric(out["best_epoch"], errors="coerce").mean()),
            "epochs_completed": float(pd.to_numeric(out["epochs_completed"], errors="coerce").mean()),
            "distance_best_to_2560": float(pd.to_numeric(out["distance_best_to_2560"], errors="coerce").mean()),
            "reached_max_epochs": bool(out.get("reached_max_epochs", pd.Series(dtype=bool)).fillna(False).all()),
            "early_stopped": bool(out.get("early_stopped", pd.Series(dtype=bool)).fillna(False).any()),
            "best_in_last_patience_window": bool(out.get("best_in_last_patience_window", pd.Series(dtype=bool)).fillna(False).any()),
            "best_epoch_near_max_90pct": bool(out.get("best_epoch_near_max_90pct", pd.Series(dtype=bool)).fillna(False).any()),
            "rerun_3840_48cycles_recommendation": (
                "not_recommended_as_default; only rerun if fold-specific QC shows undertraining"
                if not any_pressure
                else "consider_targeted_rerun_3840_48cycles_before_final_claims"
            ),
        }
        out = pd.concat([out, pd.DataFrame([summary])], ignore_index=True, sort=False)
    return out
...
def metadata_counts(path: Path) -> Dict[str, Any]:
    ...
    man = meta["Manufacturer"].fillna("UNKNOWN").astype(str).str.upper()
    ...
    out["AD_Siemens"] = int((diag.eq("AD") & man.eq("SIEMENS")).sum())
    out["AD_Philips"] = int((diag.eq("AD") & man.eq("PHILIPS")).sum())
    return out
...
def top_level_findings(
    ...
    fold3_primary = primary[primary["fold"].eq(3)]
    fold3_auc = float(fold3_primary["auc_final"].iloc[0]) if len(fold3_primary) else np.nan
    fold3_is_worst = bool(worst.get("fold") == 3) if worst else False
    ...
    return {
        ...
        "fold3_auc_primary": fold3_auc,
        "fold3_is_worst_primary_auc": fold3_is_worst,
...
def write_readme(
    ...
    vae_epoch: pd.DataFrame,
    run_config: Optional[Dict[str, Any]],
    run_manifest: Optional[Dict[str, Any]],
) -> None:
    findings = top_level_findings(foldwise, subgroup, threshold, fold3, comparison)
    ...
    final_row = comparison[comparison["version"].eq("v5_1_batch20260514b_ch1_0_2")]
    interim_row = comparison[comparison["version"].eq("v5_1_batch20260514_interim_ch1_0_2")]
    final_logreg_auc = float(final_row["logreg_auc_final_mean"].iloc[0]) if len(final_row) and "logreg_auc_final_mean" in final_row else np.nan
    interim_logreg_auc = float(interim_row["logreg_auc_final_mean"].iloc[0]) if len(interim_row) and "logreg_auc_final_mean" in interim_row else np.nan
    final_svm_auc = float(final_row["svm_auc_final_mean"].iloc[0]) if len(final_row) and "svm_auc_final_mean" in final_row else np.nan
    interim_svm_auc = float(interim_row["svm_auc_final_mean"].iloc[0]) if len(interim_row) and "svm_auc_final_mean" in interim_row else np.nan
    vae_summary = vae_epoch[vae_epoch["fold"].astype(str).eq("summary")] if not vae_epoch.empty and "fold" in vae_epoch.columns else pd.DataFrame()
    vae_recommendation = (
        str(vae_summary["rerun_3840_48cycles_recommendation"].iloc[0])
        if len(vae_summary) and "rerun_3840_48cycles_recommendation" in vae_summary.columns
        else "unknown"
    )
    ...
    lines = [
        "# ADNI v5.1 batch20260514b final-candidate baseline audit",
        "",
        ...
        "- Dataset: v5.1_batch20260514b no-Python-bandpass",
        "- Channels: `[1,0,2]`",
        ...
        (
            f"- Mejora contra interim: logreg delta AUC={final_logreg_auc - interim_logreg_auc:.3f}; "
            f"svm delta AUC={final_svm_auc - interim_svm_auc:.3f}."
            if pd.notna(final_logreg_auc) and pd.notna(interim_logreg_auc)
            else "- Mejora contra interim: no calculable porque falta alguna métrica."
        ),
        (
        ...
        (
            f"- Fold 3: {'is' if findings['fold3_is_worst_primary_auc'] else 'is not'} "
            f"the worst {primary} fold by AUC. Fold 3 {primary} AUC="
            f"{findings['fold3_auc_primary']:.3f}; worst fold="
            f"{findings['worst_primary_fold']}."
        ),
        (
            "- Sensibilidad AD sigue siendo el principal problema si se usa threshold 0.5; "
            "el análisis de threshold muestra que el ranking tiene más señal que la regla de decisión fija."
        ),
        "",
        "## Threshold audit",
        "",
    ...
    lines.append(
        "- Leakage-safe threshold recommendation for the next experiment: select threshold/calibration inside each training fold only "
        "(inner-CV or train/validation split), freeze it, and apply once to the untouched outer fold."
    )
    ...
    lines += [
        ...
        (
            ...
            "now especially because CN-GE is large enough to expose manufacturer-specific false positives."
        ),
        "",
        "## Scanner leakage",
        "",
    ...
    lines += [
        ...
        "## VAE training",
        "",
        f"- 3840/48-cycle rerun recommendation: `{vae_recommendation}`.",
        "- Interpret this from `vae_best_epoch_audit.csv`: if best epochs are far from 2560 and not in the last patience window, more epochs are not the first fix.",
    ]
    ...
    lines += [
        ...
        (
            ...
            "sensitivity/specificity tradeoff. A manufacturer-aware split/sensitivity analysis is recommended "
            "before final claims; it should be an analysis design constraint, not a post-hoc threshold selection."
        ),
        "",
        "## Files written",
        "",
...
def main() -> int:
    ...
    fold3.to_csv(OUTPUT_DIR / "fold3_error_audit.csv", index=False)
    vae_epoch.to_csv(OUTPUT_DIR / "vae_best_epoch_audit.csv", index=False)
    ...
    comparison.to_csv(OUTPUT_DIR / "comparison_with_previous_runs.csv", index=False)
    ...
    manifest = {
        ...
        "vae_history_rows": int(len(vae_epoch)),
    }
