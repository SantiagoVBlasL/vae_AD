#!/usr/bin/env python3
"""Read-only integrity audit for recover035_scheduler90_sync FULL 5x5.

Checks both the metadata-rescue aspects (recover035) and the scheduler-sync aspects:
  - Zero parameter diffs vs locked v5.1b EXCEPT the three scheduler changes
    (epochs_vae 4480->4500, cyclical_beta_n_cycles 56->50, lr_scheduler_T0 80->90)
  - metadata_path ends with patched_metadata_candidate.csv
  ...
  - VAE internal val non-empty per fold (validates early stopping was active)
  - vae_pool_required_metadata_removed_fold_N.csv exists and contains 128_S_2002 per fold
  - candidate VAE checkpoints differ from locked (different training data + different schedule)
...
CANDIDATE_RUN = RESULTS / "recover035_scheduler90_sync_full5x5"
...
OUTPUT_DIR = RESULTS / "recover035_scheduler90_sync_full5x5_integrity_audit"
...
CANDIDATE_T0 = 90
CANDIDATE_EPOCHS_VAE = 4500
CANDIDATE_N_CYCLES = 50
LOCKED_CYCLE_LEN = 80
LOCKED_T0 = 80

# recover035 VAE pool sizes (patched metadata n=647, same seed as locked)
EXPECTED_VAE_POOL = {1: 567, 2: 567, 3: 568, 4: 568, 5: 568}
RECOVER_SUBJECT_TEST_FOLD = 1
...
def dry_run_config_check() -> None:
    ...
    param_diffs = {
        k: (src_params.get(k), tgt_params.get(k))
        for k in set(list(src_params) + list(tgt_params))
        if src_params.get(k) != tgt_params.get(k)
    }
    ...
    tgt_epochs = tgt_params.get("epochs_vae", 0)
    tgt_cycles = tgt_params.get("cyclical_beta_n_cycles", 1)
    tgt_T0 = tgt_params.get("lr_scheduler_T0", 0)
    tgt_cycle_len = tgt_epochs / tgt_cycles if tgt_cycles else float("nan")
    cycle_sync = tgt_cycle_len == tgt_T0 == CANDIDATE_CYCLE_LEN

    print("# recover035_scheduler90_sync Integrity Audit — Dry-Run Report")
    print()
    print("## Config-level verification")
    print()
    print(f"- Parameter diffs vs locked source    : {len(param_diffs)} (expected 3)")
    for k, (sv, tv) in sorted(param_diffs.items()):
        expected = expected_diffs.get(k)
        is_expected = expected is not None and (sv, tv) == expected
        print(f"    {k}: {sv!r} -> {tv!r}  [{'EXPECTED' if is_expected else 'UNEXPECTED'}]")
    unexpected_diffs = {k: v for k, v in param_diffs.items() if k not in expected_diffs}
    missing_diffs = {k: v for k, v in expected_diffs.items() if k not in param_diffs}
    if unexpected_diffs:
        print(f"  -> UNEXPECTED diffs: {list(unexpected_diffs.keys())}  [FAIL]")
    if missing_diffs:
        print(f"  -> MISSING expected diffs: {list(missing_diffs.keys())}  [FAIL]")
    if not unexpected_diffs and not missing_diffs:
        print("  -> exactly 3 expected scheduler diffs  [VERIFIED]")
    print(f"- metadata_path changed to patched    : {src_meta != tgt_meta}  [{'VERIFIED' if src_meta != tgt_meta else 'FAIL'}]")
    ...
    print()
    print("## Scheduler synchronization check")
    print()
    print(f"- epochs_vae / cyclical_beta_n_cycles  : {tgt_epochs} / {tgt_cycles} = {tgt_cycle_len:.1f}  [{'PASS' if tgt_cycle_len == CANDIDATE_CYCLE_LEN else 'FAIL'}]")
    print(f"- lr_scheduler_T0                      : {tgt_T0}  [{'PASS' if tgt_T0 == CANDIDATE_T0 else 'FAIL'}]")
    print(f"- beta/LR cycles synchronized (both {CANDIDATE_CYCLE_LEN}): {cycle_sync}  [{'PASS' if cycle_sync else 'FAIL'}]")
    print(f"- Restart epochs                       : {', '.join(str(i * CANDIDATE_CYCLE_LEN) for i in range(min(7, CANDIDATE_N_CYCLES + 1)))}...")
    print(f"- Locked reference                     : {LOCKED_CYCLE_LEN}-epoch cycles (T0={LOCKED_T0})")
    print()
    print("## What would be checked post-training")
    print()
    print("- VAE checkpoint SHA256: candidate must DIFFER from locked (different data + schedule)")
    print(f"- Latent mu: {RECOVER_SUBJECT} must appear in test latent cache for fold {RECOVER_SUBJECT_TEST_FOLD}")
    print(f"- Latent mu: {RECOVER_SUBJECT} must appear in trainDev latent cache for folds {[f for f in FOLDS if f != RECOVER_SUBJECT_TEST_FOLD]}")
    ...
    print()
    print("## VAE internal val health checks")
    print()
    print("- Per fold: vae_internal_val_idx_local_to_pool.npy must exist and have len > 0")
    print(f"  (len == 0 means early stopping was disabled — run is NOT valid)")
    print(f"- Per fold: vae_pool_required_metadata_removed_fold_N.csv must exist and contain {EXCLUDED_SUBJECT}")
    print(f"  (confirms --vae_required_metadata_cols filtered the tensor-only subject)")
    print()
    print("## Expected VAE pool sizes (patched metadata n=647 minus test CN/AD per fold)")
    for fold, n in EXPECTED_VAE_POOL.items():
        print(f"  Fold {fold}: VAE pool expected n={n}")
    print()
...
def check_subject_in_latents(candidate_readout: Path) -> pd.DataFrame:
...
def check_vae_val_health(candidate_run: Path) -> pd.DataFrame:
    """Per-fold check: VAE val non-empty, 128_S_2002 removal confirmed."""
    rows: List[Dict[str, Any]] = []
    for fold in FOLDS:
        ...
        if removed_exists:
            try:
                ...
                excluded_removed = bool(
                    (removed_df["SubjectID"].astype(str) == EXCLUDED_SUBJECT).any()
                )
            except Exception:
                excluded_removed = False

        expected_pool = EXPECTED_VAE_POOL.get(fold, -1)
        ...
        rows.append({
            ...
            "expected_vae_pool_n": expected_pool,
...
def prediction_score_comparison(candidate_readout: Path, locked_readout: Path) -> pd.DataFrame:
    ...
    for fold in FOLDS:
        ...
        recover_in_test = (
            RECOVER_SUBJECT in set(c["SubjectID"].astype(str).tolist())
            if "SubjectID" in c.columns
            else None
        )
        row: Dict[str, Any] = {
            "fold": fold,
            ...
            "threshold_equal": (
                bool(math.isclose(c_threshold, l_threshold, rel_tol=0.0, abs_tol=1e-15))
                if not (math.isnan(c_threshold) or math.isnan(l_threshold))
                else False
            ),
        ...
        for metric in ["auc", "pr_auc", "balanced_accuracy"]:
            ...
            row[f"delta_{metric}"] = (
                c_val - l_val
                if not (math.isnan(c_val) or math.isnan(l_val))
                else np.nan
            )
        rows.append(row)
    return pd.DataFrame(rows)
...
def stageb_report_text(
    ...
    test_035_folds: List[int] = []
    if col_035 in subject_check.columns:
        test_035 = subject_check[
            (subject_check["split"] == "test")
            & subject_check[col_035].fillna(False)
        ]
        test_035_folds = sorted(test_035["fold"].astype(int).tolist())
    excluded_present = (
        subject_check[subject_check.get(col_128, pd.Series(dtype=bool)).fillna(False)]
        if col_128 in subject_check.columns
        else pd.DataFrame()
    )

    recover_test_fold: Optional[int] = None
    ...
    lines = [
        "# Stage B Readout Integrity Report (recover035_scheduler90_sync)",
        "",
        "## Path checks",
        ...
        "",
        "## VAE checkpoint freshness (must differ from locked — different data + schedule)",
        ...
        f"- 035_S_6927 test-latent folds: `{test_035_folds}` (expected: [{RECOVER_SUBJECT_TEST_FOLD}])",
        f"- 035_S_6927 outer-test fold (from predictions): `{recover_test_fold}` (expected: {RECOVER_SUBJECT_TEST_FOLD})",
        "",
        "## Excluded subject 128_S_2002 in latent caches",
        f"- 128_S_2002 appears in any latent cache (should be False): **{not excluded_present.empty}**",
        "",
        "## VAE internal val health",
        "",
    ]
...
def final_decision_text(
    ...
    if ckpt_same_folds:
        invalid_conditions.append(
            f"VAE checkpoints SHA-identical to locked for folds {ckpt_same_folds} — "
            "retrain may not have run cleanly"
        )
    if not excluded_present.empty:
        invalid_conditions.append(
            f"{EXCLUDED_SUBJECT} appears in candidate latent caches — should be absent"
        )
    if RECOVER_SUBJECT_TEST_FOLD not in test_035_folds:
        invalid_conditions.append(
            f"{RECOVER_SUBJECT} not found in test latent cache for fold {RECOVER_SUBJECT_TEST_FOLD} — "
            "expected as AD test subject in fold 1"
        )
    if not vae_val_health.empty:
        bad_val = vae_val_health[~vae_val_health["vae_internal_val_nonempty"].fillna(False)]
        if not bad_val.empty:
    ...
    lines = [
        "# Final Integrity Decision (recover035_scheduler90_sync)",
        "",
        "## Decision",
        "",
    ]
    if invalid_conditions:
        lines.append("**Integrity issues detected. Do not treat recover035_scheduler90_sync as a valid retrain.**")
        lines.append("")
        for c in invalid_conditions:
            lines.append(f"- {c}")
    else:
        lines.append(
            "No integrity issues detected. Candidate checkpoints differ from locked (expected: "
            "different data and different schedule), 128_S_2002 is absent from all latent caches, "
            f"035_S_6927 appears in fold {RECOVER_SUBJECT_TEST_FOLD} test latent cache, "
            "and VAE internal val was non-empty in all folds."
        )
    ...
    lines.extend([
        ...
        f"- {RECOVER_SUBJECT} in fold {RECOVER_SUBJECT_TEST_FOLD} test latent cache: `{RECOVER_SUBJECT_TEST_FOLD in test_035_folds}` (expected True)",
...
def main() -> int:
    ...
    subject_check = check_subject_in_latents(candidate_readout)
    ...
    write_table(outdir, "subject_latent_check", subject_check, max_rows=20)
    ...
    command_log = {
        ...
        "vae_val_health_all_folds_ok": (
            bool(vae_val_health["all_checks_ok"].all()) if not vae_val_health.empty else None
        ),
