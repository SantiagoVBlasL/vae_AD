#!/usr/bin/env python3
"""Read-only integrity audit for scheduler_cycle90_sync FULL 5x5.

Checks that the candidate fold artifacts are fresh and independent from the
locked horizon4480/cycles56 reference, verifies Stage B readout path integrity,
and reports schedule-phase metrics from VAE training histories.
...
CANDIDATE_RUN = RESULTS / "scheduler_cycle90_sync_full5x5"
...
OUTPUT_DIR = RESULTS / "scheduler_cycle90_sync_full5x5_integrity_audit"
...
def dry_run_report(candidate_run: Path, locked_run: Path) -> str:
    lines = [
        "# scheduler_cycle90_sync Integrity Audit — Dry-Run Report",
        "",
        ...
        "- Latent mu cache SHA256 identity and numerical equality vs locked (per fold × split)",
...
def stageb_report(
    ...
    lines = [
        "# Stage B Readout Integrity Report — scheduler_cycle90_sync",
        "",
        "## Path checks",
        "",
...
def final_decision(pred_cmp: pd.DataFrame, lat_cmp: pd.DataFrame, inventory: pd.DataFrame) -> str:
    ...
    lines = [
        "# Final Integrity Decision — scheduler_cycle90_sync",
        "",
        "## Decision",
        "",
    ]
    if invalid:
        lines.append("**Stale fold evidence detected: do not treat scheduler_cycle90_sync as a valid FULL 5x5 result yet.**")
    else:
        lines.append("No stale-fold evidence detected; the candidate appears internally independent of the locked reference.")
    lines.extend([
        "",
        "## Evidence",
        "",
        ...
        "Only after all five fold checkpoints differ from the locked run should promotion be evaluated.",
    ])
    return "\n".join(lines) + "\n"
...
def readme_text(
    ...
    return "\n".join([
        "# scheduler_cycle90_sync Integrity Audit",
        "",
        "Read-only audit of the synchronized 90-epoch cycle candidate versus locked v5.1b horizon4480/cycles56.",
        "",
        "## Summary",
        "",
        ...
        "",
        f"- Candidate: 90-epoch synchronized beta/LR cycles (4500/50=90, T0=90)",
        f"- Locked reference: 80-epoch cycles (4480/56=80, T0=80)",
        f"- Phase at epoch 0: both cycles start phase-aligned (0.0)",
        "",
        "## Interpretation",
        "",
...
def main() -> int:
    ...
    (outdir / "stageb_readout_integrity_report.md").write_text(
        stageb_report(candidate_run, candidate_readout, locked_run, locked_readout, pred_cmp, latent_cmp, inventory, phase_df),
        encoding="utf-8",
    )
    (outdir / "final_integrity_decision.md").write_text(final_decision(pred_cmp, latent_cmp, inventory), encoding="utf-8")
    (outdir / "README.md").write_text(readme_text(inventory, latent_cmp, pred_cmp, phase_df, stageb_ok), encoding="utf-8")
    command_log = {
    ...
    print(readme_text(inventory, latent_cmp, pred_cmp, phase_df, stageb_ok))
    return 0
