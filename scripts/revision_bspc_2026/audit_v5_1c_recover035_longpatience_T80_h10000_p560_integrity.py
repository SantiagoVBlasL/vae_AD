#!/usr/bin/env python3
"""Read-only integrity audit for recover035_longpatience_T80_h10000_p560 FULL 5x5.

Verifies:
  - Exactly 5 parameter diffs vs recover035_full5x5 (the base model):
      epochs_vae 4480->10000, cyclical_beta_n_cycles 56->125,
      early_stopping_patience_vae 320->560, n_iter_logreg 300->500, n_iter_svm 300->500
  - metadata_path ends with patched_metadata_candidate.csv (same as recover035_full5x5)
  - global_tensor_path unchanged
  - cycle length = 10000/125 = 80 epochs (unchanged from recover035 and locked)
  - lr_scheduler_T0 = 80 (unchanged)
  ...
  - VAE checkpoints differ from locked and recover035 (different training horizon/patience)
...
CANDIDATE_RUN = RESULTS / "recover035_longpatience_T80_h10000_p560_full5x5"
...
RECOVER035_RUN = RESULTS / "recover035_full5x5"
OUTPUT_DIR = RESULTS / "recover035_longpatience_T80_h10000_p560_integrity_audit"
...
CANDIDATE_CYCLE_LEN = 80   # 10000/125 = 80
CANDIDATE_T0 = 80
CANDIDATE_PATIENCE = 560   # 7 × 80
BASE_EPOCHS_VAE = 4480     # recover035 reference
BASE_N_CYCLES = 56
BASE_PATIENCE = 320
...
def check_longpatience_diff(source: Dict[str, Any], target: Dict[str, Any]) -> List[str]:
    errors: List[str] = []
    ...
    expected_diffs = {
        "epochs_vae": (BASE_EPOCHS_VAE, CANDIDATE_EPOCHS_VAE),
        "cyclical_beta_n_cycles": (BASE_N_CYCLES, CANDIDATE_N_CYCLES),
        "early_stopping_patience_vae": (BASE_PATIENCE, CANDIDATE_PATIENCE),
    ...
    if diffs != expected_diffs:
        ...
        if extra:
            errors.append(f"Unexpected diffs vs recover035: {extra}")
        if missing:
            errors.append(f"Missing expected diffs vs recover035: {missing}")
        if wrong:
    ...
    # metadata_path must be the same (patched_metadata_candidate.csv)
    src_meta = source["paths"]["metadata_path"]
    tgt_meta = target["paths"]["metadata_path"]
    if src_meta != tgt_meta:
        errors.append(f"metadata_path changed vs recover035: {src_meta!r} -> {tgt_meta!r} (should be identical)")
...
def check_latent_cache(run_dir: Path) -> List[Dict[str, Any]]:
    ...
    for fold in FOLDS:
        for split in ["trainDev", "test"]:
            path = run_dir / f"fold_{fold}" / f"fold_{fold}_{split}_latent_mu.csv"
            exists = path.exists()
            row: Dict[str, Any] = {
                "fold": fold, "split": split, "path": str(path), "exists": exists,
                "n_rows": None, "has_excluded": None, "has_recover_subj": None, "errors": [],
            }
            if not exists:
            ...
            # fold 1 test should contain RECOVER_SUBJECT; trainDev should not contain in fold 1
...
def check_vae_internal_val(run_dir: Path) -> List[Dict[str, Any]]:
    ...
    for fold in FOLDS:
        ...
        history_candidates = sorted(fold_dir.glob("vae_training_history_fold_*.csv")) if fold_dir.exists() else []
        row: Dict[str, Any] = {"fold": fold, "history_file": None, "n_val_steps": None, "errors": []}
        if not history_candidates:
            row["errors"].append("no vae_training_history_fold_*.csv found")
            rows.append(row)
            continue
        history_path = history_candidates[0]
        row["history_file"] = str(history_path)
        hist = pd.read_csv(history_path)
        val_col = next((c for c in hist.columns if "val" in c.lower() and "loss" in c.lower()), None)
        if val_col is None:
            row["errors"].append("no val_loss column in training history")
        else:
            non_nan = hist[val_col].dropna()
...
def check_vae_pool_sizes(run_dir: Path) -> List[Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    for fold in FOLDS:
        cache_path = run_dir / f"fold_{fold}" / f"fold_{fold}_trainDev_latent_mu.csv"
        row: Dict[str, Any] = {"fold": fold, "expected_vae_pool": EXPECTED_VAE_POOL.get(fold), "actual_vae_pool": None, "errors": []}
        if not cache_path.exists():
            row["errors"].append("trainDev latent cache missing; cannot compute vae_pool size")
            rows.append(row)
            continue
        df = pd.read_csv(cache_path)
        row["actual_vae_pool"] = int(len(df))
        expected = EXPECTED_VAE_POOL.get(fold)
        if expected is not None and row["actual_vae_pool"] != expected:
            row["errors"].append(
                f"vae_pool n={row['actual_vae_pool']}, expected {expected}"
            )
        rows.append(row)
    return rows
    ...
    for fold in FOLDS:
        ...
        if cand_ckpt.exists() and ref_ckpt.exists():
            ...
            if cand_hash == ref_hash:
                row["errors"].append(
                    f"Checkpoint SHA256 identical to {label} — longpatience should produce different weights"
                )
        rows.append(row)
    return rows
...
def write_summary(
    ...
    all_latent_errors = [e for r in latent_rows for e in r["errors"]]
    all_val_errors = [e for r in val_rows for e in r["errors"]]
    all_pool_removed_errors = [e for r in pool_removed_rows for e in r["errors"]]
    all_pool_size_errors = [e for r in pool_size_rows for e in r["errors"]]
    all_ckpt_errors = [e for r in ckpt_rows for e in r["errors"]]
    readout_errors = readout_result.get("errors", [])

    all_errors = (
        [f"[config] {e}" for e in config_errors]
        + [f"[latent_cache] {e}" for e in all_latent_errors]
        + [f"[val] {e}" for e in all_val_errors]
        + [f"[pool_removed] {e}" for e in all_pool_removed_errors]
        + [f"[pool_size] {e}" for e in all_pool_size_errors]
        + [f"[checkpoint] {e}" for e in all_ckpt_errors]
        + [f"[readout] {e}" for e in readout_errors]
    )

    lines = [
        "# recover035_longpatience_T80_h10000_p560 Integrity Audit",
        "",
    ...
    if not dry_run:
        ...
        lines += [
            ...
            "",
            f"| model | threshold | n | AUC | PR-AUC | BA | Sens | Spec | F1 |",
            f"|---|---|---|---|---|---|---|---|---|",
    ...
    lines += [
        "## Config diff check (vs recover035_full5x5 base model)",
        "",
        f"Config errors: {len(config_errors)}",
        "",
        "## Schedule invariants",
        "",
        f"- Cycle length: {CANDIDATE_EPOCHS_VAE}/{CANDIDATE_N_CYCLES} = {CANDIDATE_CYCLE_LEN} epochs (same as recover035 and locked)",
        f"- lr_scheduler_T0: {CANDIDATE_T0} (unchanged from recover035 — phase-aligned)",
...
def main() -> int:
    ...
    config_errors = check_longpatience_diff(source_cfg, candidate_cfg)
    ...
    if config_errors:
        ...
        print("Config diff check: PASS — exactly 5 parameter diffs vs recover035_full5x5")
        print(f"  epochs_vae: {BASE_EPOCHS_VAE} -> {CANDIDATE_EPOCHS_VAE}")
        print(f"  cyclical_beta_n_cycles: {BASE_N_CYCLES} -> {CANDIDATE_N_CYCLES}")
        print(f"  early_stopping_patience_vae: {BASE_PATIENCE} -> {CANDIDATE_PATIENCE}")
        print(f"  n_iter_logreg: 300 -> 500")
        print(f"  n_iter_svm: 300 -> 500")
    ...
    if not args.dry_run:
        ...
        if resolve(RECOVER035_RUN).exists():
            ckpt_rows += compare_checkpoints_vs_reference(candidate_run, resolve(RECOVER035_RUN), "recover035")
        ...
        if not latent_errors:
            print("Latent cache checks: PASS")
        else:
        ...
        if not ckpt_errors:
            print("Checkpoint diff checks: PASS (candidate differs from locked and recover035)")
        else:
    ...
    write_summary(outdir if not args.dry_run else Path("."), config_errors, latent_rows, val_rows,
                  pool_removed_rows, pool_size_rows, ckpt_rows, readout_result, dry_run=args.dry_run)

    if args.dry_run:
        status = "PASS" if not config_errors else "FAIL"
        print(f"\nDry-run complete. Config-level checks: {status}")
    else:
        all_errors = (config_errors + [e for r in latent_rows for e in r["errors"]]
