#!/usr/bin/env python3
"""Build an incremental no-Python-bandpass tensor for Martin batch 20260514.

This script uses only rows marked import_ready_new=yes by the batch audit. It
does not assemble with v5.1_batch20260513 and does not train.
"""
...
DEFAULT_LOCAL_SYMLINK = (
    PROJECT_ROOT / "data" / "revision_bspc_2026" / "adni_v5_1_batch20260514_incremental_no_pybandpass"
)

DATASET_NAME = "adni_v5_1_batch20260514_incremental_no_pybandpass"
EXPECTED_IMPORT_READY_N = 52
FINAL_TENSOR_NAME = "INCREMENTAL_TENSOR_MARTIN_BANDPASS_BATCH20260514.npz"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build incremental tensor for Martin bandpass batch 20260514.",
...
def import_ready_rows(decision_csv: Path) -> pd.DataFrame:
    ...
    if "import_ready_new" not in df.columns:
        raise RuntimeError(f"Decision CSV is missing import_ready_new: {decision_csv}")
    ready = df[df["import_ready_new"].astype(str).str.lower().eq("yes")].copy()
    ready = ready.sort_values("SubjectID").reset_index(drop=True)
    if len(ready) != EXPECTED_IMPORT_READY_N:
        raise RuntimeError(f"Expected {EXPECTED_IMPORT_READY_N} import_ready_new=yes rows, found {len(ready)}")
...
def write_outputs(
    ...
    np.savez_compressed(
        ...
        source_batch=np.asarray(BATCH_LABEL, dtype="U32"),
    ...
    readme_lines = [
        "# ADNI v5.1 Martin Bandpass Batch 20260514 Incremental Tensor",
        "",
        ...
        "- No training was run.",
        "- This output is not assembled with v5.1_batch20260513.",
        "",
        "## Tensor",
        "",
        ...
        "## Next Step",
        "",
        "Review this incremental output before any assembly with v5.1_batch20260513. Do not train until the final v5.1 dataset is explicitly confirmed.",
    ]
...
def dry_run_report(ready: pd.DataFrame, metadata: pd.DataFrame, args: argparse.Namespace) -> None:
    ...
    print(f"import_ready_new_rows={len(ready)}")
    print(f"expected_import_ready_new_rows={EXPECTED_IMPORT_READY_N}")
