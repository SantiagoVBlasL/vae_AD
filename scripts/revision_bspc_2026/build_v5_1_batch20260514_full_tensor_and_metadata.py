#!/usr/bin/env python3
"""Assemble ADNI v5.1 batch-20260514 final candidate dataset.

This is an assembly-only step:
- reuse v5.1_batch20260513;
- append only QC-pass, non-duplicate subjects from Martin bandpass batch 20260514;
...
DATASET_NAME = "adni_expanded_v5_1_batch20260514_no_pybandpass"
INCLUDED_DATASET_VERSION = "v5.1_batch20260514"
SOURCE_BATCH = "20260514_bandpass_batch2"
EXPECTED_CHANNELS = 7
EXPECTED_ROIS = 131
...
DEFAULT_BASE_METADATA = DEFAULT_BASE_ROOT / "subject_metadata_v5_1_batch20260513_no_pybandpass.csv"
DEFAULT_BASE_TRAINING_METADATA = DEFAULT_BASE_ROOT / "training_ready_metadata_v5_1_batch20260513_no_pybandpass.csv"
...
DEFAULT_INCREMENTAL_TENSOR = Path(
    "/media/diego/Datos/vae_AD_data/revision_bspc_2026/"
    "adni_v5_1_batch20260514_incremental_no_pybandpass/"
    "INCREMENTAL_TENSOR_MARTIN_BANDPASS_BATCH20260514.npz"
)
...
DEFAULT_LEDGER = (
    ...
    / "revision_bspc_2026"
    / "martin_bandpass_batch_20260514"
    / "ADNI_v5_1_DATA_LEDGER_20260514_after_batch2_candidate.csv"
)
DEFAULT_LEDGER_IMPORTED = (
    ...
    / "revision_bspc_2026"
    / "martin_bandpass_batch_20260514"
    / "ADNI_v5_1_DATA_LEDGER_20260514_after_batch2_imported_candidate.csv"
)
...
DEFAULT_QC_DIR = (
    ...
    / "results"
    / "revision_bspc_2026"
    / "adni_v5_1_batch20260514_full_build_qc"
)

FINAL_TENSOR_NAME = "GLOBAL_TENSOR_ADNI_expanded_v5_1_batch20260514_no_pybandpass.npz"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build ADNI v5.1 batch20260514 final candidate tensor and metadata.",
...
def build_incremental_include_table(
    ...
    for idx, sid in enumerate(incremental_ids):
        ...
        if duplicate:
            reason = append_reason(reason, "duplicate_with_v5_1_batch20260513_excluded")
        ...
        rows.append(
            {
                ...
                "in_base_v5_1_batch20260513": duplicate,
...
def build_duplicate_report(include_table: pd.DataFrame) -> pd.DataFrame:
    ...
    duplicates = include_table[include_table["in_base_v5_1_batch20260513"]].copy()
    if duplicates.empty:
        ...
        rows.append(
            {
                ...
                "tensor_source": "martin_bandpass_batch20260514_incremental",
                ...
                "source_label": "martin_bandpass_batch20260514",
...
def write_tensor_npz(
    ...
    np.savez_compressed(
        ...
        assembled_with_v5_1_batch20260513=np.asarray(True, dtype=np.bool_),
        ...
        rows.append(
            {
                ...
                "base_v5_1_batch20260513_tensor_index": base_pos.get(sid, ""),
                "incremental_batch20260514_tensor_index": inc_pos.get(sid, ""),
        ...
        rows.append(
            {
                ...
                "final_tensor_index": "",
                "base_v5_1_batch20260513_tensor_index": base_pos.get(sid, ""),
                "incremental_batch20260514_tensor_index": row.get("incremental_tensor_index", ""),
...
def dataset_evolution_summary(
    ...
    rows = [
        ...
        evolution_row("v5_1_batch20260513", base_meta, base_train),
        evolution_row("v5_1_batch20260514", final_meta, final_train),
    ]
...
def added_subjects_by_step_summary(base_meta: pd.DataFrame, added_20260514: pd.DataFrame) -> pd.DataFrame:
...
def write_readme(
    ...
    final_ad = int(evolution.loc[evolution["version"].eq("v5_1_batch20260514"), "AD"].iloc[0])
    ...
    lines = [
        "# ADNI v5.1 Batch 20260514 No-Python-Bandpass Full Build QC",
        "",
        f"Generated: `{datetime.now().isoformat(timespec='seconds')}`",
        "",
        "Assembly-only candidate build. v5.1_batch20260513 was reused and Martin batch 20260514 was appended only for QC-pass, non-duplicate subjects.",
        "",
        "## Explicit Answers",
        "",
        f"- Paper original subjects: `{PAPER_ORIGINAL['N_tensor']}`.",
        f"- v5.1_batch20260513 subjects: `{base_n}`.",
        f"- batch20260513 added: `{added_20260513}`.",
        f"- batch20260514 incremental subjects: `{incremental_n}`.",
        f"- batch20260514 actually added: `{included_incremental_n}`.",
        ...
        f"  - `{output_root / 'subject_metadata_v5_1_batch20260514_no_pybandpass.csv'}`",
        f"  - `{output_root / 'training_ready_metadata_v5_1_batch20260514_no_pybandpass.csv'}`",
        "- QC:",
        "  - `subject_alignment.csv`",
        "  - `duplicate_report.csv`",
        "  - `diagnosis_counts.csv`",
        "  - `diagnosis_x_manufacturer.csv`",
        "  - `added_subjects_batch20260514.csv`",
...
def main() -> None:
    ...
    base = load_tensor_npz(args.base_tensor, "v5_1_batch20260513")
    incremental = load_tensor_npz(args.incremental_tensor, "incremental_batch20260514")
    ...
    if base_metadata["SubjectID"].astype(str).tolist() != base_ids:
        raise RuntimeError("Base metadata SubjectID order does not match v5.1_batch20260513 tensor subject_ids")
    ...
    metadata_path = args.output_root / "subject_metadata_v5_1_batch20260514_no_pybandpass.csv"
    training_path = args.output_root / "training_ready_metadata_v5_1_batch20260514_no_pybandpass.csv"
    ...
    added_subjects.to_csv(args.qc_dir / "added_subjects_batch20260514.csv", index=False)
    ...
    command_log = {
        ...
        "incremental_duplicates_excluded": int(include_table["in_base_v5_1_batch20260513"].sum()),
    ...
    write_readme(
        ...
        duplicate_n=int(include_table["in_base_v5_1_batch20260513"].sum()),
    ...
    print(f"incremental_duplicates_excluded={int(include_table['in_base_v5_1_batch20260513'].sum())}")
