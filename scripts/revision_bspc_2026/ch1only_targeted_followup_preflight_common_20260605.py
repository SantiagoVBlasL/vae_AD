#!/usr/bin/env python3
"""Shared preflight/launcher utilities for ch1-only targeted FULL follow-ups.

Each candidate is compared against the completed ch1-only reference:

  recover035_ch1only_latent384_beta3p75_T80_h10000_p560_full5x5

The only allowed scientific diffs are supplied by the thin candidate launcher.
...
BASE_RUN_ID = "recover035_ch1only_latent384_beta3p75_T80_h10000_p560_full5x5"
BASE_CONFIG = (
    PROJECT_ROOT
    / "configs/runs/adni_v5_1c_recover035_ch1only_latent384_beta3p75_T80_h10000_p560_full5x5.json"
)
AGGREGATE_PREFLIGHT_ROOT = (
    PROJECT_ROOT / "results/revision_bspc_2026/ch1only_targeted_full_followup_preflight_20260605"
)
...
class CandidateSpec:
    ...
    config_path: Path
    launcher_path: Path
    allowed_param_diffs: Mapping[str, Tuple[Any, Any]]

    @property
    def preflight_dir(self) -> Path:
        return AGGREGATE_PREFLIGHT_ROOT / self.run_id
...
def write_table(df: pd.DataFrame, out_dir: Path, stem: str, max_rows: int = 250) -> None:
...
def parse_args(spec: CandidateSpec) -> argparse.Namespace:
    p = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    p.add_argument("--config", type=Path, default=spec.config_path)
    p.add_argument("--reference-config", type=Path, default=BASE_CONFIG)
    p.add_argument("--preflight-dir", type=Path, default=spec.preflight_dir)
    ...
    p.add_argument("--force-clean", action="store_true", help="Reserved for real launches after manual cleanup.")
...
def validate_strict_diff(
    ...
    if set(ref_params) != set(tgt_params):
        raise RuntimeError(
            "Parameter key set differs from ch1-only reference: "
    ...
    expected = dict(spec.allowed_param_diffs)
    if param_diffs != expected:
        raise RuntimeError(f"Expected scientific diffs {expected}; got {param_diffs}")

    if reference.get("selected_channel_names") != EXPECTED_SELECTED_NAMES:
        raise RuntimeError("Reference selected_channel_names did not match expected ch1-only channel.")
    if target.get("selected_channel_names") != EXPECTED_SELECTED_NAMES:
        raise RuntimeError(f"Target selected_channel_names must remain {EXPECTED_SELECTED_NAMES}")
    if target["parameters"].get("channels_to_use") != EXPECTED_CHANNELS:
        raise RuntimeError(f"channels_to_use must remain {EXPECTED_CHANNELS}")
    ...
    if reference["split_strategy"] != target["split_strategy"]:
        raise RuntimeError("split_strategy changed unexpectedly")
    if reference["channel_names_master_in_tensor_order"] != target["channel_names_master_in_tensor_order"]:
        raise RuntimeError("Master tensor channel order changed unexpectedly")
...
def load_metadata(config: Dict[str, Any]) -> pd.DataFrame:
    ...
    required = ["ResearchGroup_Mapped", "Manufacturer", "Age", "Sex", "tensor_idx"]
    if meta[required].isna().any().any():
        bad = meta[meta[required].isna().any(axis=1)]
        raise RuntimeError(
            "Metadata has missing required values:\n"
            + bad[["SubjectID", "ResearchGroup_Mapped", "Manufacturer", "Age", "Sex", "tensor_idx"]]
            .head(20)
...
def validate_subject_pool(meta: pd.DataFrame) -> pd.DataFrame:
    ...
    for dx, expected in EXPECTED_VAE_COUNTS.items():
        ...
        rows.append({"check": f"vae_{dx}", "observed": observed, "expected": expected, "pass": observed == expected})
    for dx, expected in EXPECTED_CLF_COUNTS.items():
        observed = int(clf_counts.get(dx, 0))
        rows.append({"check": f"classifier_{dx}", "observed": observed, "expected": expected, "pass": observed == expected})
    rows.extend(
        [
            {
...
def output_symlink_status(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    setup_commands = [
        ...
        f"ln -s {shlex.quote(str(target))} {shlex.quote(str(local))}",
    ]
    return {
        ...
        "real_training_setup_commands_if_missing": " && ".join(setup_commands),
    }
...
def build_oof_command(config: Dict[str, Any], python_exe: str, run_id: str) -> List[str]:
    out = resolve(config["paths"]["output_dir"])
    return [
        python_exe,
...
def validate_commands(
    ...
    checks = [
        ...
        ("stage_a_latent_dim", values_after_flag(stage_a, "--latent_dim"), [str(params["latent_dim"])]),
        ("stage_a_beta", values_after_flag(stage_a, "--beta_vae"), [str(params["beta_vae"])]),
        ("stage_a_recon_loss", values_after_flag(stage_a, "--recon_loss_mode"), [params["recon_loss_mode"]]),
...
def write_candidate_outputs(
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    write_table(diff, out_dir, "config_diff_vs_ch1only_reference")
    ...
    write_table(
        pd.DataFrame([{**symlink, "stale_marker_count": len(stale), "stale_markers": " | ".join(str(p) for p in stale)}]),
        out_dir,
    ...
    write_json(
        ...
        {
            "base_reference": BASE_RUN_ID,
            "allowed_scientific_diffs": strict_diff,
            "unchanged_channels_to_use": target["parameters"]["channels_to_use"],
            "unchanged_selected_channel_names": target["selected_channel_names"],
        },
    )
    report = [
        f"# {spec.run_id} Preflight",
        "",
        f"Base reference: `{BASE_RUN_ID}`",
        "",
        "## Controlled Scientific Diff",
        "",
    ]
    for key, (old, new) in strict_diff.items():
    ...
    report.extend(
        [
            ...
            "## Preserved Settings",
            "",
            "- `channels_to_use=[1]` (`Pearson_Full_FisherZ_Signed`)",
            ...
            f"- `recon_loss_mode={target['parameters']['recon_loss_mode']}`",
            "- Stage B: `logreg_l2`, z+Age/Sex; OOF score calibration command prepared.",
            "",
            "## Validation",
            "",
            "- JSON syntax/config load: OK.",
            f"- Tensor selected channel: `{tensor['selected_channel_names']}`.",
            "- Subject pool: VAE CN=300/MCI=250/AD=97; classifier CN=300/AD=97.",
            "- 035_S_6927 included; 128_S_2002 absent from metadata pools.",
            "- All 5 folds feasible: OK.",
    ...
    if symlink["preflight_clean_missing_output_ok"] and not symlink["target_match"]:
        report.extend(
            [
                "",
                "## Real-Launch Setup",
                "",
                "Before real training, create the big-disk target and local symlink:",
                "",
                f"```bash\n{symlink['real_training_setup_commands_if_missing']}\n```",
            ]
        )
    ...
    status = {
        ...
        "launcher": rel(resolve(spec.launcher_path)),
        "reference_config": rel(resolve(reference_path)),
        "preflight_dir": rel(out_dir),
        "dry_run": dry_run,
        "training_launched": False,
        "allowed_scientific_diffs": strict_diff,
        "selected_channels_to_use": target["parameters"]["channels_to_use"],
        "selected_channel_names": target["selected_channel_names"],
        "latent_dim": target["parameters"]["latent_dim"],
        "beta_vae": target["parameters"]["beta_vae"],
        "vae_pool_n": 647,
        "classifier_pool_n": 397,
        "vae_counts_match": True,
        "classifier_counts_match": True,
        "folds_feasible": True,
        "tensor_ok": True,
        "stale_marker_count": len(stale),
        "output_target_match": symlink["target_match"],
        "preflight_clean_missing_output_ok": symlink["preflight_clean_missing_output_ok"],
        "real_training_setup_commands_if_missing": symlink["real_training_setup_commands_if_missing"],
    }
    write_json(out_dir / "preflight_status.json", status)
    write_json(
        out_dir / "command_log.json",
        {
            **status,
            "planned_commands": commands.to_dict(orient="records"),
            "guardrails": [
...
def write_aggregate_outputs() -> None:
    AGGREGATE_PREFLIGHT_ROOT.mkdir(parents=True, exist_ok=True)
    statuses: List[Dict[str, Any]] = []
    for path in sorted(AGGREGATE_PREFLIGHT_ROOT.glob("*/preflight_status.json")):
        statuses.append(load_json(path))
    if not statuses:
        return
    rows = []
    for st in statuses:
        rows.append(
            {
                ...
                "beta_vae": st["beta_vae"],
                "selected_channels_to_use": st["selected_channels_to_use"],
                "selected_channel_names": st["selected_channel_names"],
                "vae_pool_n": st["vae_pool_n"],
                "classifier_pool_n": st["classifier_pool_n"],
                "folds_feasible": st["folds_feasible"],
                "tensor_ok": st["tensor_ok"],
                "stale_marker_count": st["stale_marker_count"],
                "output_target_match": st["output_target_match"],
                "preflight_clean_missing_output_ok": st["preflight_clean_missing_output_ok"],
                "dry_run": st["dry_run"],
                "training_launched": st["training_launched"],
                "allowed_scientific_diffs": json.dumps(st["allowed_scientific_diffs"], sort_keys=True),
                "real_training_setup_commands_if_missing": st["real_training_setup_commands_if_missing"],
            }
        )
    df = pd.DataFrame(rows)
    write_table(df, AGGREGATE_PREFLIGHT_ROOT, "run_manifest")
    write_table(
        df[
            [
                "run_id",
                ...
                "stale_marker_count",
                "output_target_match",
                "preflight_clean_missing_output_ok",
                "training_launched",
            ]
        ],
        AGGREGATE_PREFLIGHT_ROOT,
    ...
    readme = [
        "# ch1-only Targeted FULL Follow-up Preflight",
        "",
        f"Base reference: `{BASE_RUN_ID}`",
        "",
        "Prepared candidates are controlled ch1-only FULL 5x5 sensitivity branches. Each candidate preserves the single Pearson Full channel, tensor, metadata, folds, classifier protocol, dropout, scheduler, and reconstruction-loss convention from the base ch1-only reference.",
        "",
        "No real training was launched by these preflights.",
        "",
        "## Candidates",
        "",
    ]
    for row in rows:
        readme.append(f"- `{row['run_id']}`: latent_dim={row['latent_dim']}, beta_vae={row['beta_vae']}")
    readme.extend(
        [
            "",
            "## Real-Launch Guardrail",
            "",
            "Candidate output paths were checked for stale markers. If an output symlink is missing, use the setup command recorded in each candidate's dry-run report before launching with `--confirm-training`.",
        ]
    )
    (AGGREGATE_PREFLIGHT_ROOT / "README.md").write_text("\n".join(readme) + "\n", encoding="utf-8")
    write_json(
        AGGREGATE_PREFLIGHT_ROOT / "command_log.json",
        {
            "updated_utc": now_utc(),
            "training_launched": False,
            "candidate_count": len(statuses),
            "candidates": [st["run_id"] for st in statuses],
            "guardrails": [
                "no real training during preflight",
                "no tensor modification",
                "no metadata modification",
                "no ledger modification",
                "no existing model output modification",
            ],
        },
    )
...
def main(spec: CandidateSpec) -> None:
    ...
    strict_diff = validate_strict_diff(spec, reference, target)
    ...
    print(f"Output target match: {status['output_target_match']}; clean missing output OK: {status['preflight_clean_missing_output_ok']}")
    print(f"Stale output markers: {status['stale_marker_count']}")
    ...
    if stale and not args.force_clean:
        raise RuntimeError("Refusing real training with stale output markers. Use --force-clean only after manual cleanup.")
