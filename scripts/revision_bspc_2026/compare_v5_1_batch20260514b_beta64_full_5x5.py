#!/usr/bin/env python3
"""Compare beta=6.4 FULL 5x5 against locked current FULL [1,0,2].
...
DEFAULT_CANDIDATE_RUN = RESULTS / "adni_v5_1_batch20260514b_ch1_0_2_beta64_full_5x5"
DEFAULT_CANDIDATE_READOUT = DEFAULT_CANDIDATE_RUN / "classifier_only_readout"
DEFAULT_OUTPUT = RESULTS / "adni_v5_1_batch20260514b_ch1_0_2_beta64_full_5x5_comparison"
...
def write_recommendation(outdir: Path, main: pd.DataFrame, focus: pd.DataFrame, scanner: pd.DataFrame) -> None:
    cand = main[main["run_id"].eq("beta64_full_5x5")].iloc[0]
    ref = main[main["run_id"].eq("current_locked_ch1_0_2")].iloc[0]
    focus_cand = focus[focus["run_id"].eq("beta64_full_5x5")].iloc[0]
    ...
    lines = [
        "# Beta=6.4 FULL 5x5 Recommendation",
        "",
        ...
        "Context: beta=6.4 tests 2.56× stronger KL compression than the locked model (beta=2.5).",
        "If not promoted, the locked current FULL beta=2.5 model remains the paper model.",
    ]
    (outdir / "beta64_full_5x5_recommendation.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_final_postmortem(outdir: Path, main: pd.DataFrame, scanner: pd.DataFrame) -> None:
    ...
    cand = main[main["run_id"].eq("beta64_full_5x5")].iloc[0]
    lines = [
        "# Final beta64 post-mortem",
        "",
        "This read-only audit compares the beta_vae=6.4 FULL 5x5 candidate against the locked current FULL [1,0,2] model.",
        "",
        "Primary readout: classifier-only `logreg_l2` with `inner_oof_target_sens_ge_0p70_max_spec`.",
        "",
        ...
        (
            f"| beta64 beta=6.4 | {cand['auc']:.6f} | {cand['pr_auc']:.6f} | "
        ...
        "## Context",
        "",
        "beta=6.4 is 2.56× the locked model's beta_max=2.5. At the locked model, KL contributes only ~2% of",
        ...
        "The cyclical beta schedule still ramps 0->6.4 over 40% of each cycle (ratio_increase=0.4).",
        "",
        "## Decision",
        "",
        "See beta64_full_5x5_recommendation.md for promotion gate outcome.",
        "If AUC > 0.778785 AND PR-AUC >= 0.551832 both met: promote beta64 as revised manuscript model.",
    ...
    (outdir / "final_beta64_postmortem.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    main_df = pd.DataFrame(
        [
            ...
            primary_row("beta64_full_5x5", "beta_vae=6.4 FULL [1,0,2]", cand_readout),
        ]
    )
    ...
    threshold = pd.concat(
        [
            ...
            threshold_rows("beta64_full_5x5", "beta_vae=6.4 FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    foldwise = pd.concat(
        [
            ...
            foldwise_rows("beta64_full_5x5", "beta_vae=6.4 FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    pred = pd.concat(
        [
            ...
            predictions("beta64_full_5x5", "beta_vae=6.4 FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    scanner = pd.concat(
        [
            ...
            scanner_leakage("beta64_full_5x5", "beta_vae=6.4 FULL [1,0,2]", cand_run),
        ],
        ignore_index=True,
    )
