#!/usr/bin/env python3
"""Compare block-order norm_act FULL 5x5 against locked current FULL [1,0,2].
...
DEFAULT_CANDIDATE_RUN = RESULTS / "adni_v5_1_batch20260514b_ch1_0_2_block_order_norm_act_full_5x5"
DEFAULT_CANDIDATE_READOUT = DEFAULT_CANDIDATE_RUN / "classifier_only_readout"
DEFAULT_OUTPUT = RESULTS / "adni_v5_1_batch20260514b_ch1_0_2_block_order_norm_act_full_5x5_comparison"
...
def write_recommendation(outdir: Path, main: pd.DataFrame, focus: pd.DataFrame, scanner: pd.DataFrame) -> None:
    cand = main[main["run_id"].eq("block_order_norm_act_full_5x5")].iloc[0]
    ref = main[main["run_id"].eq("current_locked_ch1_0_2")].iloc[0]
    focus_cand = focus[focus["run_id"].eq("block_order_norm_act_full_5x5")].iloc[0]
    ...
    lines = [
        "# Block-Order norm_act FULL 5x5 Recommendation",
        "",
        ...
        "Context: norm_act tests Conv/Linear -> Norm -> GELU -> Dropout ordering while preserving the locked objective, beta, dropout rate, channels, and readout.",
        "If not promoted, the locked current FULL legacy_act_norm model remains the paper model.",
    ]
    (outdir / "block_order_norm_act_full_5x5_recommendation.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_final_postmortem(outdir: Path, main: pd.DataFrame, scanner: pd.DataFrame) -> None:
    ...
    cand = main[main["run_id"].eq("block_order_norm_act_full_5x5")].iloc[0]
    lines = [
        "# Final block-order norm_act post-mortem",
        "",
        "This read-only audit compares the vae_block_order=norm_act FULL 5x5 candidate against the locked current FULL [1,0,2] model.",
        "",
        "Primary readout: classifier-only `logreg_l2` with `inner_oof_target_sens_ge_0p70_max_spec`.",
        "",
        ...
        "|---|---:|---:|---:|---:|---:|---:|",
        (
            f"| locked current legacy_act_norm | {ref['auc']:.6f} | {ref['pr_auc']:.6f} | "
            f"{ref['balanced_accuracy']:.6f} | {ref['sensitivity']:.6f} | "
            f"{ref['specificity']:.6f} | {ref['f1']:.6f} |"
        ),
        (
            f"| norm_act block order | {cand['auc']:.6f} | {cand['pr_auc']:.6f} | "
        ...
        "## Context",
        "",
        "The norm_act candidate changes only module ordering inside eligible VAE blocks:",
        "Conv/Linear -> Norm -> GELU -> Dropout instead of the historical legacy_act_norm ordering.",
        "All objective, capacity, dropout-rate, schedule, channel, split, and readout settings remain locked.",
        "",
        "## Decision",
        "",
        "See block_order_norm_act_full_5x5_recommendation.md for promotion gate outcome.",
        "If AUC > 0.778785 AND PR-AUC >= 0.551832 both met: promote norm_act as revised manuscript model.",
        "Otherwise: locked current FULL legacy_act_norm model remains the paper model.",
    ]
    ...
    (outdir / "final_block_order_norm_act_postmortem.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    main_df = pd.DataFrame(
        [
            ...
            primary_row("block_order_norm_act_full_5x5", "vae_block_order=norm_act FULL [1,0,2]", cand_readout),
        ]
    )
    ...
    threshold = pd.concat(
        [
            ...
            threshold_rows("block_order_norm_act_full_5x5", "vae_block_order=norm_act FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    foldwise = pd.concat(
        [
            ...
            foldwise_rows("block_order_norm_act_full_5x5", "vae_block_order=norm_act FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    pred = pd.concat(
        [
            ...
            predictions("block_order_norm_act_full_5x5", "vae_block_order=norm_act FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    scanner = pd.concat(
        [
            ...
            scanner_leakage("block_order_norm_act_full_5x5", "vae_block_order=norm_act FULL [1,0,2]", cand_run),
        ],
        ignore_index=True,
    )
