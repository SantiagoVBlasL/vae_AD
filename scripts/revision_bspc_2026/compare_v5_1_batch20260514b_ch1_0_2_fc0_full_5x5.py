#!/usr/bin/env python3
"""Read-only fc0 FULL 5x5 comparison audit for ADNI v5.1 candidates.

Compares fc0 [1,0,2], current [1,0,2], objective-v2 [1,0,2], [1], and [1,4]
using only classifier-only logreg_l2. The primary operating point is
...
DEFAULT_FC0_RUN_DIR = RESULTS / "adni_v5_1_batch20260514b_ch1_0_2_fc0_full_5x5"
DEFAULT_FC0_READOUT_DIR = DEFAULT_FC0_RUN_DIR / "classifier_only_readout"
DEFAULT_OUTPUT_DIR = RESULTS / "adni_v5_1_batch20260514b_ch1_0_2_fc0_full_5x5_comparison"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--fc0-run-dir", type=Path, default=DEFAULT_FC0_RUN_DIR)
    parser.add_argument("--fc0-readout-dir", type=Path, default=DEFAULT_FC0_READOUT_DIR)
...
def write_outputs(
    ...
    main.to_csv(output_dir / "fc0_full_main_comparison.csv", index=False)
    (output_dir / "fc0_full_main_comparison.md").write_text(md_table(main), encoding="utf-8")
    foldwise.to_csv(output_dir / "fc0_full_foldwise_comparison.csv", index=False)
    (output_dir / "fc0_full_foldwise_comparison.md").write_text(md_table(foldwise), encoding="utf-8")
    manufacturer.to_csv(output_dir / "fc0_full_subgroup_by_manufacturer.csv", index=False)
    (output_dir / "fc0_full_subgroup_by_manufacturer.md").write_text(md_table(manufacturer), encoding="utf-8")
    sex.to_csv(output_dir / "fc0_full_subgroup_by_sex.csv", index=False)
    (output_dir / "fc0_full_subgroup_by_sex.md").write_text(md_table(sex), encoding="utf-8")
    thresholds.to_csv(output_dir / "fc0_full_threshold_comparison.csv", index=False)
    (output_dir / "fc0_full_threshold_comparison.md").write_text(
        md_table(thresholds),
        encoding="utf-8",
    )
    fold4.to_csv(output_dir / "fc0_full_fold4_deep_dive.csv", index=False)
    (output_dir / "fc0_full_fold4_deep_dive.md").write_text(md_table(fold4), encoding="utf-8")
...
def write_recommendation(
    ...
    if "fc0_ch1_0_2" in by_id.index and "current_ch1_0_2" in by_id.index:
        fc0 = by_id.loc["fc0_ch1_0_2"]
        cur = by_id.loc["current_ch1_0_2"]
        auc_ok = bool(fc0["auc"] >= 0.794)
        auc_preferred = bool(fc0["auc"] >= 0.800)
        pr_ok = bool(fc0["pr_auc"] >= cur["pr_auc"])
        ba_ok = bool(fc0["balanced_accuracy"] >= cur["balanced_accuracy"] - 0.01)
        f1_ok = bool(fc0["f1"] >= cur["f1"] - 0.01)
        subgroup_fragility = False
        fragility_notes: List[str] = []
        if not manufacturer.empty:
            fc0_m = manufacturer[manufacturer["run_id"].eq("fc0_ch1_0_2")]
            cur_m = manufacturer[manufacturer["run_id"].eq("current_ch1_0_2")]
            if not fc0_m.empty and not cur_m.empty:
                merged_m = fc0_m.merge(cur_m, on="Manufacturer", suffixes=("_fc0", "_current"))
                for _, row in merged_m.iterrows():
                    ba_drop = float(row["balanced_accuracy_fc0"]) - float(row["balanced_accuracy_current"])
                    sens_drop = float(row["sensitivity_fc0"]) - float(row["sensitivity_current"])
                    spec_drop = float(row["specificity_fc0"]) - float(row["specificity_current"])
        ...
        if not sex.empty:
            fc0_s = sex[sex["run_id"].eq("fc0_ch1_0_2")]
            cur_s = sex[sex["run_id"].eq("current_ch1_0_2")]
            if not fc0_s.empty and not cur_s.empty:
                merged_s = fc0_s.merge(cur_s, on="Sex", suffixes=("_fc0", "_current"))
                for _, row in merged_s.iterrows():
                    ba_drop = float(row["balanced_accuracy_fc0"]) - float(row["balanced_accuracy_current"])
                    if ba_drop < -0.05:
                        subgroup_fragility = True
                        fragility_notes.append(f"Sex {row['Sex']}: BA {ba_drop:+.3f}")
        foldwise_fc0 = foldwise[foldwise["run_id"].eq("fc0_ch1_0_2")].copy()
        fold4_driver = False
        if not foldwise_fc0.empty and "auc" in foldwise_fc0.columns:
            fold4_auc = foldwise_fc0.loc[foldwise_fc0["fold"].eq(4), "auc"]
            non_fold4_auc = foldwise_fc0.loc[~foldwise_fc0["fold"].eq(4), "auc"]
        ...
        lines.append(
            "- fc0 [1,0,2] vs current [1,0,2]: "
            f"delta AUC={fc0['auc'] - cur['auc']:+.4f}, "
            f"delta PR-AUC={fc0['pr_auc'] - cur['pr_auc']:+.4f}, "
            f"delta BA={fc0['balanced_accuracy'] - cur['balanced_accuracy']:+.4f}, "
            f"delta F1={fc0['f1'] - cur['f1']:+.4f}, "
            f"delta sensitivity={fc0['sensitivity'] - cur['sensitivity']:+.4f}, "
            f"delta specificity={fc0['specificity'] - cur['specificity']:+.4f}."
        )
        lines.append(
            "- Revised-main decision rule for fc0: "
            ...
            f"Recommendation: {'promote fc0 as revised main model' if decision_ok else 'do not promote fc0 as revised main model'}."
        )
        if fragility_notes:
    ...
    runs = [
        {
            ...
            "channels": "[1,0,2]",
            "objective": "mse_sum_batchmean_current",
            "readout_dir": resolve(args.fc0_readout_dir),
            "run_dir": resolve(args.fc0_run_dir),
            "notes": "FULL 5x5 confirmation candidate with intermediate_fc_dim_vae=0",
        },
        *RUNS,
    ]
