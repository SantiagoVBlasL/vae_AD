#!/usr/bin/env python3
"""Compare dropout=0.20 FULL 5x5 against locked current FULL [1,0,2].
...
DEFAULT_CANDIDATE_RUN = RESULTS / "adni_v5_1_batch20260514b_ch1_0_2_dropout020_full_5x5"
DEFAULT_CANDIDATE_READOUT = DEFAULT_CANDIDATE_RUN / "classifier_only_readout"
DEFAULT_OUTPUT = RESULTS / "adni_v5_1_batch20260514b_ch1_0_2_dropout020_full_5x5_comparison"
...
def write_recommendation(outdir: Path, main: pd.DataFrame, focus: pd.DataFrame, scanner: pd.DataFrame) -> None:
    cand = main[main["run_id"].eq("dropout020_full_5x5")].iloc[0]
    ref = main[main["run_id"].eq("current_locked_ch1_0_2")].iloc[0]
    focus_cand = focus[focus["run_id"].eq("dropout020_full_5x5")].iloc[0]
    ...
    lines = [
        "# Dropout 0.20 FULL 5x5 Recommendation",
        "",
        ...
        "Context: dropout020 completes the monotone dropout experiment (0.10 -> 0.15 -> 0.20).",
        "If not promoted, the locked current FULL dropout=0.15 model remains the paper model.",
    ]
    (outdir / "dropout020_full_5x5_recommendation.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_final_postmortem(outdir: Path, main: pd.DataFrame, scanner: pd.DataFrame) -> None:
    ...
    cand = main[main["run_id"].eq("dropout020_full_5x5")].iloc[0]
    lines = [
        "# Final dropout020 post-mortem",
        "",
        "This read-only audit compares the dropout_rate_vae=0.20 FULL 5x5 candidate against the locked current FULL [1,0,2] model.",
        "",
        "Primary readout: classifier-only `logreg_l2` with `inner_oof_target_sens_ge_0p70_max_spec`.",
        "",
        ...
        (
            f"| dropout020 dropout=0.20 | {cand['auc']:.6f} | {cand['pr_auc']:.6f} | "
        ...
        "## Context",
        "",
        "dropout020 completes the monotone single-variable dropout experiment (0.10 -> 0.15 -> 0.20).",
        "Architecture audit (2026-05-20) found: only 5% change in effective active FC units (103/2048);",
        "KL contributes ~2% of total loss; FAST->FULL reversal risk exists from manufacturer-balanced sampler precedent.",
        "",
        "## Decision",
        "",
        "See dropout020_full_5x5_recommendation.md for promotion gate outcome.",
        "If AUC > 0.778785 AND PR-AUC >= 0.551832 both met: promote dropout020 as revised manuscript model.",
        "Otherwise: locked current FULL dropout=0.15 model remains the paper model.",
        "If both fail: this is positive evidence that the regularization regime is at a stable optimum.",
    ]
    ...
    (outdir / "final_dropout020_postmortem.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    main_df = pd.DataFrame(
        [
            ...
            primary_row("dropout020_full_5x5", "dropout_rate_vae=0.20 FULL [1,0,2]", cand_readout),
        ]
    )
    ...
    threshold = pd.concat(
        [
            ...
            threshold_rows("dropout020_full_5x5", "dropout_rate_vae=0.20 FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    foldwise = pd.concat(
        [
            ...
            foldwise_rows("dropout020_full_5x5", "dropout_rate_vae=0.20 FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    pred = pd.concat(
        [
            ...
            predictions("dropout020_full_5x5", "dropout_rate_vae=0.20 FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    scanner = pd.concat(
        [
            ...
            scanner_leakage("dropout020_full_5x5", "dropout_rate_vae=0.20 FULL [1,0,2]", cand_run),
        ],
        ignore_index=True,
    )
