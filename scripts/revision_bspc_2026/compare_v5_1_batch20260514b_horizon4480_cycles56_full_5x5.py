#!/usr/bin/env python3
"""Compare horizon4480/cycles56 FULL 5x5 against locked current FULL [1,0,2].

Promotion rule:
  - AUC > 0.778785 AND PR-AUC >= 0.551832 (must beat BOTH simultaneously)
...
DEFAULT_CANDIDATE_RUN = RESULTS / "adni_v5_1_batch20260514b_ch1_0_2_horizon4480_cycles56_full_5x5"
DEFAULT_CANDIDATE_READOUT = DEFAULT_CANDIDATE_RUN / "classifier_only_readout"
DEFAULT_OUTPUT = RESULTS / "adni_v5_1_batch20260514b_ch1_0_2_horizon4480_cycles56_full_5x5_comparison"
...
def write_recommendation(outdir: Path, main: pd.DataFrame, focus: pd.DataFrame, scanner: pd.DataFrame) -> None:
    cand = main[main["run_id"].eq("horizon4480_cycles56_full_5x5")].iloc[0]
    ref = main[main["run_id"].eq("current_locked_ch1_0_2")].iloc[0]
    focus_cand = focus[focus["run_id"].eq("horizon4480_cycles56_full_5x5")].iloc[0]
    focus_ref = focus[focus["run_id"].eq("current_locked_ch1_0_2")].iloc[0]
    # Strict dual promotion gate: must beat BOTH AUC AND PR-AUC simultaneously
    ...
    lines = [
        "# Horizon4480/Cycles56 FULL 5x5 Recommendation",
        "",
        ...
        "Context: horizon4480/cycles56 preserves the locked 80-epoch beta/LR cycle length while extending the training budget by 640 epochs.",
        "If not promoted, the locked current FULL 3840/48 model remains the paper model.",
    ]
    (outdir / "horizon4480_cycles56_full_5x5_recommendation.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_final_postmortem(outdir: Path, main: pd.DataFrame, scanner: pd.DataFrame) -> None:
    ...
    cand = main[main["run_id"].eq("horizon4480_cycles56_full_5x5")].iloc[0]
    lines = [
        "# Final horizon4480/cycles56 post-mortem",
        "",
        "This read-only audit compares the long-horizon 4480-epoch/56-cycle FULL 5x5 candidate against the locked current FULL [1,0,2] model.",
        "",
        "Primary readout: classifier-only `logreg_l2` with `inner_oof_target_sens_ge_0p70_max_spec`.",
        "",
        ...
        "|---|---:|---:|---:|---:|---:|---:|",
        (
            f"| locked current beta=2.5 | {ref['auc']:.6f} | {ref['pr_auc']:.6f} | "
            f"{ref['balanced_accuracy']:.6f} | {ref['sensitivity']:.6f} | "
            f"{ref['specificity']:.6f} | {ref['f1']:.6f} |"
        ),
        (
            f"| horizon4480 cycles56 | {cand['auc']:.6f} | {cand['pr_auc']:.6f} | "
        ...
        "## Context",
        "",
        "The controlled change extends the training horizon from 3840 to 4480 epochs while preserving the 80-epoch beta/LR cycle length",
        "(3840/48 = 80 and 4480/56 = 80). This directly tests whether the locked run was horizon-limited without changing beta,",
        "dropout, loss scaling, channels, batch size, classifier setup, or stratification.",
        "",
        "## Decision",
        "",
        "See horizon4480_cycles56_full_5x5_recommendation.md for promotion gate outcome.",
        "If AUC > 0.778785 AND PR-AUC >= 0.551832 both met: promote the long-horizon candidate as revised manuscript model.",
        "Otherwise: locked current FULL beta=2.5 model remains the paper model.",
    ]
    ...
    (outdir / "final_horizon4480_cycles56_postmortem.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    main_df = pd.DataFrame(
        [
            ...
            primary_row("horizon4480_cycles56_full_5x5", "horizon4480/cycles56 FULL [1,0,2]", cand_readout),
        ]
    )
    ...
    threshold = pd.concat(
        [
            ...
            threshold_rows("horizon4480_cycles56_full_5x5", "horizon4480/cycles56 FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    foldwise = pd.concat(
        [
            ...
            foldwise_rows("horizon4480_cycles56_full_5x5", "horizon4480/cycles56 FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    pred = pd.concat(
        [
            ...
            predictions("horizon4480_cycles56_full_5x5", "horizon4480/cycles56 FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    scanner = pd.concat(
        [
            ...
            scanner_leakage("horizon4480_cycles56_full_5x5", "horizon4480/cycles56 FULL [1,0,2]", cand_run),
        ],
        ignore_index=True,
    )
