#!/usr/bin/env python3
"""Compare no-decoder-dropout FULL 5x5 against locked current FULL [1,0,2]."""
...
DEFAULT_CANDIDATE_RUN = RESULTS / "adni_v5_1_batch20260514b_ch1_0_2_no_decoder_dropout_full_5x5"
DEFAULT_CANDIDATE_READOUT = DEFAULT_CANDIDATE_RUN / "classifier_only_readout"
DEFAULT_OUTPUT = RESULTS / "adni_v5_1_batch20260514b_ch1_0_2_no_decoder_dropout_full_5x5_comparison"
...
def write_recommendation(outdir: Path, main: pd.DataFrame, focus: pd.DataFrame, scanner: pd.DataFrame) -> None:
    cand = main[main["run_id"].eq("no_decoder_dropout_full_5x5")].iloc[0]
    ref = main[main["run_id"].eq("current_locked_ch1_0_2")].iloc[0]
    focus_cand = focus[focus["run_id"].eq("no_decoder_dropout_full_5x5")].iloc[0]
    ...
    lines = [
        "# No-Decoder-Dropout FULL 5x5 Recommendation",
        "",
        ...
        "",
        "Promotion requires AUC > locked current, PR-AUC nondecreasing, BA/F1 not materially worse, Philips CN FP and GE AD FN not worse, and QC/scanner leakage not worse.",
    ]
    (outdir / "no_decoder_dropout_full_5x5_recommendation.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_final_postmortem(outdir: Path, main: pd.DataFrame, scanner: pd.DataFrame) -> None:
    ...
    cand = main[main["run_id"].eq("no_decoder_dropout_full_5x5")].iloc[0]
    lines = [
        "# Final No-Decoder-Dropout Post-Mortem",
        "",
        "This read-only audit compares the `vae_dropout_scope=no_decoder_dropout` FULL 5x5 candidate against the locked current FULL [1,0,2] model.",
        "",
        "Primary readout: classifier-only `logreg_l2` with `inner_oof_target_sens_ge_0p70_max_spec`.",
        "",
        ...
        "|---|---:|---:|---:|---:|---:|---:|",
        (
            f"| locked current legacy_all | {ref['auc']:.6f} | {ref['pr_auc']:.6f} | "
            f"{ref['balanced_accuracy']:.6f} | {ref['sensitivity']:.6f} | "
            f"{ref['specificity']:.6f} | {ref['f1']:.6f} |"
        ),
        (
            f"| no_decoder_dropout | {cand['auc']:.6f} | {cand['pr_auc']:.6f} | "
        ...
        "## Decision",
        "",
        "The no-decoder-dropout candidate should NOT replace the locked current FULL model unless it satisfies the pre-registered promotion rule.",
        "",
        "The promotion decision is based on Stage B classifier-only `logreg_l2` readout with true inner-CV OOF thresholding, not Stage A dummy/canonical classifier metrics.",
        "",
        "If the candidate does not exceed locked AUC while preserving PR-AUC, BA/F1, Philips CN FP, GE AD FN, and scanner leakage behavior, the locked current FULL tanh [1,0,2] model remains the paper model.",
    ]
    ...
    (outdir / "final_no_decoder_dropout_postmortem.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    main = pd.DataFrame(
        [
            ...
            primary_row("no_decoder_dropout_full_5x5", "vae_dropout_scope=no_decoder_dropout FULL [1,0,2]", cand_readout),
        ]
    )
    ...
    threshold = pd.concat(
        [
            ...
            threshold_rows("no_decoder_dropout_full_5x5", "vae_dropout_scope=no_decoder_dropout FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    foldwise = pd.concat(
        [
            ...
            foldwise_rows("no_decoder_dropout_full_5x5", "vae_dropout_scope=no_decoder_dropout FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    pred = pd.concat(
        [
            ...
            predictions("no_decoder_dropout_full_5x5", "vae_dropout_scope=no_decoder_dropout FULL [1,0,2]", cand_readout),
        ],
        ignore_index=True,
    )
    ...
    scanner = pd.concat(
        [
            ...
            scanner_leakage("no_decoder_dropout_full_5x5", "vae_dropout_scope=no_decoder_dropout FULL [1,0,2]", cand_run),
        ],
        ignore_index=True,
    )
