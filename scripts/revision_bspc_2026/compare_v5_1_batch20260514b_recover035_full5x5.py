#!/usr/bin/env python3
"""Compare recover035 FULL 5x5 against locked v5.1b horizon4480/cycles56 [1,0,2].

Reference: locked v5.1b (horizon4480/cycles56) readout.
Candidate: recover035_full5x5 readout.

Promotion rule (exploratory):
  - AUC > 0.782951 AND PR-AUC >= 0.559873 (must beat BOTH simultaneously)
  - BA/F1 do not materially worsen vs locked
  - Philips CN FP rate does not worsen
  - GE AD FN rate does not worsen
  - Scanner leakage/QC does not worsen
  - Note: exploratory because outer-fold splits changed with the rescued subject.
"""
...
DEFAULT_CANDIDATE_RUN = RESULTS / "recover035_full5x5"
DEFAULT_CANDIDATE_READOUT = DEFAULT_CANDIDATE_RUN / "classifier_only_readout"
DEFAULT_OUTPUT = RESULTS / "recover035_full5x5_comparison"
...
RUN_ID_REF = "locked_horizon4480_cycles56"
RUN_ID_CAND = "recover035_full5x5"
LABEL_REF = "locked v5.1b horizon4480/cycles56 [1,0,2]"
LABEL_CAND = "recover035 FULL [1,0,2] (035_S_6927 added)"
...
def add_deltas(main: pd.DataFrame) -> pd.DataFrame:
    ref = main[main["run_id"].eq(RUN_ID_REF)]
    if ref.empty:
        return main
    ref_row = ref.iloc[0]
    out = main.copy()
...
def philips_ge_focus(pred: pd.DataFrame) -> pd.DataFrame:
    ...
    ref = out[out["run_id"].eq(RUN_ID_REF)]
    if not ref.empty:
        ref_row = ref.iloc[0]
...
def recover035_fold_report(pred_cand: pd.DataFrame) -> pd.DataFrame:
    """Report fold assignment and prediction for 035_S_6927 in the candidate."""
...
def write_recommendation(
    ...
    ref = main[main["run_id"].eq(RUN_ID_REF)].iloc[0]
    focus_cand = focus[focus["run_id"].eq(RUN_ID_CAND)].iloc[0]
    focus_ref = focus[focus["run_id"].eq(RUN_ID_REF)].iloc[0]
    ...
    lines = [
        "# recover035 FULL 5x5 Recommendation",
        "",
        ...
        "The locked parameters (epochs_vae=4480, cycles=56, T0=80, etc.) are identical to locked v5.1b.",
        "",
        ...
        "Context: recover035 adds one AD SIEMENS subject (035_S_6927) rescued from idaSearch. "
        "Any metric change is due to (a) the additional training subject and (b) the shifted outer-fold splits.",
        "If not promoted, the locked v5.1b (horizon4480/cycles56) remains the paper model.",
        fold035_note,
    ]
    (outdir / "recover035_recommendation.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_final_postmortem(outdir: Path, main: pd.DataFrame, scanner: pd.DataFrame) -> None:
    ...
    cand = main[main["run_id"].eq(RUN_ID_CAND)].iloc[0]
    lines = [
        "# recover035 FULL 5x5 post-mortem",
        "",
        "This read-only comparison contrasts the metadata-rescue recover035 retrain against the locked v5.1b (horizon4480/cycles56) [1,0,2].",
        "",
        f"Primary readout: classifier-only `{PRIMARY_MODEL}` with `{PRIMARY_THRESHOLD}`.",
        "",
        ...
        (
            f"| recover035 FULL [1,0,2] | {cand['auc']:.6f} | {cand['pr_auc']:.6f} | "
        ...
        "## Context",
        "",
        "recover035 adds 035_S_6927 (AD, Age=59.6, Sex=F, Manufacturer=SIEMENS, tensor_index=256) to the",
        "classifier pool (n=396→397, AD=96→97) and VAE pool (n=646→647, AD=96→97).",
        "All VAE/classifier parameters are identical to locked v5.1b (epochs_vae=4480, cycles=56, T0=80).",
        "The outer-fold stratification shifts slightly due to the additional AD SIEMENS subject.",
        "Any observed metric change is attributable to the added subject and split shift, not hyperparameter change.",
        "",
        "## Decision",
        "",
        "See recover035_recommendation.md for promotion gate outcome.",
        f"If AUC > {LOCKED_AUC} AND PR-AUC >= {LOCKED_PR_AUC} both met: consider promoting the rescue retrain as revised manuscript model.",
        "Otherwise: locked v5.1b horizon4480/cycles56 remains the paper model and 035_S_6927 remains excluded.",
    ]
    ...
    (outdir / "final_recover035_postmortem.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    print(f"Promotion gate       : AUC > {LOCKED_AUC} AND PR-AUC >= {LOCKED_PR_AUC} (both required simultaneously; exploratory)")
    ...
    main_df = pd.DataFrame([
        primary_row(RUN_ID_REF, LABEL_REF, ref_readout),
        primary_row(RUN_ID_CAND, LABEL_CAND, cand_readout),
    ])
    main_df = add_deltas(main_df)
    write_table(outdir, "main_model_comparison", main_df)

    threshold = pd.concat([
        threshold_rows(RUN_ID_REF, LABEL_REF, ref_readout),
        threshold_rows(RUN_ID_CAND, LABEL_CAND, cand_readout),
    ], ignore_index=True)
    write_table(outdir, "threshold_comparison", threshold)

    foldwise = pd.concat([
        foldwise_rows(RUN_ID_REF, LABEL_REF, ref_readout),
        foldwise_rows(RUN_ID_CAND, LABEL_CAND, cand_readout),
    ], ignore_index=True)
    write_table(outdir, "foldwise_comparison", foldwise)

    pred = pd.concat([
        predictions(RUN_ID_REF, LABEL_REF, ref_readout),
        predictions(RUN_ID_CAND, LABEL_CAND, cand_readout),
    ], ignore_index=True)
    ...
    scanner = pd.concat([
        scanner_leakage(RUN_ID_REF, LABEL_REF, ref_run),
        scanner_leakage(RUN_ID_CAND, LABEL_CAND, cand_run),
    ], ignore_index=True)
    write_table(outdir, "scanner_leakage_comparison", scanner)

    write_recommendation(outdir, main_df, focus, scanner, fold035)
