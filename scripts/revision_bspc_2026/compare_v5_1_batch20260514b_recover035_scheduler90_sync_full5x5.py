#!/usr/bin/env python3
"""Compare recover035_scheduler90_sync FULL 5x5 against locked v5.1b and recover035_full5x5.

Two reference comparisons:
  1. vs locked v5.1b horizon4480/cycles56 [1,0,2] — primary promotion gate
  2. vs recover035_full5x5 [1,0,2] — isolates the scheduler-sync effect (same metadata pool)

Promotion rule:
  AUC > 0.782951 AND PR-AUC >= 0.559873 (must beat BOTH locked reference values simultaneously)
  Note: exploratory because outer-fold splits shifted slightly due to the 035_S_6927 rescue.
"""
...
DEFAULT_CANDIDATE_RUN = RESULTS / "recover035_scheduler90_sync_full5x5"
DEFAULT_CANDIDATE_READOUT = DEFAULT_CANDIDATE_RUN / "classifier_only_readout"
DEFAULT_OUTPUT = RESULTS / "recover035_scheduler90_sync_full5x5_comparison"
...
RUN_ID_CAND = "recover035_scheduler90_sync_full5x5"

LABEL_LOCKED = "locked v5.1b horizon4480/cycles56 [1,0,2]"
LABEL_RECOVER035 = "recover035 FULL [1,0,2] (035_S_6927, locked schedule)"
LABEL_CAND = "recover035_scheduler90_sync FULL [1,0,2] (035_S_6927, 90-epoch cycles)"
...
def scanner_leakage(run_id: str, label: str, run_dir: Path) -> pd.DataFrame:
    ...
    if "latent_minus_raw_site_acc" not in raw.columns:
        if {"acc_site_latent", "acc_site_raw"}.issubset(raw.columns):
            ...
            warnings.append("latent_minus_raw_site_acc computed as acc_site_latent - acc_site_raw")
        else:
...
def add_deltas_vs_recover035(main: pd.DataFrame) -> pd.DataFrame:
    ...
    out = main.copy()
    for metric in METRICS:
        delta_col = f"delta_vs_recover035_{metric}"
        out[delta_col] = (
            pd.to_numeric(out[metric], errors="coerce") - float(ref_row[metric])
        )
    return out
...
def write_recommendation(
    ...
    if not fold035.empty and "fold" in fold035.columns:
        ...
        fold035_note = (
            ...
            f"Predicted label: {y_pred} (y_score={y_score:.4f}). "
            f"y_true=1 (AD). "
            f"{'Correctly classified as AD.' if y_pred == 1 else 'Misclassified as CN.'}"
        )

    recover035_delta_note = ""
    if recover035_available and RUN_ID_RECOVER035 in main["run_id"].values:
        r035 = main[main["run_id"].eq(RUN_ID_RECOVER035)].iloc[0]
        delta_auc_sched = float(cand["auc"]) - float(r035["auc"])
        delta_pr_auc_sched = float(cand["pr_auc"]) - float(r035["pr_auc"])
        delta_ba_sched = float(cand["balanced_accuracy"]) - float(r035["balanced_accuracy"])
        recover035_delta_note = (
            f"\n\n## Scheduler effect (candidate vs recover035_full5x5)\n\n"
            f"recover035_full5x5 uses identical metadata but locked schedule (epochs=4480, cycles=56, T0=80).\n"
            f"Delta AUC (scheduler): {delta_auc_sched:+.4f}\n"
            f"Delta PR-AUC (scheduler): {delta_pr_auc_sched:+.4f}\n"
            f"Delta BA (scheduler): {delta_ba_sched:+.4f}\n"
        )

    lines = [
        "# recover035_scheduler90_sync FULL 5x5 Recommendation",
        "",
        f"Primary readout: classifier-only `{PRIMARY_MODEL}` with `{PRIMARY_THRESHOLD}`.",
        "",
        "**Exploratory run**: outer-fold splits changed because 035_S_6927 was added to the classifier pool.",
        "Two controlled changes vs locked v5.1b: (1) recover035 metadata (035_S_6927 added as AD), "
        "(2) scheduler-sync (epochs=4500, cycles=50, T0=90 — 90-epoch β/LR cycles).",
        "",
        ...
        f"Delta AUC={cand['delta_vs_locked_auc']:+.4f}, Delta PR-AUC={cand['delta_vs_locked_pr_auc']:+.4f}.",
    ]
    if focus_cand is not None and focus_ref is not None:
        lines += [
    ...
    lines += [
        ...
        "Context: recover035_scheduler90_sync combines (1) the 035_S_6927 metadata rescue and "
        "(2) synchronized 90-epoch β/LR cycles. Any metric change is attributable to both factors.",
        fold035_note,
        recover035_delta_note,
    ]
    (outdir / "recover035_scheduler90_sync_recommendation.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_final_postmortem(
    ...
    if recover035_available and RUN_ID_RECOVER035 in main["run_id"].values:
        r035 = main[main["run_id"].eq(RUN_ID_RECOVER035)].iloc[0]
        table_rows.append(
            f"| recover035 FULL (locked schedule) | {r035['auc']:.6f} | {r035['pr_auc']:.6f} | "
            f"{r035['balanced_accuracy']:.6f} | {r035['sensitivity']:.6f} | "
            f"{r035['specificity']:.6f} | {r035['f1']:.6f} |"
        )
    table_rows.append(
        f"| recover035_scheduler90_sync FULL | {cand['auc']:.6f} | {cand['pr_auc']:.6f} | "
        f"{cand['balanced_accuracy']:.6f} | {cand['sensitivity']:.6f} | "
        f"{cand['specificity']:.6f} | {cand['f1']:.6f} |"
    )

    lines = [
        "# recover035_scheduler90_sync FULL 5x5 post-mortem",
        "",
        "Read-only comparison of the recover035_scheduler90_sync candidate vs locked v5.1b (primary) "
        "and vs recover035_full5x5 (scheduler effect isolation).",
        "",
        f"Primary readout: classifier-only `{PRIMARY_MODEL}` with `{PRIMARY_THRESHOLD}`.",
        "",
        ...
        *table_rows,
        ...
        "## Context",
        "",
        "recover035_scheduler90_sync combines two controlled changes vs locked v5.1b:",
        "  1. Metadata rescue: adds 035_S_6927 (AD, Age=59.6, Sex=F, Manufacturer=SIEMENS, tensor_index=256)",
        "     to the classifier pool (n=396→397, AD=96→97) and VAE pool (n=646→647, AD=96→97).",
        "  2. Scheduler sync: epochs_vae 4480→4500, cyclical_beta_n_cycles 56→50, lr_scheduler_T0 80→90.",
        "     β and LR cosine warm-restart cycles phase-aligned at 90 epochs each.",
        "128_S_2002 remains excluded from both VAE and classifier pools.",
        "The outer-fold stratification shifts slightly vs locked due to the additional AD SIEMENS subject.",
        "",
        "## Decision",
        "",
        "See recover035_scheduler90_sync_recommendation.md for promotion gate outcome.",
...
def main() -> int:
    ...
    print(f"Recover subject              : {RECOVER_SUBJECT} (AD, SIEMENS, Age=59.6)")
    if not recover035_ok:
        print("  WARNING: recover035_full5x5 readout not available — scheduler-effect delta table will be omitted.")
    if args.dry_run:
    ...
    active_runs = [
        (RUN_ID_LOCKED, LABEL_LOCKED, locked_readout),
        (RUN_ID_CAND, LABEL_CAND, cand_readout),
    ]
    if recover035_ok:
        active_runs.insert(1, (RUN_ID_RECOVER035, LABEL_RECOVER035, recover035_readout))
    ...
    scanner_parts = []
    for rid, lbl, run_dir in [(RUN_ID_LOCKED, LABEL_LOCKED, locked_run), (RUN_ID_CAND, LABEL_CAND, cand_run)]:
    ...
    write_recommendation(outdir, main_df, focus, scanner, fold035, recover035_available=recover035_ok)
    write_final_postmortem(outdir, main_df, scanner, recover035_available=recover035_ok)
