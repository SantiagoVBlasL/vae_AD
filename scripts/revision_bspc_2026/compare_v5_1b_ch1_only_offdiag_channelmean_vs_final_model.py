#!/usr/bin/env python3
"""Read-only comparison for v5.1b [1] offdiag_channelmean FULL run."""
...
OUT_DIR = RESULTS / "adni_v5_1b_ch1_only_offdiag_channelmean_vs_final_model_comparison"

PRIMARY_MODEL = "logreg_l2"
PRIMARY_THRESHOLD = "inner_oof_target_sens_ge_0p70_max_spec"
FIXED_THRESHOLD = "fixed_0p5"
REFERENCE_ID = "v5_1b_horizon4480_ch1_0_2"
CANDIDATE_ID = "v5_1b_ch1_only_offdiag_channelmean"
...
def write_recommendation(outdir: Path, main: pd.DataFrame) -> None:
    ...
    text = f"""# v5.1b [1] Offdiag-Channelmean FULL Recommendation

Primary readout: `{PRIMARY_MODEL}` with `{PRIMARY_THRESHOLD}`.

Promotion reference is the final v5.1b horizon4480/cycles56 `[1,0,2]` model.
...
Final promotion still requires integrity audit PASS and no worse manufacturer/site subgroup behavior or scanner/manufacturer leakage.
"""
...
def main() -> int:
    ...
    specs = [
        ...
        (CANDIDATE_ID, "v5.1b horizon4480/cycles56 [1] offdiag_channelmean", candidate_run, candidate_readout),
    ]
    ...
    write_table(outdir, "manufacturer_subgroup_metrics", subgroup(preds, "Manufacturer"))
    write_table(outdir, "sex_subgroup_metrics", subgroup(preds, "Sex"))
    write_table(outdir, "sitecode_subgroup_metrics", subgroup(preds, "SiteCode"))
