#!/usr/bin/env python3
"""Read-only comparison for v5.1c objective-v2 horizon10000/cycles125 FULL run."""
...
CANDIDATE_RUN = RESULTS / "adni_v5_1c_recover035_ch1_0_2_objective_v2_offdiag_channelmean_horizon10000_cycles125_full_5x5"
...
V51C10000_CURRENT_RUN = RESULTS / "adni_v5_1c_recover035_ch1_0_2_horizon10000_cycles125_full_5x5"
V51C10000_CURRENT_READOUT = V51C10000_CURRENT_RUN / "classifier_only_readout"
OUT_DIR = RESULTS / "adni_v5_1c_objective_v2_offdiag_channelmean_horizon10000_vs_references_comparison"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v51c10000-current-run-dir", type=Path, default=V51C10000_CURRENT_RUN)
    parser.add_argument("--v51c10000-current-readout-dir", type=Path, default=V51C10000_CURRENT_READOUT)
...
def write_recommendation(outdir: Path, main: pd.DataFrame, leakage: pd.DataFrame, mfr: pd.DataFrame, site: pd.DataFrame) -> None:
    cand = main[main["run_id"].eq("v5_1c_horizon10000_objective_v2_offdiag_channelmean")].iloc[0]
    ...
    text = f"""# v5.1c Objective-v2 Horizon10000/Cycles125 Recommendation
...
- Candidate AUC={float(cand['auc']):.6f}, PR-AUC={float(cand['pr_auc']):.6f}, BA={float(cand['balanced_accuracy']):.6f}, sensitivity={float(cand['sensitivity']):.6f}, F1={float(cand['f1']):.6f}
...
Controlled scientific diff versus v5.1c horizon10000/cycles125 current-loss run:
...
def main() -> int:
    ...
    v51c10000_current_run = resolve(args.v51c10000_current_run_dir)
    v51c10000_current_readout = resolve(args.v51c10000_current_readout_dir)
    ...
    validate_readout("v5.1c 10000 current-loss reference", v51c10000_current_readout)
    ...
    if args.dry_run:
        ...
        print(f"v5.1c 10000 current-loss ref: {v51c10000_current_run}")
    ...
    specs = [
        ...
        ("v5_1c_horizon10000_current_loss", "v5.1c recover035 horizon10000/cycles125 current loss", v51c10000_current_run, v51c10000_current_readout),
        (
            "v5_1c_horizon10000_objective_v2_offdiag_channelmean",
            "v5.1c recover035 horizon10000/cycles125 offdiag_channelmean_sum",
            candidate_run,
            candidate_readout,
        ),
    ]
    ...
    command_log = {
        ...
        "v51c10000_current_loss_reference_run": rel(v51c10000_current_run),
        "v51c10000_current_loss_reference_readout": rel(v51c10000_current_readout),
