#!/usr/bin/env python3
"""Compare recover035_latent384_T80_h10000_p560 FULL 5x5 against three references.

References:
  1. locked v5.1b horizon4480/cycles56 [1,0,2]            — primary promotion gate
  2. recover035_longpatience_T80_h10000_p560 [1,0,2]      — direct base (same everything except latent_dim)
  3. recover035_full5x5 [1,0,2]                           — original latent256 short-horizon run (context)

Isolation principle:
  latent384 vs longpatience256 isolates the latent capacity effect:
  same metadata, scheduler, channels, loss — only latent_dim 256 -> 384.
...
DEFAULT_CANDIDATE_RUN = RESULTS / "recover035_latent384_T80_h10000_p560_full5x5"
DEFAULT_CANDIDATE_READOUT = DEFAULT_CANDIDATE_RUN / "classifier_only_readout"
DEFAULT_OUTPUT = RESULTS / "recover035_latent384_T80_h10000_p560_full5x5_comparison"
...
RUN_ID_CAND = "recover035_latent384_T80_h10000_p560_full5x5"
...
LABEL_CAND = "recover035_latent384 FULL [1,0,2] (latent384, 10000/125, T0=80, p=560)"
...
def add_deltas_vs_longpatience256(main: pd.DataFrame) -> pd.DataFrame:
    ref = main[main["run_id"].eq(RUN_ID_LONGPATIENCE256)]
    if ref.empty:
        return main
    ref_row = ref.iloc[0]
    out = main.copy()
    for metric in METRICS:
        out[f"delta_vs_longpatience256_{metric}"] = (
            pd.to_numeric(out[metric], errors="coerce") - float(ref_row[metric])
        )
    return out
...
def write_recommendation(
    ...
    longpatience256_available: bool,
    ...
    latent384_vs_longpatience256_note = ""
    if longpatience256_available and RUN_ID_LONGPATIENCE256 in main["run_id"].values:
        lp = main[main["run_id"].eq(RUN_ID_LONGPATIENCE256)].iloc[0]
        delta_auc = float(cand["auc"]) - float(lp["auc"])
        delta_pr = float(cand["pr_auc"]) - float(lp["pr_auc"])
        delta_ba = float(cand["balanced_accuracy"]) - float(lp["balanced_accuracy"])
        latent384_vs_longpatience256_note = (
            f"\n\n## Latent capacity effect (latent384 vs longpatience256 — primary isolation)\n\n"
            f"recover035_longpatience256 is the direct base: same metadata, scheduler, channels, loss.\n"
            f"Only difference: latent_dim 256 -> 384.\n"
            f"Delta AUC (latent384 - longpatience256): {delta_auc:+.4f}\n"
            f"Delta PR-AUC: {delta_pr:+.4f}\n"
            f"Delta BA: {delta_ba:+.4f}\n"
        )

    latent384_vs_recover035_note = ""
    if recover035_available and RUN_ID_RECOVER035 in main["run_id"].values:
        r035 = main[main["run_id"].eq(RUN_ID_RECOVER035)].iloc[0]
        delta_auc_r = float(cand["auc"]) - float(r035["auc"])
        delta_pr_r = float(cand["pr_auc"]) - float(r035["pr_auc"])
        latent384_vs_recover035_note = (
            f"\n\n## Latent384 vs recover035_full5x5\n\n"
            f"recover035_full5x5 uses epochs=4480, cycles=56, T0=80, patience=320, latent_dim=256.\n"
            f"latent384 uses epochs=10000, cycles=125, T0=80, patience=560, latent_dim=384.\n"
            f"Delta AUC (latent384 - recover035): {delta_auc_r:+.4f}\n"
            f"Delta PR-AUC: {delta_pr_r:+.4f}\n"
        )

    lines = [
        "# recover035_latent384_T80_h10000_p560 FULL 5x5 Recommendation",
        "",
        ...
        "Base model: recover035_longpatience_T80_h10000_p560_full5x5.",
        "Controlled change: latent_dim 256 → 384 (single-factor capacity sensitivity).",
        "All other parameters unchanged: epochs=10000, cycles=125, T0=80, patience=560, n_iter=500, β=2.5.",
        "",
    ...
    (outdir / "latent384_recommendation.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_final_postmortem(
    ...
    table_rows = [
        f"| locked v5.1b horizon4480/cycles56 latent256 | {ref['auc']:.6f} | {ref['pr_auc']:.6f} | "
    ...
    if recover035_available and RUN_ID_RECOVER035 in main["run_id"].values:
        r035 = main[main["run_id"].eq(RUN_ID_RECOVER035)].iloc[0]
        table_rows.append(
            f"| recover035 FULL latent256 (epochs=4480, patience=320) | {r035['auc']:.6f} | {r035['pr_auc']:.6f} | "
    ...
    if longpatience256_available and RUN_ID_LONGPATIENCE256 in main["run_id"].values:
        lp = main[main["run_id"].eq(RUN_ID_LONGPATIENCE256)].iloc[0]
        table_rows.append(
            f"| recover035_longpatience256 (epochs=10000, T0=80, p=560) | {lp['auc']:.6f} | {lp['pr_auc']:.6f} | "
            f"{lp['balanced_accuracy']:.6f} | {lp['sensitivity']:.6f} | "
            f"{lp['specificity']:.6f} | {lp['f1']:.6f} |"
        )
    table_rows.append(
        f"| recover035_latent384 (epochs=10000, T0=80, p=560, latent=384) | {cand['auc']:.6f} | {cand['pr_auc']:.6f} | "
        f"{cand['balanced_accuracy']:.6f} | {cand['sensitivity']:.6f} | "
        f"{cand['specificity']:.6f} | {cand['f1']:.6f} |"
    )

    lines = [
        "# recover035_latent384_T80_h10000_p560 FULL 5x5 post-mortem",
        "",
        "Read-only comparison of the latent384 candidate vs three references.",
        "",
        f"Primary readout: classifier-only `{PRIMARY_MODEL}` with `{PRIMARY_THRESHOLD}`.",
        "",
        ...
        *table_rows,
        ...
        "## Context",
        "",
        "recover035_latent384 controlled change vs recover035_longpatience_T80_h10000_p560_full5x5:",
        "  - latent_dim: 256 → 384  (the ONLY change)",
        "Unchanged: epochs_vae=10000, cycles=125, T0=80, patience=560, beta=2.5,",
        "  channels=[1,0,2], metadata_path=patched_metadata_candidate.csv,",
        "  recon_loss=mse_sum_batchmean_current, dropout=0.15, batch_size=64.",
        "128_S_2002 remains excluded from both VAE and classifier pools.",
        "Stage B C-grid unchanged: original [1e-3, 1e-2, 1e-1, 1.0].",
        "",
        "## Decision",
        "",
        "See latent384_recommendation.md for promotion gate outcome.",
        f"If AUC > {LOCKED_AUC} AND PR-AUC >= {LOCKED_PR_AUC} both met: consider promoting as revised manuscript model.",
        "Otherwise: evaluate whether the capacity increase provides any benefit relative to the added parameters.",
        "If no AUC/PR-AUC gain: latent_dim=256 (longpatience or locked) remains preferred for parsimony.",
    ]
...
def main() -> int:
    ...
    longpatience256_run = resolve(args.reference_longpatience256_run_dir)
    longpatience256_readout = resolve(args.reference_longpatience256_readout_dir)
    ...
    longpatience256_ok = validate_readout("longpatience256 reference", longpatience256_readout, required=False)
    ...
    print(f"longpatience256 reference readout   : {longpatience256_ok} ({longpatience256_readout})")
    ...
    print(f"Primary isolation comparison        : latent384 vs longpatience256 (same sched, diff latent_dim only)")
    if not longpatience256_ok:
        print("  WARNING: longpatience256 readout not available — latent capacity isolation table will be omitted.")
    if not recover035_ok:
        print("  WARNING: recover035_full5x5 readout not available — full-context comparison will be omitted.")
    if args.dry_run:
    ...
    if longpatience256_ok:
        active_runs.append((RUN_ID_LONGPATIENCE256, LABEL_LONGPATIENCE256, longpatience256_readout))
    if recover035_ok:
    ...
    main_df = add_deltas_vs_longpatience256(main_df)
    ...
    if longpatience256_ok:
        s = scanner_leakage(RUN_ID_LONGPATIENCE256, LABEL_LONGPATIENCE256, longpatience256_run)
        if not s.empty:
            scanner_parts.append(s)
    if recover035_ok:
    ...
    write_recommendation(
        outdir, main_df, focus, scanner, fold035,
        longpatience256_available=longpatience256_ok,
        recover035_available=recover035_ok,
    )
    write_final_postmortem(
        outdir, main_df, scanner,
        longpatience256_available=longpatience256_ok,
        recover035_available=recover035_ok,
    )
    ...
    command_log = {
        ...
        "reference_longpatience256_run_dir": str(longpatience256_run),
        "reference_longpatience256_readout_dir": str(longpatience256_readout),
        "reference_recover035_run_dir": str(recover035_run),
        "reference_recover035_readout_dir": str(recover035_readout),
        "longpatience256_reference_available": bool(longpatience256_ok),
        ...
        "primary_isolation": "latent384 vs longpatience256 (latent_dim 256 -> 384, all else equal)",
