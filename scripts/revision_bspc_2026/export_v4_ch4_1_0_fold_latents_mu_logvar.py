#!/usr/bin/env python3
"""Export fold-wise VAE posterior parameters for the completed V4 [4,1,0] run.

This is inference-only. It loads saved fold checkpoints, saved fold splits, the
existing ADNI tensor/metadata, and writes posterior summaries to a new AUC
sprint directory. It does not train or update any model.
"""
...
def load_config(run_dir: Path, fallback: Path) -> Dict[str, Any]:
    ...
    if run_config_path.exists():
        ...
        return {
            ...
            "classifier_stratify_cols": args.get("classifier_stratify_cols", ["Sex"]),
        }
    ...
    return {
        ...
        "classifier_stratify_cols": params.get("classifier_stratify_cols", ["Sex"]),
    }
...
def prepare_output_dir(path: Path, overwrite: bool) -> Path:
    ...
    generated_patterns = [
        "fold_*_trainDev_latents_mu_logvar.csv",
        "fold_*_test_latents_mu_logvar.csv",
    ...
    if existing and not overwrite:
        raise FileExistsError(f"{path} already contains generated exports; pass --overwrite")
    if overwrite:
...
def posterior_feature_frame(mu: np.ndarray, logvar: np.ndarray) -> pd.DataFrame:
    if mu.shape != logvar.shape:
        raise ValueError(f"mu/logvar shape mismatch: {mu.shape} vs {logvar.shape}")
    std = np.exp(0.5 * logvar)
    entropy_per_dim = 0.5 * (np.log(2.0 * np.pi * np.e) + logvar)
    entropy = entropy_per_dim.sum(axis=1)

    pieces = [
        pd.DataFrame(mu, columns=[f"mu_{i}" for i in range(mu.shape[1])]),
        pd.DataFrame(logvar, columns=[f"logvar_{i}" for i in range(logvar.shape[1])]),
        pd.DataFrame(std, columns=[f"posterior_std_{i}" for i in range(std.shape[1])]),
    ]
    out = pd.concat(pieces, axis=1)
    out["posterior_entropy"] = entropy
    out["posterior_entropy_mean"] = entropy_per_dim.mean(axis=1)
    out["posterior_entropy_std"] = entropy_per_dim.std(axis=1)
    out["posterior_std_mean"] = std.mean(axis=1)
    out["posterior_std_std"] = std.std(axis=1)
    out["posterior_std_min"] = std.min(axis=1)
    out["posterior_std_max"] = std.max(axis=1)
    out["logvar_mean"] = logvar.mean(axis=1)
    out["logvar_std"] = logvar.std(axis=1)
    out["logvar_min"] = logvar.min(axis=1)
    out["logvar_max"] = logvar.max(axis=1)
    return out


def merge_subject_metadata(subjects: pd.DataFrame, metadata: pd.DataFrame) -> pd.DataFrame:
    base = subjects.copy()
    if "SubjectID" not in base.columns:
        raise ValueError("Fold subjects file is missing SubjectID")
    base["SubjectID"] = base["SubjectID"].astype(str)
    meta_cols = [c for c in ["SubjectID", "ResearchGroup_Mapped", "Age", "Sex", "Manufacturer"] if c in metadata.columns]
    if "SubjectID" not in meta_cols:
        raise ValueError("Metadata must contain SubjectID")
    merged = base.merge(
        metadata[meta_cols].drop_duplicates("SubjectID"),
        on="SubjectID",
        how="left",
        suffixes=("", "_meta"),
    )
...
def subject_frame(
    ...
    if base["y"].isna().any():
        bad = base.loc[base["y"].isna(), ["SubjectID", "ResearchGroup_Mapped"]].to_dict("records")
        raise ValueError(f"Fold {fold} {split} contains non-CN/AD labels: {bad[:5]}")
    base["fold"] = fold
    base["split"] = split
    posterior_df = posterior_feature_frame(mu, logvar)
    out = pd.concat([base.reset_index(drop=True), posterior_df], axis=1)
    first_cols = [
        "SubjectID",
        "tensor_idx",
        "y",
    ...
    posterior_cols = posterior_df.columns.tolist()
    ordered = [c for c in first_cols if c in out.columns] + posterior_cols
    return out[ordered]


def load_norm_params(fold_dir: Path, cfg: Dict[str, Any], tensor: np.ndarray, selected_channel_names: List[str]) -> List[Dict[str, Any]]:
    saved_path = fold_dir / "vae_norm_params.joblib"
    if saved_path.exists():
        params = joblib.load(saved_path)
        if not isinstance(params, list):
            raise TypeError(f"Unexpected normalization params type in {saved_path}: {type(params)}")
        return params

    pool_idx_path = fold_dir / "vae_training_pool_tensor_idx.npy"
    train_local_idx_path = fold_dir / "vae_actual_train_idx_local_to_pool.npy"
    require_files([pool_idx_path, train_local_idx_path])
    vae_pool_idx = np.load(pool_idx_path).astype(int)
    vae_train_local_idx = np.load(train_local_idx_path).astype(int)
...
def main() -> int:
    ...
    checkpoint_paths = [run_dir / f"fold_{fold}" / f"vae_model_fold_{fold}.pt" for fold in range(1, 6)]
    require_files([tensor_path, metadata_path, *checkpoint_paths])
    ...
    selected_channel_names = cfg.get("selected_channel_names") or [tensor_info["channel_names"][i] for i in channels]

    if tensor.ndim != 4 or tensor.shape[1] != len(channels):
        raise ValueError(f"Unexpected selected tensor shape: {tensor.shape}")
    if tensor_info["subject_ids"] is not None and len(tensor_info["subject_ids"]) != tensor.shape[0]:
    ...
    manifest: Dict[str, Any] = {
        ...
        "latent_dim": int(cfg["latent_dim"]),
        "beta_vae": float(cfg["beta_vae"]),
        "device": str(device),
        "classifier_stratification_preserved_from_saved_folds": cfg.get("classifier_stratify_cols"),
        "posterior_entropy_units": "nats",
        "vae_retrained": False,
    ...
    for fold in range(1, 6):
        ...
        require_files([train_subjects_path, test_subjects_path, checkpoint_path])
        ...
        norm_params = load_norm_params(fold_dir, cfg, tensor, list(selected_channel_names))
        train_idx = train_subjects["tensor_idx"].astype(int).to_numpy()
        test_idx = test_subjects["tensor_idx"].astype(int).to_numpy()
        train_norm = apply_normalization_params(tensor[train_idx], norm_params)
        test_norm = apply_normalization_params(tensor[test_idx], norm_params)
        ...
        mu_train, logvar_train = encode_posterior(model, train_norm, args.batch_size, device)
        mu_test, logvar_test = encode_posterior(model, test_norm, args.batch_size, device)

        train_out = subject_frame(train_subjects, metadata, mu_train, logvar_train, fold, "trainDev")
        test_out = subject_frame(test_subjects, metadata, mu_test, logvar_test, fold, "test")
        train_out.to_csv(outdir / f"fold_{fold}_trainDev_latents_mu_logvar.csv", index=False)
        test_out.to_csv(outdir / f"fold_{fold}_test_latents_mu_logvar.csv", index=False)

        index_cols = ["SubjectID", "tensor_idx", "y", "ResearchGroup_Mapped", "Age", "Sex", "Manufacturer", "fold", "split"]
        ...
        manifest["folds"].append(
            {
                ...
                "normalization_params": str(fold_dir / "vae_norm_params.joblib"),
                "n_trainDev": int(train_out.shape[0]),
                "n_test": int(test_out.shape[0]),
                "latent_dim": int(mu_train.shape[1]),
                "mean_posterior_entropy_trainDev": float(train_out["posterior_entropy"].mean()),
                "mean_posterior_entropy_test": float(test_out["posterior_entropy"].mean()),
            }
        )

        del model, train_norm, test_norm
        if device.type == "cuda":
            torch.cuda.empty_cache()

    (outdir / "latent_export_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    readme = [
        "# V4 [4,1,0] Mu/Logvar Fold Latent Exports",
        "",
        "Read-only export from the completed ADNI V4 beta=2.5 CVAE checkpoints.",
        "",
        ...
        "- Splits: saved original outer folds, with the original Sex-stratified setup preserved.",
        "- Exported features: `mu_*`, `logvar_*`, `posterior_std_*`, posterior entropy, and posterior summary columns.",
        "- Manufacturer is exported for diagnostics only and is not intended as a predictive feature.",
        "- No VAE training was run and no architecture was modified.",
    ]
    (outdir / "README.md").write_text("\n".join(readme) + "\n", encoding="utf-8")
    print(json.dumps({"output_dir": str(outdir), "folds_exported": len(manifest["folds"]), "vae_retrained": False}, indent=2))
    return 0
