#!/usr/bin/env python3
"""Prepare/preflight T160 p1120 controlled FULL branch.

Reference:
  recover035_latent384_beta3p75_T160_h10000_p560_full5x5

Candidate:
  recover035_latent384_beta3p75_T160_h10000_p1120_full5x5

Allowed scientific diff: early_stopping_patience_vae 560 -> 1120.
All other parameters are identical to T160 p560.
...
REFERENCE_CONFIG = (
    PROJECT_ROOT
    / "configs/runs/adni_v5_1c_recover035_latent384_beta3p75_T160_h10000_p560_full5x5.json"
)
PREFLIGHT_ROOT = PROJECT_ROOT / "results/revision_bspc_2026/T160_p1120_controlled_preflight_20260609"

STAGE_B_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/run_v5_1_batch20260514b_mfrsplit_3840_classifier_only_sweep.py"

REFERENCE_RUN_ID = "recover035_latent384_beta3p75_T160_h10000_p560_full5x5"
...
CANDIDATES: Dict[str, Candidate] = {
    "T160_p1120": Candidate(
        variant="T160_p1120",
        run_id="recover035_latent384_beta3p75_T160_h10000_p1120_full5x5",
        config_name="adni_v5_1c_recover035_latent384_beta3p75_T160_h10000_p1120_full5x5.json",
        launcher_name="run_adni_v5_1c_recover035_latent384_beta3p75_T160_h10000_p1120_full5x5.py",
        allowed_parameter_diffs={"early_stopping_patience_vae": (560, 1120)},
        scientific_note=(
            "Extended patience sensitivity: p1120 = 7.0 LR-cycle patience (T0=160), "
            "matching the promoted T80 p560 LR-cycle patience. The only allowed diff from "
            "T160 p560 is early_stopping_patience_vae 560 -> 1120."
        ),
    ),
}
...
def make_candidate_config(reference: Dict[str, Any], candidate: Candidate) -> Dict[str, Any]:
    ...
    cfg["description"] = (
        ...
        f"Allowed scientific diff: {dict(candidate.allowed_parameter_diffs)}. {candidate.scientific_note} "
        "All tensor, metadata, channel, architecture, dropout, optimizer, fold, classifier, and "
        "readout conventions otherwise match the T160 p560 reference. Prepared as "
...
def validate_commands(config: Dict[str, Any], candidate: Candidate, stage_a: Sequence[str], stage_b: Sequence[str]) -> pd.DataFrame:
    expected_beta = str(config["parameters"]["beta_vae"])
    expected_t0 = str(config["parameters"]["lr_scheduler_T0"])
    expected_patience = str(config["parameters"]["early_stopping_patience_vae"])
    checks = [
        ...
        ("stage_a_patience", values_after_flag(stage_a, "--early_stopping_patience_vae"), [expected_patience]),
...
def command_rows(config: Dict[str, Any], candidate: Candidate) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    return commands, validate_commands(config, candidate, stage_a, stage_b)
...
def write_package(
        ...
        [
            {
                ...
                "patience_reference": reference["parameters"]["early_stopping_patience_vae"],
                "patience_candidate": load_json(c.config_path)["parameters"]["early_stopping_patience_vae"],
    ...
    (out_dir / "T160_p1120_patience_extension_note.md").write_text(
        "\n".join(
            [
                "# T160 p1120 Patience Extension Note",
                "",
                "Reference: `recover035_latent384_beta3p75_T160_h10000_p560_full5x5`",
                "Candidate: `recover035_latent384_beta3p75_T160_h10000_p1120_full5x5`",
                "",
                "## Rationale",
                "",
                "T160 uses lr_scheduler_T0=160, so patience=560 corresponds to 3.5 LR-cycle lengths",
                "(560 / 160 = 3.5). The promoted T80 reference uses patience=560 / T0=80 = 7.0 LR cycles.",
                "",
                "p1120 = 1120 / 160 = 7.0 LR-cycle patience, matching the promoted reference in",
                "LR-cycle units. Beta-cycle patience is also 1120 / 80 = 14.0 beta cycles.",
                "",
                "## Scientific diff from T160 p560",
                "",
                "- early_stopping_patience_vae: 560 -> 1120",
                "- All other parameters unchanged.",
                "",
                "## Maturity audit context",
                "",
                "The VAE training curve maturity audit (2026-06-09) found that T160 p560 converged",
                "with mean patience-%gap=0.015% and 4/5 folds showing decreasing restart-level minima,",
                "and concluded NO p1120 rerun was warranted on convergence grounds.",
                "This preflight is prepared for completeness and record-keeping only.",
                "Launching this run requires an explicit --confirm-training override.",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    readme = [
        "# T160 p1120 Controlled Preflight",
        "",
        "Date: 2026-06-09",
        "Package: `T160_p1120_controlled_preflight_20260609`",
        "",
        "## Purpose",
        "",
        "Preflight for `recover035_latent384_beta3p75_T160_h10000_p1120_full5x5`.",
        "Base: `recover035_latent384_beta3p75_T160_h10000_p560_full5x5`.",
        "Allowed scientific diff: early_stopping_patience_vae 560 -> 1120 only.",
        "",
        "## Fixed convention (matches T160 p560 exactly)",
        "",
        "- channels [1,0,2] = Pearson Full, OMST, MI.",
        "- latent_dim 384, epochs 10000, beta cycles 125, dropout 0.15 legacy_all.",
        "- convtranspose decoder, legacy_act_norm block order, mse_sum_batchmean_current reconstruction loss.",
        "- lr_scheduler_T0 160, beta_vae 3.75, seed 42.",
        "- classifier readout plan: logreg_l2, z_plus_age_sex.",
        "- same tensor and patched recover035 metadata paths as T160 p560.",
        "",
        "## Validation Summary",
        "",
        f"- Strict diff guard rows: {len(checks)}; all pass = {bool(checks['pass'].all())}.",
        f"- Stale output markers: {int(stale['stale_marker_count'].sum())}.",
        ...
        "## Guardrails",
        "",
        "No real training was performed. No OASIS scoring. No tensor/metadata/model artifact modification.",
    ]
    ...
    write_json(
        out_dir / "command_log.json",
        {
            ...
            "reference_config": rel(REFERENCE_CONFIG),
...
def parse_args(default_variant: str | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--confirm-training", action="store_true", help="Launch real Stage A training after guards pass.")
