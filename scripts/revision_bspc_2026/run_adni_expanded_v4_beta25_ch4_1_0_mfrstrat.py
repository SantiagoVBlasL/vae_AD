#!/usr/bin/env python3
"""Dry-run/run wrapper for ADNI V4 [4,1,0] beta=2.5 with manufacturer-stratified folds."""
...
DEFAULT_CONFIG = PROJECT_ROOT / "configs/runs/adni_expanded_v4_beta25_ch4_1_0_mfrstrat.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build and optionally execute the ADNI V4 [4,1,0] manufacturer-stratified full-run command.",
...
def ensure_output_prepared_for_real_run(config: Dict[str, Any]) -> Path:
    ...
    if any(output_dir.iterdir()):
        raise RuntimeError(f"Refusing to start training because output_dir is not empty: {output_dir}")
    return output_dir
...
def write_manifest(config_path: Path, config: Dict[str, Any], command: List[str]) -> Path:
    ...
    manifest = {
        ...
        "classifier_stratify_cols_extra": params.get("classifier_stratify_cols"),
        "effective_outer_stratification": ["ResearchGroup_Mapped"] + list(params.get("classifier_stratify_cols", [])),
...
def main() -> int:
    ...
    print(f"Architecture unchanged: use_layernorm_vae_fc={params.get('use_layernorm_vae_fc')}, beta={params.get('beta_vae')}, dropout={params.get('dropout_rate_vae')}, final_activation={params.get('vae_final_activation')}")
    print(f"Classifier stratification extras: {params.get('classifier_stratify_cols')}")
    print(f"Effective outer stratification: {['ResearchGroup_Mapped'] + list(params.get('classifier_stratify_cols', []))}")
