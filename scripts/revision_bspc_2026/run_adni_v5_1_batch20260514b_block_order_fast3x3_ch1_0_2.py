#!/usr/bin/env python3
"""Prepare/run FAST 3x3 block-order hygiene pilot for ADNI v5.1 [1,0,2].
...
OUTPUT_ROOT = PROJECT_ROOT / "results/revision_bspc_2026/adni_v5_1_batch20260514b_block_order_fast3x3_ch1_0_2"
BIG_DISK_ROOT = Path("/media/diego/Datos/vae_AD_results/revision_bspc_2026/adni_v5_1_batch20260514b_block_order_fast3x3_ch1_0_2")

CONFIGS = [
    PROJECT_ROOT / "configs/runs/adni_v5_1_batch20260514b_ch1_0_2_fast3x3_block_order_legacy_act_norm.json",
    PROJECT_ROOT / "configs/runs/adni_v5_1_batch20260514b_ch1_0_2_fast3x3_block_order_norm_act.json",
]
...
EXPECTED_RUNS = {
    "legacy_act_norm": "legacy_act_norm",
    "norm_act": "norm_act",
}
...
def validate_config(path: Path, cfg: Dict[str, Any]) -> None:
    ...
    expected_key = "norm_act" if run_name.endswith("norm_act") else "legacy_act_norm"
    expected_block_order = EXPECTED_RUNS[expected_key]
    ...
    require_equal(params["recon_loss_mode"], "mse_sum_batchmean_current", f"{run_name}.recon_loss_mode")
    require_equal(params["vae_final_activation"], "tanh", f"{run_name}.vae_final_activation")
    require_equal(params["vae_dropout_scope"], "legacy_all", f"{run_name}.vae_dropout_scope")
    require_equal(params["vae_block_order"], expected_block_order, f"{run_name}.vae_block_order")
    require_equal(params["vae_train_sampler_strategy"], "none", f"{run_name}.vae_train_sampler_strategy")
...
def validate_stage_a_command(cmd: Sequence[str], cfg: Dict[str, Any]) -> None:
    ...
    for flag, expected in [
        ...
        ("--vae_block_order", [cfg["parameters"]["vae_block_order"]]),
...
def write_readme(output_root: Path, planned: pd.DataFrame, dry_run: bool) -> None:
    ...
    lines = [
        "# ADNI v5.1 batch20260514b Block-Order FAST 3x3 Pilot",
        "",
        "Purpose: controlled block-order hygiene pilot for the current primary channel set `[1,0,2]`.",
        "",
        "This preparation does not modify existing paper-ready results, tensors, metadata, or ledger files.",
        "",
        "## Arms",
        "",
        "- `legacy_act_norm`: historical/default order, preserving locked behavior.",
        "- `norm_act`: `Conv/Linear -> Norm -> GELU -> Dropout`.",
        "",
        "## Shared FAST Profile",
        "",
        "- channels: `[1,0,2]`",
...
def main() -> int:
    ...
    command_log = {
        ...
        "planned_runs": planned[["run_name", "vae_block_order", "recon_loss_mode", "vae_final_activation"]].to_dict(orient="records"),
    }
    (output_root / "command_log.json").write_text(json.dumps(command_log, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    print("=== Block-order FAST 3x3 [1,0,2] pilot ===")
    print(f"Mode: {'DRY-RUN' if args.dry_run or not args.confirm_training else 'REAL RUN'}")
    print(planned[["run_name", "channels_to_use", "vae_block_order", "recon_loss_mode", "vae_final_activation", "outer_folds", "inner_folds", "latent_dim", "epochs_vae"]].to_string(index=False))
