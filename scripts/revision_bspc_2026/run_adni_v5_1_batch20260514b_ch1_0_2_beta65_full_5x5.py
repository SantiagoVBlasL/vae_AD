#!/usr/bin/env python3
"""Prepare/run the controlled beta=6.5 FULL 5x5 confirmation.
...
DEFAULT_CONFIG = PROJECT_ROOT / "configs/runs/adni_v5_1_batch20260514b_ch1_0_2_beta65_full_5x5.json"
...
COMPARISON_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/compare_v5_1_batch20260514b_beta65_full_5x5.py"
...
def assert_only_beta65_scientific_diff(source: Dict[str, Any], target: Dict[str, Any]) -> None:
    ...
    require_equal(diffs, {"beta_vae": (2.5, 6.5)}, "scientific parameter diffs")
...
def validate_config(config: Dict[str, Any], source_config: Dict[str, Any]) -> None:
    assert_only_beta65_scientific_diff(source_config, config)
...
OUT_COMPARISON_DIR = PROJECT_ROOT / "results/revision_bspc_2026/adni_v5_1_batch20260514b_ch1_0_2_beta65_full_5x5_comparison"
...
def write_manifest(config_path: Path, config: Dict[str, Any], stage_a: Sequence[str], stage_b: Sequence[str], comparison: Sequence[str]) -> Path:
    ...
    manifest = {
        ...
        "controlled_change": "beta_vae 2.5 -> 6.5",
...
def main() -> int:
    ...
    print(f"Mode           : {mode}")
    print("Controlled diff: beta_vae 2.5 -> 6.5")
