#!/usr/bin/env python3
"""Prepare/run the controlled block-order norm_act FULL 5x5 confirmation.
...
vae_block_order: legacy_act_norm -> norm_act.

Tests block-order hygiene as a single controlled FULL experiment.
...
DEFAULT_CONFIG = PROJECT_ROOT / "configs/runs/adni_v5_1_batch20260514b_ch1_0_2_block_order_norm_act_full_5x5.json"
...
COMPARISON_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/compare_v5_1_batch20260514b_block_order_norm_act_full_5x5.py"
...
def assert_only_norm_act_scientific_diff(source: Dict[str, Any], target: Dict[str, Any]) -> None:
    ...
    require_equal(diffs, {"vae_block_order": ("legacy_act_norm", "norm_act")}, "scientific parameter diffs")
...
def validate_config(config: Dict[str, Any], source_config: Dict[str, Any]) -> None:
    assert_only_norm_act_scientific_diff(source_config, config)
...
OUT_COMPARISON_DIR = PROJECT_ROOT / "results/revision_bspc_2026/adni_v5_1_batch20260514b_ch1_0_2_block_order_norm_act_full_5x5_comparison"
...
def write_manifest(config_path: Path, config: Dict[str, Any], stage_a: Sequence[str], stage_b: Sequence[str], comparison: Sequence[str]) -> Path:
    ...
    manifest = {
        ...
        "controlled_change": "vae_block_order legacy_act_norm -> norm_act",
...
def main() -> int:
    ...
    print(f"Mode           : {mode}")
    print("Controlled diff: vae_block_order legacy_act_norm -> norm_act")
    print("Canonical default/source block_order: legacy_act_norm")
    ...
    print(f"Block order    : {params['vae_block_order']}")
