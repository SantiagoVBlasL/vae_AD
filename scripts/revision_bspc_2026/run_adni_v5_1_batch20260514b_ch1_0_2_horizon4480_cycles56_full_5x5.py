#!/usr/bin/env python3
"""Prepare/run the controlled long-horizon 4480/56 FULL 5x5 confirmation.
...
the only scientific parameter differences allowed are:
epochs_vae: 3840 -> 4480 and cyclical_beta_n_cycles: 48 -> 56.

Tests longer training horizon as a single controlled FULL experiment while
preserving the 80-epoch beta/LR cycle length.
...
DEFAULT_CONFIG = PROJECT_ROOT / "configs/runs/adni_v5_1_batch20260514b_ch1_0_2_horizon4480_cycles56_full_5x5.json"
...
def assert_only_horizon_scientific_diff(source: Dict[str, Any], target: Dict[str, Any]) -> None:
    ...
    require_equal(
        diffs,
        {
            "epochs_vae": (3840, 4480),
            "cyclical_beta_n_cycles": (48, 56),
        },
...
def validate_config(config: Dict[str, Any], source_config: Dict[str, Any]) -> None:
    assert_only_horizon_scientific_diff(source_config, config)
...
def write_manifest(config_path: Path, config: Dict[str, Any], stage_a: Sequence[str], stage_b: Sequence[str], comparison: Sequence[str]) -> Path:
    ...
    manifest = {
        ...
        "controlled_change": "epochs_vae 3840 -> 4480; cyclical_beta_n_cycles 48 -> 56",
...
def main() -> int:
    ...
    print(f"Mode           : {mode}")
    print("Controlled diff: epochs_vae 3840 -> 4480; cyclical_beta_n_cycles 48 -> 56")
    print("Cycle length    : 4480/56 = 80 epochs; lr_scheduler_T0=80")
