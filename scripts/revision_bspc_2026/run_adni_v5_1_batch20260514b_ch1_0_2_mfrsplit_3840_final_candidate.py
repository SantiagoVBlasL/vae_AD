#!/usr/bin/env python3
"""Dry-run / run wrapper for ADNI v5.1 batch20260514b manufacturer-aware 3840-epoch experiment.

This wrapper prepares a final-candidate experiment but refuses to train unless
--confirm-training is explicitly passed. In dry-run mode it validates the config,
tensor/metadata, scheduler arithmetic, and writes a lightweight split preview.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "manufacturer-aware 3840-epoch training command for channels [1,0,2]."
        ),
...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    require_equal(params.get("classifier_calibrate"), True, "classifier_calibrate")
    ...
    require_equal(params.get("n_iter_logreg"), 300, "n_iter_logreg")
    require_equal(params.get("n_iter_svm"), 300, "n_iter_svm")
...
def inspect_tensor_npz(path: Path) -> Dict[str, Any]:
    ...
    require_equal(selected, EXPECTED_CHANNEL_NAMES, "tensor selected channel names for [1,0,2]")
...
def write_manifest(config_path: Path, config: Dict[str, Any], command: List[str], preview_summary: pd.DataFrame) -> Path:
    output_dir = resolve_path(config["paths"]["output_dir"])
...
def main() -> int:
    ...
    print(f"\nTrial budgets  : logreg={params.get('n_iter_logreg')}, svm={params.get('n_iter_svm')}")
    print("Command:")
    ...
    manifest_path = write_manifest(args.config, config, command, preview_summary)
