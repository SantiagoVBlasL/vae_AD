#!/usr/bin/env python3
"""Prepare/run the controlled no-decoder-dropout FULL 5x5 confirmation.
...
vae_dropout_scope: legacy_all -> no_decoder_dropout.
"""
...
DEFAULT_CONFIG = PROJECT_ROOT / "configs/runs/adni_v5_1_batch20260514b_ch1_0_2_no_decoder_dropout_full_5x5.json"
...
COMPARISON_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/compare_v5_1_batch20260514b_no_decoder_dropout_full_5x5.py"
...
def assert_only_dropout_scientific_diff(source: Dict[str, Any], target: Dict[str, Any]) -> None:
    src_params = dict(source["parameters"])
    tgt_params = dict(target["parameters"])
    src_params.setdefault("vae_dropout_scope", "legacy_all")
    ...
    require_equal(diffs, {"vae_dropout_scope": ("legacy_all", "no_decoder_dropout")}, "scientific parameter diffs")

    # Non-scientific fields that must change to avoid overwriting the locked run.
...
def validate_config(config: Dict[str, Any], source_config: Dict[str, Any]) -> None:
    assert_only_dropout_scientific_diff(source_config, config)
    ...
    for key, expected in [
        ...
        ("vae_dropout_scope", "no_decoder_dropout"),
        ("batch_size", 64),
...
OUT_COMPARISON_DIR = PROJECT_ROOT / "results/revision_bspc_2026/adni_v5_1_batch20260514b_ch1_0_2_no_decoder_dropout_full_5x5_comparison"
...
def write_manifest(config_path: Path, config: Dict[str, Any], stage_a: Sequence[str], stage_b: Sequence[str], comparison: Sequence[str]) -> Path:
    ...
    manifest = {
        ...
        "controlled_change": "vae_dropout_scope legacy_all -> no_decoder_dropout",
...
def main() -> int:
    ...
    print(f"Mode           : {mode}")
    print("Controlled diff: vae_dropout_scope legacy_all -> no_decoder_dropout")
