#!/usr/bin/env python3
"""Prepare/run the controlled exploratory recover035 FULL 5x5 metadata-rescue retrain.

Default behavior is preflight only.  Real training requires --confirm-training.

The only intended scientific change vs the locked v5.1b (horizon4480/cycles56) is
the metadata_path: the branch-local patched_metadata_candidate.csv adds 035_S_6927
as an AD subject (Age=59.6, Sex=F, Manufacturer=SIEMENS, tensor_index=256).

All VAE/classifier parameters are IDENTICAL to the locked model.
Promotion rule: candidate must beat BOTH pooled OOF AUC > 0.782951
AND PR-AUC >= 0.559873 simultaneously.
"""
...
COMPARISON_SCRIPT = (
    PROJECT_ROOT / "scripts/revision_bspc_2026/compare_v5_1_batch20260514b_recover035_full5x5.py"
)
INTEGRITY_AUDIT_SCRIPT = (
    PROJECT_ROOT / "scripts/revision_bspc_2026/audit_v5_1_batch20260514b_recover035_integrity.py"
)
POOL_AUDIT_SCRIPT = (
    PROJECT_ROOT / "scripts/revision_bspc_2026/audit_adni_recover035_subject_pool.py"
)
SPLIT_PREVIEW_SCRIPT = (
    PROJECT_ROOT / "scripts/revision_bspc_2026/preview_recover035_foldwise_splits.py"
)
OUT_COMPARISON_DIR = PROJECT_ROOT / "results/revision_bspc_2026/recover035_full5x5_comparison"
...
def assert_only_recover035_scientific_diff(
    source: Dict[str, Any], target: Dict[str, Any]
) -> None:
    """Verify all VAE/classifier parameters are IDENTICAL to locked v5.1b.

    The only allowed diff is metadata_path (path, not a parameter).
    global_tensor_path and channels_to_use must be unchanged.
    """
    ...
    if diffs:
        raise RuntimeError(
            f"recover035 must have ZERO parameter diffs vs locked source config, "
            f"but found: {diffs}"
        )
    ...
    # Metadata path must have changed to the patched CSV
    ...
    if ORIGINAL_METADATA_SUFFIX in tgt_meta:
        raise RuntimeError(
            f"metadata_path still points to original training_ready_metadata; "
            f"expected patched_metadata_candidate.csv"
        )


def validate_config(config: Dict[str, Any], source_config: Dict[str, Any]) -> None:
    assert_only_recover035_scientific_diff(source_config, config)
    ...
    if params["epochs_vae"] / params["cyclical_beta_n_cycles"] != 80:
        raise RuntimeError("Expected 4480/56 = 80 cycle length.")
...
def audit_rescue_pool(meta: pd.DataFrame) -> None:
    """Verify 035 is present and 128 is absent; check pool sizes."""
    ...
    # Check 128_S_2002 absent from classifier pool
...
def inspect_tensor(path: Path) -> Dict[str, Any]:
    ...
    # Verify 035 and 128 tensor indices
    with np.load(path, allow_pickle=False) as zf:
...
def make_split_preview(meta: pd.DataFrame, config: Dict[str, Any]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for fold, (train_dev_idx, test_idx) in enumerate(splitter.split(np.zeros(len(cn_ad)), y_outer), start=1):
        ...
        # Guard: 128_S_2002 must not appear in the metadata-based VAE pool. If it does,
        # it means patched_metadata_candidate.csv incorrectly includes it as training-ready.
...
def build_stage_a_command(config: Dict[str, Any], python_exe: str) -> List[str]:
    ...
    # recover035-specific guards (not in config params):
    # exclude tensor subjects with missing metadata from the VAE pool, and abort hard
    # if the internal val split fails rather than silently disabling early stopping.
...
def build_split_preview_command(config: Dict[str, Any], python_exe: str) -> List[str]:
    return [
        python_exe, str(SPLIT_PREVIEW_SCRIPT),
...
def validate_stage_a_command(command: Sequence[str], config: Dict[str, Any]) -> None:
    ...
    # Verify metadata_path points to patched CSV
    ...
    # Verify recover035-specific VAE pool safeguards are present
    ...
    if "--vae_abort_if_val_split_fails" not in command:
        raise RuntimeError(
            ...
            "from tensor subjects absent in the patched metadata (e.g. 128_S_2002)."
        )
...
def write_manifest(
    ...
    manifest = {
        ...
        "controlled_change": "metadata_path only: patched_metadata_candidate.csv adds 035_S_6927 (AD, age=59.6, Sex=F, Manufacturer=SIEMENS). Zero parameter diffs vs locked v5.1b.",
        ...
        "promotion_rule": (
            ...
            "FAST result is NOT sufficient. Exploratory run — splits changed."
        ),
...
def write_command_log(
    ...
    payload = {
        ...
        "controlled_change": "metadata_path only — zero parameter diffs vs locked v5.1b",
        "recover_subject": RECOVER_SUBJECT,
        "excluded_subject": EXCLUDED_SUBJECT,
        "promotion_rule": (
            f"Must beat BOTH AUC > {LOCKED_AUC} AND PR-AUC >= {LOCKED_PR_AUC} simultaneously."
...
def main() -> int:
    ...
    # Report which outer-test fold 035_S_6927 lands in
    ...
    split_preview_cmd = build_split_preview_command(config, python_exe)
    params = config["parameters"]
    output_dir = resolve(config["paths"]["output_dir"])
    stale = stale_output_markers(output_dir)
    ...
    print(f"Mode           : {mode}")
    print(f"Controlled diff: metadata_path only — zero parameter diffs vs locked v5.1b")
    ...
    print(f"Cycle length   : {params['epochs_vae']}/{params['cyclical_beta_n_cycles']} = {params['epochs_vae']/params['cyclical_beta_n_cycles']} epochs; lr_scheduler_T0={params['lr_scheduler_T0']} (identical to locked)")
    ...
    print(f"VAE schedule   : outer={params['outer_folds']}, inner={params['inner_folds']}, latent_dim={params['latent_dim']}, epochs={params['epochs_vae']}, cycles={params['cyclical_beta_n_cycles']}, cycle_len={params['epochs_vae']/params['cyclical_beta_n_cycles']:.0f}, T0={params['lr_scheduler_T0']}, batch_size={params['batch_size']}, dropout={params['dropout_rate_vae']}, beta_max={params['beta_vae']}")
    ...
    print(f"Tensor shape   : {tensor_info['shape']}, n_subjects={tensor_info['n_subjects']}, python_bandpass_applied={tensor_info['python_bandpass_applied']}")
    ...
    print(f"Promotion rule : AUC > {LOCKED_AUC} AND PR-AUC >= {LOCKED_PR_AUC} simultaneously (FULL only; exploratory; FAST not sufficient)")
    ...
    # --- 128_S_2002 / tensor-metadata mismatch guard ---
    print(f"\n[128_S_2002 VAE pool guard]")
    print(f"  The tensor contains n_subjects={tensor_info['n_subjects']} entries (load_data left-joins tensor->metadata).")
    print(f"  {EXCLUDED_SUBJECT} is in the tensor (tensor_index=363) but NOT in patched_metadata_candidate.csv.")
    print(f"  Without a guard, load_data inserts a NaN-metadata row for {EXCLUDED_SUBJECT} into the merged DataFrame,")
    print(f"  which ends up in the VAE pool as a singleton stratum → ValueError in train/val split → silent fallback")
    print(f"  to empty val set → early stopping disabled → run not comparable to locked v5.1b.")
    print(f"  Fix applied in Stage A command:")
    print(f"    --vae_required_metadata_cols ResearchGroup_Mapped Manufacturer Age Sex")
    print(f"      → subjects with missing values for these columns are removed from the VAE pool before splitting.")
    print(f"    --vae_abort_if_val_split_fails")
    print(f"      → if the split fails anyway, abort hard instead of silently continuing with empty val.")
    ...
    print(f"\n[Expected per-fold VAE pool sizes after --vae_required_metadata_cols filtering]")
    print(f"  (Metadata-based preview: all 647 patched-metadata subjects; 128_S_2002 absent)")
    ...
    print("\nSplit preview command:")
    print(shlex.join(split_preview_cmd))
    print("\nClean-run policy:")
    ...
    print("\nRESTART INSTRUCTIONS (if partial Fold 1 artifacts exist from a failed run):")
    print("  1. Verify the partial run is invalid (empty val set, 'best ValL: N/A', or 128_S_2002 in VAE pool).")
    print(f"  2. python {Path(__file__).name} --dry-run   # confirm stale markers detected")
    print(f"  3. python {Path(__file__).name} --force-clean --confirm-training   # quarantine + rerun cleanly")
    print("  --force-clean moves the existing output_dir contents to a timestamped quarantine folder;")
    print("  it does NOT delete them silently.")
    if stale:
