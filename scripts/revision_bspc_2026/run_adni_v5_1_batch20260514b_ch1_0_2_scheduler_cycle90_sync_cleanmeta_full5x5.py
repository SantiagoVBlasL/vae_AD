#!/usr/bin/env python3
"""Prepare/run the controlled scheduler-cycle sensitivity FULL 5x5 — cleanmeta variant.

Scientific diff vs locked v5.1b [1,0,2] horizon4480/cycles56:
  epochs_vae          : 4480 -> 4500
  cyclical_beta_n_cycles: 56  -> 50
  lr_scheduler_T0     : 80   -> 90

This synchronizes the beta-annealing cycle length and the LR cosine warm-restart
period to exactly 90 epochs each (4500/50 = 90, T0 = 90). At epoch 0 both cycles
start at phase 0.0 and restart simultaneously at epochs 0, 90, 180, ..., 4410.

CLEANMETA FIX: The locked metadata (n=646) contains two fewer subjects than the
tensor (n=648). When run_vae_clf_ad_inference.py joins tensor->metadata (LEFT JOIN),
the two unmatched subjects receive NaN metadata rows. Those NaN rows form singleton
strata and cause VAE internal validation split failure (ValueError or silent empty-val
fallback that disables early stopping). This variant injects:
  --vae_required_metadata_cols ResearchGroup_Mapped Manufacturer Age Sex
  --vae_abort_if_val_split_fails
so the two NaN-metadata subjects are excluded from the VAE pool before the split is
attempted, and any remaining split failure aborts hard instead of continuing silently.

Excluded subject : 128_S_2002 (tensor_index=363, diagnosis unavailable)
Recover subject  : 035_S_6927 (tensor_index=256, absent from locked meta, NOT rescued here)
...
DEFAULT_CONFIG = PROJECT_ROOT / "configs/runs/adni_v5_1_batch20260514b_ch1_0_2_scheduler_cycle90_sync_cleanmeta_full5x5.json"
...
COMPARISON_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/compare_v5_1_batch20260514b_scheduler_cycle90_sync_full5x5.py"
INTEGRITY_AUDIT_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/audit_v5_1_batch20260514b_scheduler_cycle90_sync_cleanmeta_integrity.py"
SCHEDULE_PHASE_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/audit_scheduler_cycle90_phase_preflight.py"

OUT_COMPARISON_DIR = PROJECT_ROOT / "results/revision_bspc_2026/scheduler_cycle90_sync_cleanmeta_full5x5_comparison"

CLEANMETA_REQUIRED_COLS = ["ResearchGroup_Mapped", "Manufacturer", "Age", "Sex"]
...
def assert_only_cycle90_scientific_diff(source: Dict[str, Any], target: Dict[str, Any]) -> None:
...
def validate_config(config: Dict[str, Any], source_config: Dict[str, Any]) -> None:
    assert_only_cycle90_scientific_diff(source_config, config)
...
def make_split_preview(meta: pd.DataFrame, config: Dict[str, Any]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    # Cleanmeta guards: neither subject should appear in the locked metadata
    for subj, label in [(EXCLUDED_SUBJECT, "excluded"), (RECOVER_SUBJECT, "recover")]:
        if subj in meta["SubjectID"].values:
            raise RuntimeError(
                f"CLEANMETA GUARD: {label} subject {subj} is present in metadata. "
                "This subject must be absent from the locked metadata (n=646). "
                "Check that metadata_path points to the locked metadata, not a patched version."
            )
    params = config["parameters"]
    ...
    for fold, (train_dev_idx, test_idx) in enumerate(splitter.split(np.zeros(len(cn_ad)), y_outer), start=1):
        ...
        # Cleanmeta per-fold guard: EXCLUDED_SUBJECT must not appear in VAE pool
        excluded_tid = meta.loc[meta["SubjectID"].eq(EXCLUDED_SUBJECT), "tensor_idx"]
        if len(excluded_tid) > 0 and excluded_tid.iloc[0] in vae_pool_idx:
            raise RuntimeError(
                f"CLEANMETA GUARD fold {fold}: {EXCLUDED_SUBJECT} tensor_idx "
                f"{excluded_tid.iloc[0]} found in VAE pool. "
                "Check that metadata_path uses the locked metadata."
            )
...
def build_stage_a_command(config: Dict[str, Any], python_exe: str) -> List[str]:
    ...
    # Cleanmeta flags: exclude NaN-metadata subjects from VAE pool and abort on split failure
    command.append("--vae_required_metadata_cols")
    command.extend(CLEANMETA_REQUIRED_COLS)
...
def validate_stage_a_command(command: Sequence[str]) -> None:
    ...
    # Cleanmeta flag checks
    require_equal(
        values_after_flag(command, "--vae_required_metadata_cols"),
        CLEANMETA_REQUIRED_COLS,
        "Stage A --vae_required_metadata_cols",
    )
    if "--vae_abort_if_val_split_fails" not in command:
        raise RuntimeError(
            "Stage A command is missing --vae_abort_if_val_split_fails. "
            "This flag is required for the cleanmeta variant to abort hard on split failure."
        )
...
def write_manifest(
    ...
    manifest = {
        ...
        "controlled_change": (
            ...
            "cleanmeta fix: --vae_required_metadata_cols ResearchGroup_Mapped Manufacturer Age Sex "
            "--vae_abort_if_val_split_fails"
        ),
        ...
        "cleanmeta_fix_applied": True,
        "excluded_subject": EXCLUDED_SUBJECT,
        "recover_subject_not_in_pool": RECOVER_SUBJECT,
...
def write_command_log(
    ...
    payload = {
        ...
        "controlled_change": (
            "epochs_vae 4480->4500; cyclical_beta_n_cycles 56->50; lr_scheduler_T0 80->90; "
            "synchronized beta/LR cycle length = 90 epochs; "
            "cleanmeta fix: --vae_required_metadata_cols --vae_abort_if_val_split_fails"
        ),
        "cleanmeta_fix_applied": True,
        "excluded_subject": EXCLUDED_SUBJECT,
        "promotion_rule": f"Must beat BOTH AUC > {LOCKED_AUC} AND PR-AUC >= {LOCKED_PR_AUC} simultaneously.",
        "ranking_source": "Stage B classifier-only logreg_l2",
        "primary_threshold_strategy": EXPECTED_PRIMARY_THRESHOLD,
        "threshold_selection": "true_inner_cv_oof",
        "quarantine_dir": str(quarantine_dir) if quarantine_dir else "",
...
def _print_schedule_phase_analysis(params: Dict[str, Any]) -> None:
    ...
    print(f"  complete cycles      : {n_cycles}  (epochs 0 to {n_cycles * int(cycle_len) - 1})")
    print(f"  remainder            : {int(remainder) if remainder == remainder else 'n/a'} epochs  "
          f"[{'PASS (none)' if remainder == 0 else 'WARNING: non-zero remainder'}]")
    print(f"  per-cycle structure  : {ramp_epochs:.0f} ramp + {stable_epochs:.0f} stable = {cycle_len:.0f} epochs "
          f"(ratio_increase={ratio_inc})")
    print(f"  restart epochs       : {', '.join(str(e) for e in restart_epochs)}"
...
def main() -> int:
    ...
    print(f"Mode           : {mode}")
    print("Controlled diff: epochs_vae 4480->4500; cyclical_beta_n_cycles 56->50; lr_scheduler_T0 80->90")
    ...
    print(f"Block order    : {params['vae_block_order']} (locked default; not changed)")
    ...
    print(f"Promotion rule : AUC > {LOCKED_AUC} AND PR-AUC >= {LOCKED_PR_AUC} simultaneously (FULL only; FAST not sufficient)")
    print("\nCLEANMETA FIX:")
    print(f"  Excluded subject   : {EXCLUDED_SUBJECT} (tensor_index=363, diagnosis unavailable, absent from locked meta)")
    print(f"  Recover subject    : {RECOVER_SUBJECT} (tensor_index=256, absent from locked meta, NOT rescued in this run)")
    print(f"  Stage A injection  : --vae_required_metadata_cols {' '.join(CLEANMETA_REQUIRED_COLS)}")
    print("                       --vae_abort_if_val_split_fails")
    print(f"  Metadata n         : {len(meta)} (locked, excludes both missing subjects)")
    print("  Expected VAE pool sizes (cleanmeta = locked meta n=646 minus test CN/AD per fold):")
    vae_pool_rows = preview_summary[preview_summary["split_component"] == "vae_pool"][["fold", "n"]]
    for _, vrow in vae_pool_rows.iterrows():
        print(f"    Fold {int(vrow['fold'])}: VAE pool n={int(vrow['n'])}")
    print("  VAE val non-empty  : [verified by split preview above; --vae_abort_if_val_split_fails guards real run]")
    print(f"  {EXCLUDED_SUBJECT} absent from metadata: PASS")
    print(f"  {RECOVER_SUBJECT} absent from metadata : PASS")
    ...
    manifest = write_manifest(
        args.config, config, stage_a, stage_b, comparison, integrity_audit, schedule_phase,
