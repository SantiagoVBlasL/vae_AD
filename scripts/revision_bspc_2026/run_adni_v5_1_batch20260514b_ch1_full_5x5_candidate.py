#!/usr/bin/env python3
"""Dry-run / run wrapper for ADNI v5.1 batch20260514b channel-[1] full 5x5 experiment.

This wrapper prepares the full 5x5 confirmation run for the FAST-selected
single channel [1]. It refuses to train unless --confirm-training is explicitly
...
DEFAULT_CONFIG = (
    ...
    / "configs"
    / "runs"
    / "adni_v5_1_batch20260514b_ch1_full_5x5_candidate.json"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build and optionally execute the ADNI v5.1 batch20260514b channel-[1] "
...
def inspect_tensor_npz(path: Path) -> Dict[str, Any]:
    ...
    require_equal(selected, EXPECTED_CHANNEL_NAMES, "tensor selected channel names for [1]")
    print(f"[tensor] Path: {path}")
    print(f"[tensor] Subjects={subject_count}, python_bandpass_applied=False")
    print(f"[tensor] Selected [1]: {selected}")
...
def validate_stage_commands(stage_a_command: Sequence[str], stage_b_command: Sequence[str]) -> None:
    ...
    for required in ["--outer_folds", "5", "--inner_folds", "5", "--channels_to_use", "1", "--n_iter_logreg", "1"]:
