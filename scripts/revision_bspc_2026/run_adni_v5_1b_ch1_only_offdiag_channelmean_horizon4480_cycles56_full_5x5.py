#!/usr/bin/env python3
"""Launcher for v5.1b [1] offdiag_channelmean FULL 5x5 confirmation.
...
stay on the v5.1b no_pybandpass branch.
"""
...
DEFAULT_CONFIG = PROJECT_ROOT / "configs/runs/adni_v5_1b_ch1_only_offdiag_channelmean_horizon4480_cycles56_full_5x5.json"
STAGE_B_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/run_v5_1_batch20260514b_mfrsplit_3840_classifier_only_sweep.py"
COMPARISON_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/compare_v5_1b_ch1_only_offdiag_channelmean_vs_final_model.py"
INTEGRITY_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/audit_v5_1b_ch1_only_offdiag_channelmean_integrity.py"
...
def validate_stage_a_command(command: Sequence[str]) -> None:
    ...
    require_equal(values_after_flag(command, "--channels_to_use"), ["1"], "Stage A channels")
...
def write_manifest(config_path: Path, config: Dict[str, Any], stage_a: List[str], stage_b: List[str], comparison: List[str], integrity: List[str], quarantine: Path | None) -> None:
    ...
    payload = {
        ...
        "controlled_change_vs_v5_1b_horizon4480": "channels_to_use [1,0,2]->[1]; recon_loss_mode mse_sum_batchmean_current->offdiag_channelmean_sum",
...
def main() -> int:
    ...
    print("Config diff    : channels_to_use [1,0,2]->[1]; recon_loss_mode mse_sum_batchmean_current->offdiag_channelmean_sum")
    ...
    print("Channels       : [1] Pearson_Full_FisherZ_Signed")
