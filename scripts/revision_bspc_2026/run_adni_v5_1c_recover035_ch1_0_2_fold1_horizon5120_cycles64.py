#!/usr/bin/env python3
"""Prepare/run a v5.1c Fold-1-only horizon extension diagnostic.

Default mode is dry-run/preflight. Real training requires --confirm-training.
The scientific config diff versus the v5.1c horizon4480/cycles56 source run is
restricted to:
  epochs_vae: 4480 -> 5120
  cyclical_beta_n_cycles: 56 -> 64

The actual training command additionally passes --outer_fold_indices_to_run 1
...
LOCKED_CONFIG = PROJECT_ROOT / "configs/runs/adni_v5_1c_recover035_ch1_0_2_horizon4480_cycles56_full_5x5.json"
DEFAULT_CONFIG = PROJECT_ROOT / "configs/runs/adni_v5_1c_recover035_ch1_0_2_fold1_horizon5120_cycles64.json"
TARGET_FOLD = 1
EXPECTED_SCIENTIFIC_DIFFS = {
    "epochs_vae": (4480, 5120),
    "cyclical_beta_n_cycles": (56, 64),
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--confirm-training", action="store_true", help="Required for real Fold-1 training launch.")
...
def main() -> int:
    ...
    print("Controlled scientific diffs vs v5.1c horizon4480/cycles56 source:")
