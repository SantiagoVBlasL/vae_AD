#!/usr/bin/env python3
"""Launcher for v5.1c recover035 horizon4480/cycles56 FULL 5x5.

Default is dry-run/preflight behavior. Real training requires
--confirm-training. Compared with the v5.1b horizon4480/cycles56 run, the only
allowed differences are dataset paths and output paths.
"""
...
DEFAULT_CONFIG = PROJECT_ROOT / "configs/runs/adni_v5_1c_recover035_ch1_0_2_horizon4480_cycles56_full_5x5.json"
STAGE_B_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/run_v5_1_batch20260514b_mfrsplit_3840_classifier_only_sweep.py"
COMPARISON_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/compare_v5_1c_recover035_vs_v5_1b_horizon4480.py"
...
def validate_dataset_only_diff(source: Dict[str, Any], target: Dict[str, Any]) -> None:
    require_equal(source["parameters"], target["parameters"], "parameters")
...
def validate_config(config: Dict[str, Any], source: Dict[str, Any]) -> None:
    validate_dataset_only_diff(source, config)
...
def write_manifest(config_path: Path, config: Dict[str, Any], stage_a: List[str], stage_b: List[str], comparison: List[str], quarantine: Path | None) -> None:
    outdir = resolve(config["paths"]["output_dir"])
    payload = {
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "run_name": config["run_name"],
        "config_path": str(config_path),
        "controlled_change_vs_v5_1b_horizon4480": "dataset_only: recover 035_S_6927 and remove 128_S_2002 in clean v5.1c branch",
...
def write_command_log(config: Dict[str, Any], stage_a_rc: int | None, stage_b_rc: int | None, comparison_rc: int | None, quarantine: Path | None) -> None:
    ...
    payload = {
        ...
        "controlled_change_vs_v5_1b_horizon4480": "dataset_only",
        "quarantine_dir": str(quarantine) if quarantine else "",
...
def main() -> int:
    ...
    print("Config diff    : dataset paths/output paths only vs v5.1b horizon4480/cycles56")
    ...
    run_start = time.time()
    write_manifest(args.config, config, stage_a, stage_b, comparison, quarantine)
