#!/usr/bin/env python3
"""Launcher for v5.1c recover035 objective-v2 horizon10000/cycles125 FULL 5x5.

Default is dry-run/preflight behavior. Real training requires
--confirm-training. Compared with the v5.1c horizon10000/cycles125 current-loss
run, the only allowed scientific difference is:
  recon_loss_mode: mse_sum_batchmean_current -> offdiag_channelmean_sum

Output paths may differ. Tensor/metadata paths must stay on the v5.1c branch.
"""
...
SOURCE_CONFIG = PROJECT_ROOT / "configs/runs/adni_v5_1c_recover035_ch1_0_2_horizon10000_cycles125_full_5x5.json"
DEFAULT_CONFIG = PROJECT_ROOT / "configs/runs/adni_v5_1c_recover035_ch1_0_2_objective_v2_offdiag_channelmean_horizon10000_cycles125_full_5x5.json"
STAGE_B_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/run_v5_1_batch20260514b_mfrsplit_3840_classifier_only_sweep.py"
COMPARISON_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/compare_v5_1c_objective_v2_offdiag_channelmean_horizon10000_vs_references.py"
INTEGRITY_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/audit_v5_1c_objective_v2_offdiag_channelmean_horizon10000_integrity.py"
...
def validate_objective_v2_diff(source: Dict[str, Any], target: Dict[str, Any]) -> None:
    ...
    if param_diffs != allowed_param_diffs:
        raise RuntimeError(f"Unexpected parameter diffs vs v5.1c horizon10000 current-loss: {param_diffs}")
    require_equal(source["selected_channel_names"], target["selected_channel_names"], "selected_channel_names")
    ...
    if diffs != allowed_path_diffs:
        raise RuntimeError(f"Unexpected path diffs vs v5.1c horizon10000 current-loss: {sorted(diffs)}")
    if "v5_1c_recover035" not in target_paths["global_tensor_path"]:
        raise RuntimeError("Target tensor path does not point to v5.1c recover035 dataset")
    if "v5_1c_recover035" not in target_paths["metadata_path"]:
        raise RuntimeError("Target metadata path does not point to v5.1c recover035 dataset")


def validate_config(config: Dict[str, Any], source: Dict[str, Any]) -> None:
    validate_objective_v2_diff(source, config)
...
def inspect_tensor(path: Path) -> Dict[str, Any]:
    ...
    if "035_S_6927" not in ids or "128_S_2002" in ids:
        raise RuntimeError("v5.1c tensor must include 035_S_6927 and exclude 128_S_2002")
    require_equal([channels[i] for i in EXPECTED_CHANNELS], EXPECTED_SELECTED_NAMES, "selected tensor channel names")
    return {"shape": shape, "python_bandpass_applied": bandpass, "n_subjects": len(ids)}
...
def load_metadata(path: Path) -> pd.DataFrame:
    ...
    if "035_S_6927" not in set(df["SubjectID"]):
        raise RuntimeError("v5.1c metadata must include 035_S_6927")
    if "128_S_2002" in set(df["SubjectID"]):
        raise RuntimeError("v5.1c metadata must exclude 128_S_2002")
    return df
        ...
        rows.append(
            {
                ...
                "contains_035_S_6927": "035_S_6927" in set(sub["SubjectID"]),
            }
        )
...
def validate_stage_a_command(command: Sequence[str]) -> None:
    ...
    require_equal(values_after_flag(command, "--epochs_vae"), ["10000"], "Stage A epochs_vae")
    require_equal(values_after_flag(command, "--cyclical_beta_n_cycles"), ["125"], "Stage A cycles")
...
def ensure_output_prepared(config: Dict[str, Any], force_clean: bool) -> Path | None:
    ...
    if output_dir.resolve() != big_disk.resolve():
        raise RuntimeError(f"output_dir symlink target mismatch: {output_dir.resolve()} != {big_disk.resolve()}")
...
def write_manifest(
    ...
    payload = {
        ...
        "controlled_change_vs_v5_1c_horizon10000_current_loss": "recon_loss_mode mse_sum_batchmean_current->offdiag_channelmean_sum",
...
def write_command_log(config: Dict[str, Any], stage_a_rc: int | None, stage_b_rc: int | None, comparison_rc: int | None, quarantine: Path | None) -> None:
    ...
    payload = {
        ...
        "v5_1b_files_modified": False,
        "v5_1c_existing_files_modified": False,
    }
    (outdir / "command_log.json").write_text(json.dumps(payload, indent=2))
...
def main() -> int:
    ...
    print("Config diff    : recon_loss_mode mse_sum_batchmean_current->offdiag_channelmean_sum only vs v5.1c horizon10000 current-loss")
    ...
    print("Channels       : [1,0,2]")
    print("VAE schedule   : 5x5, latent_dim=256, epochs=10000, cycles=125, cycle_len=80, T0=80")
