#!/usr/bin/env python3
"""FULL 5x5 launcher — recover035 latent128 beta1.25 locked schedule.

Base: recover035_latent128_beta2p5_lockedSchedule_full5x5
      (patched_metadata_candidate.csv; 035_S_6927 as AD,
      128_S_2002 excluded; latent_dim=128, beta_vae=2.5, epochs_vae=4480,
      cyclical_beta_n_cycles=56, lr_scheduler_T0=80, patience=320)

Controlled change vs latent128 beta2.5 (1 parameter diff):
  beta_vae : 2.5 -> 1.25

All other parameters unchanged:
  channels_to_use=[1,0,2], latent_dim=128, epochs_vae=4480, cyclical_beta_n_cycles=56,
  ...
  metadata_path=patched_metadata_candidate.csv (same as latent128 beta2.5).
...
SOURCE_CONFIG = (
    PROJECT_ROOT / "configs/runs/adni_v5_1c_recover035_latent128_beta2p5_lockedSchedule_full5x5.json"
)
DEFAULT_CONFIG = (
    PROJECT_ROOT
    / "configs/runs/adni_v5_1c_recover035_latent128_beta1p25_lockedSchedule_full5x5.json"
)
...
# Single controlled change vs recover035_latent128_beta2p5_lockedSchedule_full5x5
EXPECTED_LATENT_DIM = 128
BASE_LATENT_DIM = 128
BASE_BETA = 2.5
# Locked schedule — unchanged from latent128 beta2.5
...
def assert_latent128_diff(source: Dict[str, Any], target: Dict[str, Any]) -> None:
    """Verify exactly 1 parameter diff vs latent128 beta2.5.

    Allowed diff:
      beta_vae : 2.5 -> 1.25

    Everything else must be identical: channels_to_use, latent_dim, epochs_vae,
    ...
    if set(src_params) != set(tgt_params):
        raise RuntimeError(
            "Target config parameter keys differ from source (latent128 beta2.5): "
    ...
    require_equal(
        ...
        "scientific parameter diffs (beta1.25 vs latent128 beta2.5)",
    )
    if source["paths"]["global_tensor_path"] != target["paths"]["global_tensor_path"]:
        raise RuntimeError("global_tensor_path changed; this is not allowed for a beta-scaling run.")
    ...
    if src_meta != tgt_meta:
        raise RuntimeError(
            f"metadata_path changed vs latent128 beta2.5 ({src_meta!r} -> {tgt_meta!r}). "
            "beta1.25 must use the same patched_metadata_candidate.csv."
        )
...
def _validate_stage_b_raw_command(command: Sequence[str]) -> None:
    ...
    require_equal(values_after_flag(command, "--readout-feature-sets"), ["z_plus_age_sex"], "Stage B readout feature set")
...
def write_manifest(
    ...
    manifest = {
        ...
        "controlled_change": (
            ...
            "All other parameters unchanged: latent_dim=128, epochs_vae=4480, n_cycles=56, "
            "cycle_len=80, T0=80, patience=320 (4 cycles), channels=[1,0,2]."
        ),
        "beta1p25_diff_vs_latent128_beta2p5_verified": True,
...
def main() -> int:
    ...
    print(f"Source config    : {args.source_config} (recover035 latent128 beta2.5)")
    print(f"Run name         : {config['run_name']}")
    print(f"Mode             : {mode}")
    print(f"Controlled diff  : 1 parameter diff vs latent128 beta2.5 (beta_vae {BASE_BETA}->{EXPECTED_BETA})")
    ...
    print(f"beta_vae         : {BASE_BETA} -> {EXPECTED_BETA} (only scientific change)")
