#!/usr/bin/env python3
"""Prepare/run the controlled exploratory recover035_latent384_T80_h10000_p560 FULL 5x5.
...
Base model: recover035_longpatience_T80_h10000_p560_full5x5
  (patched_metadata_candidate.csv; 035_S_6927 as AD, 128_S_2002 excluded;
   ...
   early_stopping_patience_vae=560, n_iter_logreg/svm=500)

Controlled change vs recover035_longpatience_T80_h10000_p560_full5x5 (1 parameter diff):
  latent_dim : 256 -> 384

All other parameters unchanged:
  channels_to_use=[1,0,2], epochs_vae=10000, cyclical_beta_n_cycles=125,
  lr_scheduler_T0=80, early_stopping_patience_vae=560, beta_vae=2.5,
  dropout_rate_vae=0.15, batch_size=64, n_iter_logreg=500, n_iter_svm=500,
  ...
  metadata_path=patched_metadata_candidate.csv (same as longpatience and recover035).

Scheduler logic (UNCHANGED from longpatience):
  Beta cycle length = 10000 / 125 = 80 epochs
  lr_scheduler_T0 = 80 (phase-aligned)
  Patience = 560 = 7 complete 80-epoch cycles

Motivation: 256 active units in latent_dim=256 model may indicate saturation.
  Test whether latent_dim=384 improves AD/CN discrimination without increasing
  scanner/manufacturer leakage or degrading PR-AUC/BA/F1.
  Single-factor capacity sensitivity — not a broad hyperparameter search.
...
# Single controlled change vs longpatience
EXPECTED_LATENT_DIM = 384
BASE_LATENT_DIM = 256

# Unchanged from longpatience
...
COMPARISON_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/compare_v5_1c_recover035_latent384_T80_h10000_p560_full5x5.py"
)
INTEGRITY_AUDIT_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/audit_v5_1c_recover035_latent384_T80_h10000_p560_integrity.py"
)
POOL_AUDIT_SCRIPT = (
...
CAPACITY_AUDIT_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/audit_latent_capacity_latent384_T80_h10000_p560.py"
)
DYNAMICS_HOOK_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/audit_vae_training_dynamics_latent384_T80_h10000_p560.py"
)

OUT_COMPARISON_DIR = (
    PROJECT_ROOT / "results/revision_bspc_2026/recover035_latent384_T80_h10000_p560_full5x5_comparison"
)
...
def assert_latent384_diff(source: Dict[str, Any], target: Dict[str, Any]) -> None:
    """Verify exactly 1 parameter diff vs recover035_longpatience_T80_h10000_p560_full5x5.

    Parameter diff must be exactly:
      latent_dim : 256 -> 384

    All other parameters, metadata_path, global_tensor_path, channels_to_use must be identical.
    Schedule (epochs_vae=10000, n_cycles=125, T0=80, patience=560, n_iter=500) must be unchanged.
    """
    ...
    if set(src_params) != set(tgt_params):
        raise RuntimeError(
            "Target config parameter keys differ from source (longpatience) config: "
    ...
    require_equal(
        ...
        "scientific parameter diffs (latent384 vs recover035_longpatience_T80_h10000_p560)",
    )
    ...
    # metadata_path must be patched_metadata_candidate.csv (same as longpatience and recover035)
    ...
    if src_meta != tgt_meta:
        raise RuntimeError(
            f"metadata_path changed vs longpatience source ({src_meta!r} -> {tgt_meta!r}). "
            "latent384 must use the same patched_metadata_candidate.csv as longpatience and recover035."
        )


def validate_config(config: Dict[str, Any], source_config: Dict[str, Any]) -> None:
    assert_latent384_diff(source_config, config)
    ...
    # Scheduler invariant: cycle length = epochs / n_cycles = T0
...
def write_manifest(
    ...
    manifest = {
        ...
        "controlled_change": (
            "Base: recover035_longpatience_T80_h10000_p560_full5x5 (patched_metadata_candidate.csv, 035_S_6927 as AD). "
            "Single diff vs longpatience: latent_dim 256->384. "
            "All other parameters unchanged: epochs_vae=10000, n_cycles=125, cycle_len=80, T0=80, "
            "patience=560 (7 cycles), n_iter_logreg/svm=500, beta=2.5, channels=[1,0,2]."
        ),
        "latent384_diff_vs_longpatience_verified": True,
...
def write_command_log(
    ...
    payload = {
        ...
        "controlled_change": (
            "Base: recover035_longpatience_T80_h10000_p560_full5x5. "
            "Single diff: latent_dim 256->384. "
            "Schedule, channels, loss, beta, dropout, batch_size, n_iter unchanged."
        ),
        "recover_subject": RECOVER_SUBJECT,
        "excluded_subject": EXCLUDED_SUBJECT,
        "promotion_rule": (
        ...
        "longpatience_outputs_modified": False,
    }
...
def main() -> int:
    ...
    print(f"Source config    : {args.source_config} (recover035_longpatience_T80_h10000_p560_full5x5)")
    print(f"Run name         : {config['run_name']}")
    print(f"Mode             : {mode}")
    print(f"Controlled diff  : 1 parameter diff vs longpatience (latent_dim 256->384)")
    ...
    print(f"Latent dim       : {BASE_LATENT_DIM} -> {EXPECTED_LATENT_DIM} (only change vs longpatience)")
    print(f"Epochs/cycles    : {params['epochs_vae']}/{params['cyclical_beta_n_cycles']} = {cycle_len:.0f} epoch cycle (SAME as longpatience)")
    print(f"lr_scheduler_T0  : {params['lr_scheduler_T0']} (UNCHANGED from longpatience — phase-aligned)")
    ...
    print(f"n_iter_logreg    : {params['n_iter_logreg']} (UNCHANGED from longpatience)")
    print(f"n_iter_svm       : {params['n_iter_svm']} (UNCHANGED from longpatience)")
    ...
    print("\nPool audit command (same metadata as longpatience):")
