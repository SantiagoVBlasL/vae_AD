#!/usr/bin/env python3
"""Prepare/run the controlled latent384 beta3p5 beta-axis ablation.

New run:
  recover035_latent384_beta3p5_T80_h10000_p560_full5x5
...
DEFAULT_CONFIG = (
    PROJECT_ROOT
    / "configs/runs/adni_v5_1c_recover035_latent384_beta3p5_T80_h10000_p560_full5x5.json"
)
DEFAULT_PREFLIGHT_DIR = (
    PROJECT_ROOT
    / "results/revision_bspc_2026/recover035_latent384_beta3p5_T80_h10000_p560_full5x5_preflight"
)
...
def validate_config(config: Dict[str, Any], reference: Dict[str, Any]) -> Dict[str, Tuple[Any, Any]]:
    ...
    expected_run_name = "recover035_latent384_beta3p5_T80_h10000_p560_full5x5"
    require_equal(
        diffs,
...
def make_oof_command(config: Dict[str, Any], python_exe: str) -> List[str]:
    ...
    return [
        ...
        str(out),
        "--output-dir",
        str(PROJECT_ROOT / "results/revision_bspc_2026/recover035_latent384_beta3p5_stageB_oof_score_calibration"),
    ]
...
def validate_commands(stage_a: Sequence[str], stage_b: Sequence[str], oof: Sequence[str]) -> pd.DataFrame:
    checks = [
        ...
        ("stage_a_beta", values_after_flag(stage_a, "--beta_vae"), ["3.5"]),
...
def write_preflight_outputs(
    ...
    report = [
        "# Latent384 beta3p5 FULL 5x5 Preflight",
        "",
    ...
    command_log = {
        ...
        "controlled_diff": {"beta_vae": [3.75, 3.5]},
...
def write_run_manifest(config: Dict[str, Any], stage_a: Sequence[str], stage_b: Sequence[str], oof: Sequence[str], quarantine: Path | None) -> None:
    ...
    payload = {
        "created_utc": now_utc(),
        "run_name": config["run_name"],
        "controlled_change": "beta_vae 3.75 -> 3.5 only versus promoted latent384 beta3p75 reference",
        "stage_a_command": list(stage_a),
        "stage_b_command": list(stage_b),
        "oof_score_harmonization_command": list(oof),
...
def main() -> int:
    ...
    print("Controlled diff: beta_vae 3.75 -> 3.5 only.")
