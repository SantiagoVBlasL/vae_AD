#!/usr/bin/env python3
"""Prepare/run the controlled exploratory recover035_latent384_beta3p75_T80_h10000_p560 FULL 5x5.

Base model: recover035_latent384_T80_h10000_p560_full5x5
  (patched_metadata_candidate.csv; 035_S_6927 as AD, 128_S_2002 excluded;
   ...
   lr_scheduler_T0=80, early_stopping_patience_vae=560, n_iter=500, beta_vae=2.5)

Controlled change vs recover035_latent384_T80_h10000_p560_full5x5 (1 parameter diff):
  beta_vae : 2.5 -> 3.75
...
All other parameters unchanged:
  ...
  metadata_path=patched_metadata_candidate.csv (same as latent384_beta2.5).

Motivation: latent384_beta2.5 does not promote (Stage B pooled AUC=0.769/PR-AUC=0.499).
  Score-scale audit showed Fold 1 score-range anomaly (0.993 vs 0.43 for Folds 2-5).
  Hypothesis: stronger beta bottleneck (3.75 = 1.5x beta2.5) may improve information
  disentanglement, stabilise cross-fold score scale, and raise pooled PR-AUC.
  Single-factor beta sensitivity — not a broad hyperparameter search.
...
SOURCE_CONFIG = (
    PROJECT_ROOT
    / "configs/runs/adni_v5_1c_recover035_latent384_T80_h10000_p560_full5x5.json"
)
DEFAULT_CONFIG = (
    PROJECT_ROOT
...
# Single controlled change vs latent384_beta2.5
EXPECTED_BETA = 3.75
BASE_BETA = 2.5
# Unchanged from base
...
COMPARISON_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/compare_v5_1c_recover035_latent384_beta3p75_T80_h10000_p560_full5x5.py"
)
INTEGRITY_AUDIT_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/audit_v5_1c_recover035_latent384_beta3p75_T80_h10000_p560_integrity.py"
)
POOL_AUDIT_SCRIPT = (
...
CAPACITY_AUDIT_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/audit_latent_capacity_beta3p75_latent384.py"
)
DYNAMICS_HOOK_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/audit_vae_training_dynamics_beta3p75_vs_beta2p5_latent384.py"
)
SCORE_SCALE_AUDIT_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/audit_stageB_score_scale_beta3p75_latent384.py"
)
PHILIPS_FP_AUDIT_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/audit_philips_cn_fp_beta3p75_latent384.py"
)

OUT_COMPARISON_DIR = (
    PROJECT_ROOT / "results/revision_bspc_2026/recover035_latent384_beta3p75_T80_h10000_p560_full5x5_comparison"
)
...
def assert_beta3p75_diff(source: Dict[str, Any], target: Dict[str, Any]) -> None:
    """Verify exactly 1 parameter diff vs recover035_latent384_T80_h10000_p560_full5x5.
    ...
    if set(src_params) != set(tgt_params):
        raise RuntimeError(
            "Target config parameter keys differ from source (latent384_beta2.5) config: "
    ...
    require_equal(
        ...
        "scientific parameter diffs (beta3p75 vs latent384_beta2.5)",
    )
    ...
    if src_meta != tgt_meta:
        raise RuntimeError(
            f"metadata_path changed vs latent384_beta2.5 ({src_meta!r} -> {tgt_meta!r}). "
            "beta3p75 must use the same patched_metadata_candidate.csv."
        )
    # latent_dim must remain 384
...
def validate_config(config: Dict[str, Any], source_config: Dict[str, Any]) -> None:
    assert_beta3p75_diff(source_config, config)
...
def write_manifest(
    ...
    manifest = {
        ...
        "controlled_change": (
            "Base: recover035_latent384_T80_h10000_p560_full5x5 (patched_metadata_candidate.csv, 035_S_6927 as AD). "
            f"Single diff vs latent384_beta2.5: beta_vae {BASE_BETA}->{EXPECTED_BETA}. "
        ...
        "beta3p75_diff_vs_latent384_beta2p5_verified": True,
...
def write_command_log(
    ...
    payload = {
        ...
        "controlled_change": (
            "Base: recover035_latent384_T80_h10000_p560_full5x5. "
        ...
        "latent384_beta2p5_outputs_modified": False,
    }
...
def main() -> int:
    ...
    print(f"Source config    : {args.source_config} (recover035_latent384_beta2.5)")
    print(f"Run name         : {config['run_name']}")
    print(f"Mode             : {mode}")
    print(f"Controlled diff  : 1 parameter diff vs latent384_beta2.5 (beta_vae {BASE_BETA}->{EXPECTED_BETA})")
    ...
    print(f"beta_vae         : {BASE_BETA} -> {EXPECTED_BETA} (only change vs latent384_beta2.5)")
    print(f"latent_dim       : {EXPECTED_LATENT_DIM} (UNCHANGED)")
    print(f"Epochs/cycles    : {params['epochs_vae']}/{params['cyclical_beta_n_cycles']} = {cycle_len:.0f} epoch cycle (SAME as latent384_beta2.5)")
    ...
    print("\nPool audit command (same metadata as latent384_beta2.5):")
