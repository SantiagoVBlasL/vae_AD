#!/usr/bin/env python3
"""Launcher/preflight for weighted Pearson50 reconstruction loss.
...
DEFAULT_CONFIG = PROJECT_ROOT / "configs/runs/adni_v5_1c_recover035_latent384_beta3p75_chweightedPearson50_T80_h10000_p560_full5x5.json"
RUN_NAME = "recover035_latent384_beta3p75_chweightedPearson50_T80_h10000_p560_full5x5"
EXPECTED_DIFF = {
    "recon_loss_channel_weights": (None, [0.5, 0.25, 0.25]),
...
def validate_config(ref: dict[str, Any], cand: dict[str, Any]) -> None:
    ...
    weights = cand["parameters"].get("recon_loss_channel_weights")
    if weights != [0.5, 0.25, 0.25]:
        raise RuntimeError(f"Unexpected channel weights: {weights}")
    if abs(sum(float(x) for x in weights) - 1.0) > 1e-9:
        raise RuntimeError(f"Channel weights must sum to 1.0, got {weights}")
    if cand["parameters"].get("channels_to_use") != [1, 0, 2]:
        raise RuntimeError("Weighted branch must keep selected channels [1,0,2].")
