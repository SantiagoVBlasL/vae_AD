#!/usr/bin/env python3
"""Prepare/run the controlled exploratory recover035_latent384_beta4p0_T80_h10000_p560 FULL 5x5.

Base model: recover035_latent384_beta3p75_T80_h10000_p560_full5x5
  (patched_metadata_candidate.csv; 035_S_6927 as AD, 128_S_2002 excluded;
   latent_dim=384, epochs_vae=10000, cyclical_beta_n_cycles=125,
   lr_scheduler_T0=80, early_stopping_patience_vae=560, n_iter=500, beta_vae=3.75)

Controlled change vs recover035_latent384_beta3p75_T80_h10000_p560_full5x5 (1 parameter diff):
  beta_vae : 3.75 -> 4.0

All other parameters unchanged:
  latent_dim=384, channels_to_use=[1,0,2], epochs_vae=10000, cyclical_beta_n_cycles=125,
  lr_scheduler_T0=80, early_stopping_patience_vae=560, n_iter_logreg=500, n_iter_svm=500,
  dropout_rate_vae=0.15, batch_size=64,
  ...
  metadata_path=patched_metadata_candidate.csv (same as beta3p75).

Motivation: beta3p75 promoted (OOF-logitz AUC=0.7951, PR-AUC=0.5728).
  Hypothesis: slight increase in beta bottleneck (4.0 = 1.067x beta3p75) may further
  improve information disentanglement and reduce Philips CN false-positive rate
  without sacrificing AUC. Single-factor beta sensitivity — not a broad hyperparameter search.
...
DEFAULT_CONFIG = (
    PROJECT_ROOT
    / "configs/runs/adni_v5_1c_recover035_latent384_beta4p0_T80_h10000_p560_full5x5.json"
)
...
# Single controlled change vs latent384_beta3p75
EXPECTED_BETA = 4.0
BASE_BETA = 3.75
# Unchanged from base
...
COMPARISON_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/compare_v5_1c_recover035_latent384_beta4p0_T80_h10000_p560_full5x5.py"
)
INTEGRITY_AUDIT_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/audit_v5_1c_recover035_latent384_beta4p0_T80_h10000_p560_integrity.py"
)
POOL_AUDIT_SCRIPT = (
...
CAPACITY_AUDIT_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/audit_latent_capacity_beta4p0_latent384.py"
)
DYNAMICS_HOOK_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/audit_vae_training_dynamics_beta4p0_vs_beta3p75_latent384.py"
)
SCORE_SCALE_AUDIT_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/audit_stageB_score_scale_beta4p0_latent384.py"
)
PHILIPS_FP_AUDIT_SCRIPT = (
    PROJECT_ROOT
    / "scripts/revision_bspc_2026/audit_philips_cn_fp_beta4p0_latent384.py"
)

OUT_COMPARISON_DIR = (
    PROJECT_ROOT / "results/revision_bspc_2026/recover035_latent384_beta4p0_T80_h10000_p560_full5x5_comparison"
)
...
def assert_beta4p0_diff(source: Dict[str, Any], target: Dict[str, Any]) -> None:
    """Verify exactly 1 parameter diff vs recover035_latent384_beta3p75_T80_h10000_p560_full5x5.

    Parameter diff must be exactly:
      beta_vae : 3.75 -> 4.0

    All other parameters, metadata_path, global_tensor_path, channels_to_use must be identical.
    latent_dim must remain 384. Schedule (epochs_vae=10000, n_cycles=125, T0=80, patience=560) unchanged.
    """
    ...
    if set(src_params) != set(tgt_params):
        raise RuntimeError(
            "Target config parameter keys differ from source (latent384_beta3p75) config: "
    ...
    require_equal(
        diffs,
        {"beta_vae": (BASE_BETA, EXPECTED_BETA)},
        "scientific parameter diffs (beta4p0 vs latent384_beta3p75)",
    )
    ...
    if src_meta != tgt_meta:
        raise RuntimeError(
            f"metadata_path changed vs latent384_beta3p75 ({src_meta!r} -> {tgt_meta!r}). "
            "beta4p0 must use the same patched_metadata_candidate.csv."
        )
    if tgt_params.get("latent_dim") != EXPECTED_LATENT_DIM:
        raise RuntimeError(
            f"latent_dim must remain {EXPECTED_LATENT_DIM}, got {tgt_params.get('latent_dim')!r}"
        )


def validate_config(config: Dict[str, Any], source_config: Dict[str, Any]) -> None:
    assert_beta4p0_diff(source_config, config)
...
def build_capacity_audit_command(config: Dict[str, Any], python_exe: str) -> List[str]:
    outdir = resolve(config["paths"]["output_dir"])
    return [
        python_exe, str(CAPACITY_AUDIT_SCRIPT),
        "--beta4p0-run-dir", str(outdir),
        "--dry-run",
    ]
...
def build_score_scale_audit_command(config: Dict[str, Any], python_exe: str) -> List[str]:
    outdir = resolve(config["paths"]["output_dir"])
    return [
        python_exe, str(SCORE_SCALE_AUDIT_SCRIPT),
        "--run-dir", str(outdir),
        "--dry-run",
    ]


def build_philips_fp_audit_command(config: Dict[str, Any], python_exe: str) -> List[str]:
    outdir = resolve(config["paths"]["output_dir"])
    return [
        python_exe, str(PHILIPS_FP_AUDIT_SCRIPT),
        "--run-dir", str(outdir),
        "--dry-run",
    ]
...
def validate_stage_a_command(command: Sequence[str], config: Dict[str, Any]) -> None:
    ...
    for flag, expected in [
        ("--outer_folds", ["5"]),
        ("--inner_folds", ["5"]),
        ("--latent_dim", [str(EXPECTED_LATENT_DIM)]),
...
def write_manifest(
    ...
    manifest = {
        ...
        "controlled_change": (
            "Base: recover035_latent384_beta3p75_T80_h10000_p560_full5x5 (patched_metadata_candidate.csv, 035_S_6927 as AD). "
            f"Single diff vs latent384_beta3p75: beta_vae {BASE_BETA}->{EXPECTED_BETA}. "
            "All other parameters unchanged: latent_dim=384, epochs_vae=10000, n_cycles=125, cycle_len=80, T0=80, "
            "patience=560 (7 cycles), n_iter_logreg/svm=500, channels=[1,0,2]."
        ),
        "beta4p0_diff_vs_latent384_beta3p75_verified": True,
...
def write_command_log(
    ...
    payload = {
        ...
        "controlled_change": (
            "Base: recover035_latent384_beta3p75_T80_h10000_p560_full5x5. "
            f"Single diff: beta_vae {BASE_BETA}->{EXPECTED_BETA}. "
            "Schedule, latent_dim, channels, loss, dropout, batch_size, n_iter unchanged."
        ),
        "recover_subject": RECOVER_SUBJECT,
        "excluded_subject": EXCLUDED_SUBJECT,
        "promotion_rule": (
        ...
        "latent384_beta3p75_outputs_modified": False,
    }
...
def main() -> int:
    ...
    capacity_audit = build_capacity_audit_command(config, python_exe)
    dynamics_hook = build_dynamics_hook_command(config, python_exe)
    score_scale_audit = build_score_scale_audit_command(config, python_exe)
    philips_fp_audit = build_philips_fp_audit_command(config, python_exe)
    output_dir = resolve(config["paths"]["output_dir"])
    big_disk = Path(config["paths"]["big_disk_output_dir"])
    stale = stale_output_markers(output_dir)
    ...
    print(f"Source config    : {args.source_config} (recover035_latent384_beta3p75)")
    print(f"Run name         : {config['run_name']}")
    print(f"Mode             : {mode}")
    print(f"Controlled diff  : 1 parameter diff vs latent384_beta3p75 (beta_vae {BASE_BETA}->{EXPECTED_BETA})")
    ...
    print(f"beta_vae         : {BASE_BETA} -> {EXPECTED_BETA} (only change vs latent384_beta3p75)")
    print(f"latent_dim       : {EXPECTED_LATENT_DIM} (UNCHANGED)")
    print(f"Epochs/cycles    : {params['epochs_vae']}/{params['cyclical_beta_n_cycles']} = {cycle_len:.0f} epoch cycle (SAME as latent384_beta3p75)")
    print(f"lr_scheduler_T0  : {params['lr_scheduler_T0']} (UNCHANGED — phase-aligned)")
    print(f"Patience         : {params['early_stopping_patience_vae']} = {params['early_stopping_patience_vae'] // EXPECTED_CYCLE_LEN} × {EXPECTED_CYCLE_LEN}-epoch cycles (UNCHANGED)")
    print(f"n_iter_logreg    : {params['n_iter_logreg']} (UNCHANGED)")
    print(f"n_iter_svm       : {params['n_iter_svm']} (UNCHANGED)")
    print(f"Stage B C-grid   : original [1e-3, 1e-2, 1e-1, 1.0]")
    ...
    print("\nPool audit command (same metadata as latent384_beta3p75):")
    print(shlex.join(pool_audit))
    print("\nLatent capacity audit command (dry-run):")
    print(shlex.join(capacity_audit))
    print("\nTraining dynamics audit command (dry-run):")
    print(shlex.join(dynamics_hook))
    print("\nScore-scale audit command (dry-run):")
    print(shlex.join(score_scale_audit))
    print("\nPhilips CN FP audit command (dry-run):")
    print(shlex.join(philips_fp_audit))
    print("\nClean-run policy:")
    ...
    symlink_target = big_disk
    symlink_path = output_dir
    symlink_exists = symlink_path.is_symlink() and symlink_path.resolve() == symlink_target.resolve()
    print("\n[Expected output symlink]")
    if symlink_exists:
        print(f"  Symlink already in place: {symlink_path} -> {symlink_target}  [OK]")
    else:
        print(f"  Symlink NOT yet created. Before launching real training, run:")
        print(f"    mkdir -p {shlex.quote(str(symlink_target))}")
        print(f"    ln -s {shlex.quote(str(symlink_target))} {shlex.quote(str(symlink_path))}")
