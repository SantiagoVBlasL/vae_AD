#!/usr/bin/env python3
"""Prepare/run the controlled latent448 beta4p0 capacity/beta ablation.

New run:
  recover035_latent448_beta4p0_T80_h10000_p560_full5x5
...
DEFAULT_CONFIG = (
    PROJECT_ROOT
    / "configs/runs/adni_v5_1c_recover035_latent448_beta4p0_T80_h10000_p560_full5x5.json"
)
DEFAULT_PREFLIGHT_DIR = (
    PROJECT_ROOT
    / "results/revision_bspc_2026/recover035_latent448_beta4p0_T80_h10000_p560_full5x5_preflight"
)
...
def validate_config(config: Dict[str, Any], reference: Dict[str, Any]) -> Dict[str, Tuple[Any, Any]]:
    ...
    expected_run_name = "recover035_latent448_beta4p0_T80_h10000_p560_full5x5"
    require_equal(
        diffs,
        {"beta_vae": (BASE_BETA, EXPECTED_BETA), "latent_dim": (BASE_LATENT_DIM, EXPECTED_LATENT_DIM)},
        "effective scientific config diff vs promoted latent384 beta3p75",
    )
    require_equal(config["run_name"], expected_run_name, "run_name")
    require_equal(Path(config["paths"]["output_dir"]).name, expected_run_name, "paths.output_dir provenance name")
    require_equal(Path(config["paths"]["big_disk_output_dir"]).name, expected_run_name, "paths.big_disk_output_dir provenance name")
    require_equal(
        Path(config["paths"]["split_preview_csv"]).name,
        f"{expected_run_name}_split_preview.csv",
        "paths.split_preview_csv provenance name",
    )
    require_equal(
        Path(config["paths"]["split_preview_summary_csv"]).name,
        f"{expected_run_name}_split_preview_summary.csv",
        "paths.split_preview_summary_csv provenance name",
    )
    require_equal(params["latent_dim"], EXPECTED_LATENT_DIM, "latent_dim")
    require_equal(params["beta_vae"], EXPECTED_BETA, "beta_vae")
...
def make_split_preview(meta: pd.DataFrame, config: Dict[str, Any]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    return preview, pd.DataFrame(subject_rows)
...
def make_oof_command(config: Dict[str, Any], python_exe: str) -> List[str]:
    ...
    return [
        ...
        str(out),
        "--output-dir",
        str(PROJECT_ROOT / "results/revision_bspc_2026/recover035_latent448_beta4p0_stageB_oof_score_calibration"),
    ]
...
def validate_commands(stage_a: Sequence[str], stage_b: Sequence[str], oof: Sequence[str]) -> pd.DataFrame:
    checks = [
        ...
        ("stage_a_latent_dim", values_after_flag(stage_a, "--latent_dim"), ["448"]),
        ("stage_a_beta", values_after_flag(stage_a, "--beta_vae"), ["4.0"]),
...
def write_preflight_outputs(
    ...
    report = [
        "# Latent448 beta4p0 FULL 5x5 Preflight",
        "",
        ...
        "- `latent_dim: 384 -> 448`",
        "- `beta_vae: 3.75 -> 4.0`",
        "- All other scientific parameters are unchanged from the promoted reference.",
        "",
        "## Validation",
        "",
        f"- JSON syntax: OK.",
        ...
        "## Guardrail",
        "",
        "The launcher refuses real training without `--confirm-training`; it also refuses stale output folders unless `--force-clean` is used to quarantine existing contents.",
    ]
    ...
    command_log = {
        ...
        "controlled_diff": {"latent_dim": [384, 448], "beta_vae": [3.75, 4.0]},
...
def write_run_manifest(config: Dict[str, Any], stage_a: Sequence[str], stage_b: Sequence[str], oof: Sequence[str], quarantine: Path | None) -> None:
    ...
    payload = {
        "created_utc": now_utc(),
        "run_name": config["run_name"],
        "controlled_change": "latent_dim 384 -> 448 and beta_vae 3.75 -> 4.0 only versus promoted latent384 beta3p75 reference",
        "stage_a_command": list(stage_a),
        "stage_b_command": list(stage_b),
        "oof_score_harmonization_command": list(oof),
        "quarantine_dir": str(quarantine) if quarantine else "",
        "promotion_gate": "AUC > 0.795155 and PR-AUC >= 0.573934, or Fold4/BA/F1 improvement without material AUC/PR-AUC harm",
    }
...
def main() -> int:
    ...
    split_summary, split_subjects = make_split_preview(meta, config)
    ...
    print("Controlled diff: latent_dim 384 -> 448; beta_vae 3.75 -> 4.0 only.")
