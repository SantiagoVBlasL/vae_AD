#!/usr/bin/env python3
"""Prepare/run the controlled latent512 capacity ablation.

New run:
  recover035_latent512_beta3p75_T80_h10000_p560_full5x5
...
DEFAULT_CONFIG = (
    PROJECT_ROOT
    / "configs/runs/adni_v5_1c_recover035_latent512_beta3p75_T80_h10000_p560_full5x5.json"
)
DEFAULT_PREFLIGHT_DIR = (
    PROJECT_ROOT
    / "results/revision_bspc_2026/recover035_latent512_beta3p75_T80_h10000_p560_full5x5_preflight"
)
...
def validate_config(config: Dict[str, Any], reference: Dict[str, Any]) -> Dict[str, Tuple[Any, Any]]:
    ...
    require_equal(diffs, {"latent_dim": (384, 512)}, "effective config diff vs promoted latent384")
    require_equal(config["run_name"], "recover035_latent512_beta3p75_T80_h10000_p560_full5x5", "run_name")
    require_equal(params["latent_dim"], 512, "latent_dim")
...
def make_oof_command(config: Dict[str, Any], python_exe: str) -> List[str]:
    ...
    return [
        ...
        str(out),
        "--output-dir",
        str(PROJECT_ROOT / "results/revision_bspc_2026/recover035_latent512_beta3p75_stageB_oof_score_calibration"),
    ]
...
def validate_commands(stage_a: Sequence[str], stage_b: Sequence[str], oof: Sequence[str]) -> pd.DataFrame:
    checks = [
        ...
        ("stage_a_latent_dim", values_after_flag(stage_a, "--latent_dim"), ["512"]),
...
def write_preflight_outputs(
    ...
    report = [
        "# Latent512 beta3p75 FULL 5x5 Preflight",
        "",
        ...
        "",
        "- `latent_dim: 384 -> 512`",
        "- `beta_vae` remains `3.75`.",
        "- No beta scaling to 5.0 is used because beta4p0 was already negative for AUC/PR-AUC, Philips CN FPR, and scanner leakage.",
        "",
        "## Validation",
        "",
        f"- JSON syntax: OK.",
    ...
    command_log = {
        ...
        "controlled_diff": {"latent_dim": [384, 512]},
...
def write_run_manifest(config: Dict[str, Any], stage_a: Sequence[str], stage_b: Sequence[str], oof: Sequence[str], quarantine: Path | None) -> None:
    ...
    payload = {
        "created_utc": now_utc(),
        "run_name": config["run_name"],
        "controlled_change": "latent_dim 384 -> 512 only versus promoted latent384 beta3p75 reference",
        "stage_a_command": list(stage_a),
        "stage_b_command": list(stage_b),
        "oof_score_harmonization_command": list(oof),
...
def main() -> int:
    ...
    print("Controlled diff: latent_dim 384 -> 512 only.")
