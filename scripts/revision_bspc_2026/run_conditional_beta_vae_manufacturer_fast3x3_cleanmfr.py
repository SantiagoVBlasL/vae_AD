#!/usr/bin/env python3
"""Launcher for conditional Manufacturer beta-VAE FAST 3x3 — clean-Manufacturer pool.

Identical to ``run_conditional_beta_vae_manufacturer_fast3x3.py`` except that
every Stage A command also receives:

    --vae_required_metadata_cols Manufacturer

This removes subjects with missing Manufacturer from the VAE train/val pool
**before** the pool split, fold-locally, with per-fold QC CSVs.  The filter
applies to ALL four candidates (including unconditioned baselines) so that all
four are trained on the same pool and are directly comparable.

The original ``conditional_beta_vae_manufacturer_fast3x3`` baselines used the
full pool (including Manufacturer-missing subjects) and are NOT valid
comparators for the conditioning candidates here.
...
BIG_DISK_RUN_ROOT = Path("/media/diego/Datos/vae_AD_results/revision_bspc_2026/conditional_beta_vae_manufacturer_fast3x3_cleanmfr/runs")
...
AGGREGATOR_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/aggregate_conditional_beta_vae_manufacturer_fast3x3_cleanmfr.py"
...
def check_datos_drive() -> None:
    """Abort with actionable message if the Datos drive is not mounted."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--status", action="store_true", help="Print per-candidate status audit and exit (read-only, no drive check).")
...
def read_matrix(preflight_root: Path) -> pd.DataFrame:
    ...
    if df["candidate_id"].tolist() != EXPECTED_CANDIDATES:
        raise ValueError("Preflight matrix candidate order does not match the validated 4-candidate plan.")
...
def candidate_dirs(output_root: Path, big_disk_run_root: Path, candidate_id: str, no_external_symlink: bool) -> Dict[str, Path]:
    ...
    readout = local_run / f"classifier_only_readout_unused"
...
def build_commands(row: pd.Series, preflight_root: Path, output_root: Path, big_disk_run_root: Path, python_executable: str, no_external_symlink: bool, force_clean: bool) -> Dict[str, Any]:
    ...
    # cleanmfr: require non-missing Manufacturer in VAE pool for all candidates
...
def validate_stage_a_command(spec: Dict[str, Any], real: bool) -> List[str]:
    ...
    if "Manufacturer" in command_values_after(cmd, "--metadata_features"):
        errors.append(f"{spec['candidate_id']}: Manufacturer must not be passed to the Stage B classifier metadata features.")
    ...
    # cleanmfr-specific: require VAE pool filter for Manufacturer in all candidates
    if command_values_after(cmd, "--vae_required_metadata_cols") != ["Manufacturer"]:
        errors.append(
            f"{spec['candidate_id']}: cleanmfr requires --vae_required_metadata_cols Manufacturer "
            f"in all Stage A commands (got {command_values_after(cmd, '--vae_required_metadata_cols')})."
        )
...
def folds_stage_a_complete_count(run_dir: Path) -> int:
    """Return how many of the 3 outer folds have all Stage A artifacts."""
...
def aggregate_ready(output_root: Path) -> bool:
    """True if aggregator outputs exist under the experiment output_root."""
...
def run_status_audit(args: argparse.Namespace) -> int:
    """Print per-candidate status table. Read-only; tolerates unmounted Datos drive."""
    datos_mounted = DATOS_MOUNT.exists() and os.path.ismount(str(DATOS_MOUNT))
    if not datos_mounted:
        print(
            f"[WARN] Datos drive not mounted ({DATOS_MOUNT}). "
            "big_run_dir and Stage A status may be incomplete.\n"
            "       Run: udisksctl mount -b /dev/sda1"
        )
    ...
    for _, row in rows.iterrows():
        ...
        audit_rows.append({
            ...
            "folds_stageA_complete": f"{n_folds}/3",
    ...
    print("\n=== Candidate Status Audit (cleanmfr) ===")
...
def write_manifest(output_root: Path, rows: List[Dict[str, Any]], dry_run: bool) -> pd.DataFrame:
    ...
    lines = [
        "# Conditional beta-VAE Manufacturer FAST 3x3 — clean-Manufacturer pool",
        "",
        f"Generated UTC: {now_utc()}",
        f"Dry-run only: {dry_run}",
        "",
        "All four candidates use --vae_required_metadata_cols Manufacturer: subjects with missing",
        "Manufacturer are removed from the VAE train/val pool per fold before any split.",
        "This is the ONLY valid matched comparison for Manufacturer conditioning.",
        "",
...
def main() -> int:
    ...
    # Status audit: purely read-only, works without Datos drive
    if args.status:
    ...
    # Datos drive must be mounted before tensor/metadata path validation
    check_datos_drive()
    ...
    for spec in specs:
        ...
        log_path = output_root / "logs" / f"cleanmfr_fast3x3_{spec['candidate_id']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    if not args.skip_aggregation:
        ...
        code = run_logged(aggregate_cmd, output_root / "logs" / f"cleanmfr_fast3x3_aggregate_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
        if code != 0:
