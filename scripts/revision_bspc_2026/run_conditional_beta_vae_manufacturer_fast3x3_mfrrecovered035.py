#!/usr/bin/env python3
"""Launcher for conditional Manufacturer beta-VAE FAST 3x3 — recovered-035 Manufacturer pool.
...
3. With --vae_required_metadata_cols Manufacturer:
...
Expected outcome vs cleanmfr:
  ...
  - mfrrecovered035: 1 subject removed per fold (128_S_2002 only)
...
OUTPUT_ROOT = PROJECT_ROOT / "results/revision_bspc_2026/conditional_beta_vae_manufacturer_fast3x3_mfrrecovered035"
BIG_DISK_RUN_ROOT = Path("/media/diego/Datos/vae_AD_results/revision_bspc_2026/conditional_beta_vae_manufacturer_fast3x3_mfrrecovered035/runs")
...
AGGREGATOR_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/aggregate_conditional_beta_vae_manufacturer_fast3x3_mfrrecovered035.py"
...
PATCHED_METADATA_SUBPATH = Path("metadata/training_ready_metadata_v5_1b_mfrrecovered035.csv")
...
def read_matrix(preflight_root: Path) -> pd.DataFrame:
    ...
    if df["candidate_id"].tolist() != EXPECTED_CANDIDATES:
        raise ValueError("Preflight matrix candidate order does not match validated 4-candidate plan.")
...
def write_patch_audit(output_root: Path, audit_info: Dict[str, Any]) -> None:
    ...
    decision_lines = [
        ...
        "- Classifier pool: unchanged (neither subject is in classifier pool; 035_S_6927 has NaN Age/Sex)",
        "",
...
def run_status_audit(args: argparse.Namespace) -> int:
    ...
    print(f"\n=== Candidate Status Audit (mfrrecovered035) ===")
...
def write_manifest(output_root: Path, rows: List[Dict[str, Any]], dry_run: bool, patched_metadata_path: Path) -> pd.DataFrame:
    ...
    lines = [
        "# Conditional beta-VAE Manufacturer FAST 3x3 — recovered-035 Manufacturer pool (mfrrecovered035)",
        "",
        ...
        "",
        "All four candidates use --vae_required_metadata_cols Manufacturer.",
        "After the patch: only 128_S_2002 is removed per fold (1 subject instead of 2).",
        "The cleanmfr baselines are NOT valid comparators here — use mfrrecovered035 matched baselines.",
        "",
        "Stage A trains FAST VAE with dummy canonical logreg (n_iter_logreg=1), ignored for ranking.",
