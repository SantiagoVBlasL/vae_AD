#!/usr/bin/env python3
"""Launcher for conditional Manufacturer beta-VAE FAST-long 3x3 — recovered-035 VAE pool, classifier pool locked.
...
BIG_DISK_RUN_ROOT = Path("/media/diego/Datos/vae_AD_results/revision_bspc_2026/conditional_beta_vae_manufacturer_fastlong_mfrrecovered035_clfpoollocked/runs")
...
AGGREGATOR_SCRIPT = PROJECT_ROOT / "scripts/revision_bspc_2026/aggregate_conditional_beta_vae_manufacturer_fastlong_mfrrecovered035_clfpoollocked.py"
...
def build_commands(
    ...
    stage_a = replace_flag_value(stage_a, "--epochs_vae", ["1920"])
    stage_a = replace_flag_value(stage_a, "--cyclical_beta_n_cycles", ["24"])
...
def run_status_audit(args: argparse.Namespace) -> int:
    ...
    print(f"\n=== Candidate Status Audit (mfrrecovered035_clfpoollocked FAST-long) ===")
...
def write_fastlong_target_validation(output_root: Path) -> None:
    df = pd.DataFrame(FASTLONG_TARGET_VALIDATION_ROWS)
    df.to_csv(output_root / "fastlong_target_validation.csv", index=False)
    write_markdown_table(output_root / "fastlong_target_validation.md", df)
...
def write_manifest(output_root: Path, rows: List[Dict[str, Any]], dry_run: bool, patched_metadata_path: Path) -> pd.DataFrame:
    ...
    lines = [
        "# Conditional beta-VAE Manufacturer FAST-long 3x3 — recovered-035 VAE pool, classifier pool locked",
        "",
        ...
        "",
        "Both FAST-long candidates use --vae_required_metadata_cols Manufacturer.",
        "After the patch: only 128_S_2002 is removed from the VAE pool per fold (1 subject instead of 2).",
        "The cleanmfr classifier pool must remain identical; only the VAE pool changes by recovering 035_S_6927.",
        "",
        "## FAST-long target selection",
        "",
        "This focused FAST-long diagnostic targets `[1,0,2]`, not `[1]`, because the training-maturity audit",
        ...
        "`ch1_0_2_decoder_only_manufacturer`.",
        "",
        "Stage A trains FAST-long VAE (1920 epochs / 24 cycles) with dummy canonical logreg (n_iter_logreg=1), ignored for ranking.",
        "Stage B is classifier-only logreg_l2 on z plus Age/Sex with true inner-CV OOF thresholds.",
        "",
    ]
    (output_root / "README.md").write_text("\n".join(lines), encoding="utf-8")
    write_fastlong_target_validation(output_root)
    return df
...
def main() -> int:
    ...
    command_log: Dict[str, Any] = {
        ...
        "fastlong_target_validation": FASTLONG_TARGET_VALIDATION_ROWS,
        "target_note": (
            "Focused FAST-long targets ch1_0_2_decoder_only_manufacturer, not ch1_decoder_only_manufacturer, "
