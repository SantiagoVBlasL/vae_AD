...
def _sha256_ndarray(array: np.ndarray) -> str:
...
def _save_foldwise_combat_tensor_samples(
    fold_output_dir: Path,
    *,
    fold: int,
    split_name: str,
    ...
    """Persist deterministic pre/post examples without saving a full tensor copy."""
    if len(subjects) != len(before) or len(before) != len(after):
        raise ValueError(
            f"Fold {fold} {split_name}: sample lineage length mismatch "
            f"subjects={len(subjects)}, before={len(before)}, after={len(after)}"
        )
    n_sample = min(int(max_subjects), len(subjects))
    sample_idx = np.arange(n_sample, dtype=int)
    subject_ids = subjects.iloc[sample_idx]["SubjectID"].astype(str).to_numpy()
    tensor_indices = subjects.iloc[sample_idx]["tensor_idx"].to_numpy(dtype=int)
    before_sample = np.asarray(before[sample_idx], dtype=np.float32)
    after_sample = np.asarray(after[sample_idx], dtype=np.float32)
    path = fold_output_dir / f"input_harmonization_{split_name}_sample_tensors.npz"
    np.savez_compressed(
        path,
        fold=np.asarray([int(fold)], dtype=np.int64),
        split_name=np.asarray([str(split_name)]),
        SubjectID=subject_ids,
        tensor_idx=tensor_indices,
        raw_input=before_sample,
        harmonized_input=after_sample,
    )
    return {
        "fold": int(fold),
        "split": str(split_name),
        "path": str(path),
        "file_sha256": _compute_file_sha256(path),
        "n_total_subjects": int(len(subjects)),
        "n_sample_subjects": int(n_sample),
        "subject_ids": ";".join(subject_ids.tolist()),
        "tensor_indices": ";".join(map(str, tensor_indices.tolist())),
        "raw_sample_sha256": _sha256_ndarray(before_sample),
        "harmonized_sample_sha256": _sha256_ndarray(after_sample),
        "max_abs_pre_post_delta": float(np.max(np.abs(after_sample - before_sample))),
        "all_values_finite": bool(
            np.isfinite(before_sample).all() and np.isfinite(after_sample).all()
        ),
    }


def _save_harmonized_downstream_latent_cache(
    output_base_dir: Path,
    *,
    fold: int,
    train_dev_subjects: pd.DataFrame,
    test_subjects: pd.DataFrame,
    train_dev_mu: np.ndarray,
    test_mu: np.ndarray,
    fitted_combat_path: Path,
) -> None:
    """Save posterior means computed in-process from fold-harmonized inputs."""
    cache_dir = output_base_dir / "downstream_diagnostic_classifier" / "latent_cache"
    cache_dir.mkdir(parents=True, exist_ok=True)
    mu_columns = [f"mu_{i}" for i in range(int(train_dev_mu.shape[1]))]
    required_metadata = [
        "SubjectID",
        "tensor_idx",
    ...
    for split, subjects, mu in [
        ("trainDev", train_dev_subjects, train_dev_mu),
        ("test", test_subjects, test_mu),
    ]:
        missing = [column for column in required_metadata if column not in subjects.columns]
        if missing:
            raise ValueError(f"Fold {fold} {split}: missing cache metadata columns: {missing}")
        if len(subjects) != len(mu):
            raise ValueError(
                f"Fold {fold} {split}: subject/latent mismatch "
                f"subjects={len(subjects)}, latents={len(mu)}"
            )
        if not np.isfinite(mu).all():
            raise ValueError(f"Fold {fold} {split}: posterior means contain non-finite values")
        frame = subjects[required_metadata].reset_index(drop=True).copy()
        frame["y"] = frame["ResearchGroup_Mapped"].map({"CN": 0, "AD": 1})
        if frame["y"].isna().any():
            raise ValueError(f"Fold {fold} {split}: cache contains non-CN/AD subjects")
        ...
        frame = pd.concat(
            [
                frame,
                pd.DataFrame(np.asarray(mu, dtype=np.float32), columns=mu_columns),
            ],
            axis=1,
        )
        cache_path = cache_dir / f"fold_{fold}_{split}_latent_mu.csv"
        frame.to_csv(cache_path, index=False)
        lineage = {
            "fold": int(fold),
            "split": str(split),
            "cache_path": str(cache_path),
            "cache_sha256": _compute_file_sha256(cache_path),
            "n_subjects": int(len(frame)),
            "latent_dim": int(len(mu_columns)),
            "subject_ids_sha256": hashlib.sha256(
                "\n".join(frame["SubjectID"].astype(str)).encode("utf-8")
            ).hexdigest(),
            ...
            "combat_object_path": str(fitted_combat_path),
            "combat_object_sha256": _compute_file_sha256(fitted_combat_path),
        }
        _safe_json_dump(
            lineage,
            cache_dir / f"fold_{fold}_{split}_latent_mu_lineage.json",
        )
...
def train_and_evaluate_pipeline(global_tensor_all_channels: np.ndarray, 
    ...
    #for fold_idx, (train_dev_clf_idx_in_cn_ad_df, test_clf_idx_in_cn_ad_df) in enumerate(outer_cv_clf.split(X_classifier_subject_indices_in_cn_ad_df, stratify_key_for_clf_cv)):
        ...
        if input_harmonization_mode != "none":
            ...
            fitted_combat_dir = fold_output_dir / "input_harmonization_fitted_objects"
            fitted_combat_dir.mkdir(parents=True, exist_ok=False)
            fitted_combat_path = fitted_combat_dir / "foldwise_combat_tensor.joblib"
            joblib.dump(fitted_combat, fitted_combat_path)
            combat_object_rows = [{
                "fold": int(fold_idx + 1),
                "scope": "all_selected_channels",
                "channel_position": -1,
                "selected_channel_index": "",
                "channel_name": "all",
                "path": str(fitted_combat_path),
                "sha256": _compute_file_sha256(fitted_combat_path),
                "fit_scope": "outer_train_dev_only",
            }]
            for channel_position, channel_wrapper in enumerate(fitted_combat.channels):
                channel_path = (
                    fitted_combat_dir
                    / f"combat_channel_position_{channel_position}_tensor_index_{channel_wrapper.channel_index}.joblib"
                )
                joblib.dump(channel_wrapper, channel_path)
                combat_object_rows.append({
                    "fold": int(fold_idx + 1),
                    "scope": "single_channel",
                    "channel_position": int(channel_position),
                    "selected_channel_index": int(channel_wrapper.channel_index),
                    "channel_name": str(channel_wrapper.channel_name),
                    "path": str(channel_path),
                    "sha256": _compute_file_sha256(channel_path),
                    "fit_scope": "outer_train_dev_only",
                })
            pd.DataFrame(combat_object_rows).to_csv(
                fitted_combat_dir / "fitted_combat_object_manifest.csv",
                index=False,
            )
            ...
            tensor_sample_rows = [
                _save_foldwise_combat_tensor_samples(
                    fold_output_dir,
                    fold=fold_idx + 1,
                    split_name="vae_pool_fit_train_dev",
                    subjects=vae_train_pool_df,
                    before=vae_train_pool_tensor_original_scale,
                    after=vae_train_pool_tensor_harmonized,
                ),
                _save_foldwise_combat_tensor_samples(
                    fold_output_dir,
                    fold=fold_idx + 1,
                    split_name="classifier_train_dev_apply",
                    subjects=clf_train_dev_df_for_harmonization,
                    before=clf_train_dev_tensor_original,
                    after=harmonized_clf_train_dev_tensor_original_scale,
                ),
                _save_foldwise_combat_tensor_samples(
                    fold_output_dir,
                    fold=fold_idx + 1,
                    split_name="classifier_test_apply",
                    subjects=clf_test_df_for_harmonization,
                    before=clf_test_tensor_original,
                    after=harmonized_clf_test_tensor_original_scale,
                ),
            ]
            pd.DataFrame(tensor_sample_rows).to_csv(
                fold_output_dir / "input_harmonization_tensor_sample_manifest.csv",
                index=False,
            )
        ...
        for epoch in range(args.epochs_vae):
            ...
            vae_fold_k.train()
            history_data["learning_rate_start"].append(
                float(optimizer_vae.param_groups[0]["lr"])
            )
            ...
            history_data["learning_rate_end"].append(
                float(optimizer_vae.param_groups[0]["lr"])
            )
        ...
        if args.save_vae_training_history:
            history_data["selected_checkpoint_epoch"] = int(best_epoch)
            history_data["selected_checkpoint_val_loss_modelsel"] = (
                float(best_val_loss) if np.isfinite(best_val_loss) else None
            )
            history_data["terminal_epoch"] = int(final_epoch_seen)
            history_data["selection_rule"] = "minimum_inner_validation_loss_at_beta_max"
        ...
        with torch.no_grad():
            ...
            if input_harmonization_mode == "foldwise_combat":
                if (
                    harmonized_clf_train_dev_tensor_original_scale is None
                    or harmonized_clf_test_tensor_original_scale is None
                ):
                    raise RuntimeError(
                        f"{fold_idx_str} Refusing downstream cache write: "
                        "foldwise ComBat tensors are unavailable."
                    )
                _save_harmonized_downstream_latent_cache(
                    output_base_dir,
                    fold=fold_idx + 1,
                    train_dev_subjects=clf_train_dev_df,
                    test_subjects=cn_ad_df.iloc[test_clf_idx_in_cn_ad_df].copy(),
                    train_dev_mu=mu_train_dev_np,
                    test_mu=mu_test_final_np,
                    fitted_combat_path=fitted_combat_path,
                )
